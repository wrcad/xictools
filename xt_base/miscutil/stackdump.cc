...
//==========================================
//
// Misc. updates and tweaks Steve Whiteley (stevew@wrcad.com).
// $Id: stackdump.cc,v 1.4 2015/06/17 19:08:49 stevew Exp $
...
// Set up an exception handler that will dump a stack trace.
// Microsoft Windows only!
// Warning: this is NOT GOOD for 64 bits.
//
// This is adapted from drmingw-0.4.3, exchndl.cpp.
//
// The header internal.h and libcoff.h files used here are obtained
// from the binutils source.
//
// THESE SHOULD BE CURRENT WITH THE BINUTILS IN YOUR TOOLCHAIN!
//
// Run configure to get libcoff.h if it is not already there.  The
// internal.h needs some lines commented, when updating search for
// "SRW" in the existing file and make the corresponding changes to
// the new file.
//
// in binutils-2.24 locations are:
// libcoff.h        ./bfd
// internal.h       ./include/coff
...
typedef BOOL (WINAPI *PFNSTACKWALK)(DWORD, HANDLE, HANDLE, LPSTACKFRAME,
    LPVOID, PREAD_PROCESS_MEMORY_ROUTINE, PFUNCTION_TABLE_ACCESS_ROUTINE,
...
namespace {
    // This stucture is used to pass information between
    ...
    namespace StackDump {
        int __cdecl rprintf(const TCHAR* ...);
        DWORD GetModuleBase(DWORD);
        bfd_boolean slurp_symtab(bfd*, asymbol***, long*);
        void find_address_in_section(bfd*, asection*, PTR);
        BOOL BfdDemangleSymName(LPCTSTR, LPTSTR, DWORD);
        BOOL BfdGetSymFromAddr(bfd*, asymbol**, long, DWORD, LPTSTR, DWORD);
        BOOL BfdGetLineFromAddr(bfd*, asymbol**, long, DWORD,  LPTSTR, DWORD,
            LPDWORD);
        BOOL WINAPI j_SymInitialize(HANDLE, PSTR, BOOL);
        BOOL WINAPI j_SymCleanup(HANDLE);
        DWORD WINAPI j_SymSetOptions(DWORD);
        BOOL WINAPI j_SymUnDName(PIMAGEHLP_SYMBOL, PSTR, DWORD);
        PVOID WINAPI j_SymFunctionTableAccess(HANDLE, DWORD);
        DWORD WINAPI j_SymGetModuleBase(HANDLE, DWORD);
        BOOL WINAPI j_StackWalk(DWORD, HANDLE, HANDLE, LPSTACKFRAME, PVOID, 
            PREAD_PROCESS_MEMORY_ROUTINE,  PFUNCTION_TABLE_ACCESS_ROUTINE,
            PGET_MODULE_BASE_ROUTINE, PTRANSLATE_ADDRESS_ROUTINE);
        BOOL WINAPI j_SymGetSymFromAddr(HANDLE, DWORD, PDWORD, PIMAGEHLP_SYMBOL);
        BOOL WINAPI j_SymGetLineFromAddr(HANDLE, DWORD, PDWORD, PIMAGEHLP_LINE);
        BOOL ImagehlpDemangleSymName(LPCTSTR, LPTSTR, DWORD);
        BOOL ImagehlpGetSymFromAddr(HANDLE, DWORD, LPTSTR, DWORD);
        BOOL ImagehlpGetLineFromAddr(HANDLE, DWORD, LPTSTR, DWORD, LPDWORD);
        BOOL PEGetSymFromAddr(HANDLE, DWORD, LPTSTR, DWORD);
        BOOL WINAPI IntelStackWalk(DWORD, HANDLE, HANDLE, LPSTACKFRAME, 
            PCONTEXT, PREAD_PROCESS_MEMORY_ROUTINE,  
            PFUNCTION_TABLE_ACCESS_ROUTINE, PGET_MODULE_BASE_ROUTINE, 
            PTRANSLATE_ADDRESS_ROUTINE);
        BOOL StackBackTrace(HANDLE, HANDLE, PCONTEXT);
        void GenerateExceptionReport(PEXCEPTION_POINTERS);
        LONG WINAPI TopLevelExceptionFilter(PEXCEPTION_POINTERS);
    }
...
using namespace StackDump;


cStackDump::cStackDump(const char *logname)
{
    if (logname)
        _tcscpy(szLogFileName, logname);
    else {
        // If a dump file name wasn't given, invent one.
        if(GetModuleFileName(NULL, szLogFileName, MAX_PATH))
        {
            // Look for the '.' before the "EXE" extension.  Replace
            // the extension with "stackdump".

            LPTSTR lpszDot;
            if((lpszDot = _tcsrchr(szLogFileName, _T('.'))))
            {
                lpszDot++;  // Advance past the '.'
                _tcscpy(lpszDot, _T("stackdump"));
            }
            else
                _tcscat(szLogFileName, _T(".stackdump"));
        }
    ...
    // If were already running, just return, having reset the log file
    // name.
    if (prevExceptionFilter)
        return;

    prevExceptionFilter = 0;
    hReportFile = 0;
    ...
    pfnSymSetOptions = 0;
    pfnSymUnDName = 0;
    pfnSymFunctionTableAccess = 0;
    pfnSymGetModuleBase = 0;
    pfnStackWalk = 0;
    pfnSymGetSymFromAddr = 0;
    pfnSymGetLineFromAddr = 0;
...
int __cdecl
StackDump::rprintf(const TCHAR * format, ...)
{
    TCHAR szBuff[4096];
    int retValue;
    DWORD cbWritten;
    ...
    // Apparently there is no wvsnprintf.
    retValue = wvsprintf(szBuff, format, argptr);
    va_end(argptr);

    WriteFile(hReportFile, szBuff, retValue * sizeof(TCHAR),
        &cbWritten, 0);
...
// The GetModuleBase function retrieves the base address of the module
// that contains the specified address. 
//
DWORD
StackDump::GetModuleBase(DWORD dwAddress)
{
    MEMORY_BASIC_INFORMATION Buffer;
    
    return VirtualQuery((LPCVOID) dwAddress, &Buffer,
        sizeof(Buffer)) ? (DWORD) Buffer.AllocationBase : 0;
}
...
//
bfd_boolean
StackDump::slurp_symtab(bfd *abfd, asymbol ***syms, long *symcount)
{
    ...
    *symcount = bfd_read_minisymbols(abfd, FALSE, (void**)syms, &storage);
    if (*symcount == 0)
        *symcount = bfd_read_minisymbols(abfd, TRUE, (void**)syms, &storage);
    return TRUE;
...
//
void
StackDump::find_address_in_section (bfd *abfd, asection *section, PTR data)
{
    ...
    if (info->pc >= vma + bfd_section_size (abfd, section))
        return;

    info->found = bfd_find_nearest_line (abfd, section, info->syms,
        info->pc - vma, &info->filename, &info->functionname, &info->line);

/* SRW - binutils-2.19.1 returns NULL for functionname for class
 * member functions, but provides line numbers and filename.  */

    if (info->found) {
        if (!info->functionname && info->line)
            info->functionname = "???";
    }
}
...
BOOL
StackDump::BfdDemangleSymName(LPCTSTR lpName, LPTSTR lpDemangledName,
    DWORD nSize)
{
    assert(lpName != NULL);
...
BOOL
StackDump::BfdGetSymFromAddr(bfd *abfd, asymbol **syms, long symcount,
    DWORD dwAddress, LPTSTR lpSymName, DWORD nSize)
{
    HMODULE hModule;
...
BOOL
StackDump::BfdGetLineFromAddr(bfd *abfd, asymbol **syms, long symcount,
    DWORD dwAddress,  LPTSTR lpFileName, DWORD nSize, LPDWORD lpLineNumber)
{
    HMODULE hModule;
    if(!(hModule = (HMODULE) GetModuleBase(dwAddress)))
        return FALSE;
...
BOOL WINAPI
StackDump::j_SymInitialize(HANDLE hProcess, PSTR UserSearchPath,
    BOOL fInvadeProcess)
{
    if(
        (hModule_Imagehlp ||
        (hModule_Imagehlp = LoadLibrary(_T("IMAGEHLP.DLL")))) &&
        (pfnSymInitialize || (pfnSymInitialize =
        (PFNSYMINITIALIZE) GetProcAddress(hModule_Imagehlp, "SymInitialize")))
    )
...
BOOL WINAPI
StackDump::j_SymCleanup(HANDLE hProcess)
{
    if(
        (hModule_Imagehlp ||
        (hModule_Imagehlp = LoadLibrary(_T("IMAGEHLP.DLL")))) &&
        (pfnSymCleanup || (pfnSymCleanup =
        (PFNSYMCLEANUP) GetProcAddress(hModule_Imagehlp, "SymCleanup")))
    )
...
DWORD WINAPI
StackDump::j_SymSetOptions(DWORD SymOptions)
{
    if(
        (hModule_Imagehlp ||
        (hModule_Imagehlp = LoadLibrary(_T("IMAGEHLP.DLL")))) &&
        (pfnSymSetOptions || (pfnSymSetOptions =
        (PFNSYMSETOPTIONS) GetProcAddress(hModule_Imagehlp, "SymSetOptions")))
    )
...
BOOL WINAPI
StackDump::j_SymUnDName(PIMAGEHLP_SYMBOL Symbol, PSTR UnDecName,
    DWORD UnDecNameLength)
{
    if(
        ...
        (hModule_Imagehlp = LoadLibrary(_T("IMAGEHLP.DLL")))) &&
        (pfnSymUnDName || (pfnSymUnDName =
        (PFNSYMUNDNAME) GetProcAddress(hModule_Imagehlp, "SymUnDName")))
    )
...
PVOID WINAPI
StackDump::j_SymFunctionTableAccess(HANDLE hProcess, DWORD AddrBase)
{
    if(
        (hModule_Imagehlp ||
        (hModule_Imagehlp = LoadLibrary(_T("IMAGEHLP.DLL")))) &&
        (pfnSymFunctionTableAccess || (pfnSymFunctionTableAccess =
        (PFNSYMFUNCTIONTABLEACCESS) GetProcAddress(hModule_Imagehlp,
...
DWORD WINAPI
StackDump::j_SymGetModuleBase(HANDLE hProcess, DWORD dwAddr)
{
    if(
        (hModule_Imagehlp ||
        (hModule_Imagehlp = LoadLibrary(_T("IMAGEHLP.DLL")))) &&
        (pfnSymGetModuleBase || (pfnSymGetModuleBase =
        (PFNSYMGETMODULEBASE) GetProcAddress(hModule_Imagehlp,
        "SymGetModuleBase")))
    )
    ...
    PTRANSLATE_ADDRESS_ROUTINE TranslateAddress)
{
    if(
        ...
        (hModule_Imagehlp = LoadLibrary(_T("IMAGEHLP.DLL")))) &&
        (pfnStackWalk || (pfnStackWalk =
        (PFNSTACKWALK) GetProcAddress(hModule_Imagehlp, "StackWalk")))
    )
        return pfnStackWalk(
            MachineType, 
            hProcess, 
...
BOOL WINAPI
StackDump::j_SymGetSymFromAddr(HANDLE hProcess, DWORD Address,
    PDWORD Displacement, PIMAGEHLP_SYMBOL Symbol)
{
    if(
        ...
        (hModule_Imagehlp = LoadLibrary(_T("IMAGEHLP.DLL")))) &&
        (pfnSymGetSymFromAddr || (pfnSymGetSymFromAddr =
        (PFNSYMGETSYMFROMADDR) GetProcAddress(hModule_Imagehlp,
        "SymGetSymFromAddr")))
    )
...
BOOL WINAPI
StackDump::j_SymGetLineFromAddr(HANDLE hProcess, DWORD dwAddr,
    PDWORD pdwDisplacement, PIMAGEHLP_LINE Line)
{
    if(
        ...
        (hModule_Imagehlp = LoadLibrary(_T("IMAGEHLP.DLL")))) &&
        (pfnSymGetLineFromAddr || (pfnSymGetLineFromAddr =
        (PFNSYMGETLINEFROMADDR) GetProcAddress(hModule_Imagehlp,
        "SymGetLineFromAddr")))
    )
...
BOOL
StackDump::ImagehlpDemangleSymName(LPCTSTR lpName, LPTSTR lpDemangledName,
    DWORD nSize)
{
...
BOOL
StackDump::ImagehlpGetSymFromAddr(HANDLE hProcess, DWORD dwAddress,
    LPTSTR lpSymName, DWORD nSize)
{
...
BOOL
StackDump::ImagehlpGetLineFromAddr(HANDLE hProcess, DWORD dwAddress,
    LPTSTR lpFileName, DWORD nSize, LPDWORD lpLineNumber)
{
    IMAGEHLP_LINE Line;
    DWORD dwDisplacement = 0;  // Displacement of the input address,
                               // relative to the start of the symbol
...
BOOL
StackDump::PEGetSymFromAddr(HANDLE hProcess, DWORD dwAddress,
    LPTSTR lpSymName, DWORD nSize)
{
    HMODULE hModule;
    ...
    
    {
        ...
        if(!ReadProcessMemory(hProcess, &pDosHdr->e_lfanew, &e_lfanew,
                sizeof(e_lfanew), NULL))
            return FALSE;
        
        pNtHdr = (PIMAGE_NT_HEADERS)((DWORD)hModule + (DWORD)e_lfanew);
    
        if(!ReadProcessMemory(hProcess, pNtHdr, &NtHdr,
                sizeof(IMAGE_NT_HEADERS), NULL))
            return FALSE;
    }
    
    pSection = (PIMAGE_SECTION_HEADER) ((DWORD)pNtHdr + sizeof(DWORD) +
        sizeof(IMAGE_FILE_HEADER) + NtHdr.FileHeader.SizeOfOptionalHeader);
    ...
    for (i = 0; i < NtHdr.FileHeader.NumberOfSections; i++, pSection++)
    {
        ...
        BYTE ExportSectionName[IMAGE_SIZEOF_SHORT_NAME] =
            {'.', 'e', 'd', 'a', 't', 'a', '\0', '\0'};
        
        if(!ReadProcessMemory(hProcess, pSection, &Section,
                sizeof(IMAGE_SECTION_HEADER), NULL))
            return FALSE;
        
        if(memcmp(Section.Name, ExportSectionName,
                IMAGE_SIZEOF_SHORT_NAME) == 0)
            pExportDir = (PIMAGE_EXPORT_DIRECTORY) Section.VirtualAddress;
        else if ((NtHdr.OptionalHeader.DataDirectory[
        ...
        if(pExportDir)
        {
            ...
            if(!ReadProcessMemory(hProcess, (PVOID)((DWORD)hModule +
                    (DWORD)pExportDir), &ExportDir,
                    sizeof(IMAGE_EXPORT_DIRECTORY), NULL))
                return FALSE;
            
            {
                PDWORD *AddressOfFunctions = (PDWORD*)
                    alloca(ExportDir.NumberOfFunctions*sizeof(PDWORD));
    
                if(!ReadProcessMemory(hProcess,
                        (PVOID)((DWORD)hModule +
                        (DWORD)ExportDir.AddressOfFunctions),
                        AddressOfFunctions,
                        ExportDir.NumberOfFunctions*sizeof(PDWORD), NULL))
                    return FALSE;
                
                for(unsigned int j = 0; j < ExportDir.NumberOfNames; ++j)
                {
                    DWORD pFunction = (DWORD)hModule +
                        (DWORD)AddressOfFunctions[j];
                    //ReadProcessMemory(hProcess, (DWORD) hModule +
                    //    (DWORD) (&ExportDir.AddressOfFunctions[j]),
                    //    &pFunction, sizeof(pFunction), NULL);
                    ...
                    if(pFunction <= dwAddress && pFunction > dwNearestAddress)
                    {
                        ...
                        if(!ReadProcessMemory(hProcess,
                                (PVOID)((DWORD)hModule +
                            (DWORD)(&((PDWORD*)ExportDir.AddressOfNames)[j])),
    ...
    if(!ReadProcessMemory(hProcess, (PVOID)dwNearestName, lpSymName, nSize,
            NULL))
        return FALSE;
    lpSymName[nSize - 1] = 0;
    ...
    PTRANSLATE_ADDRESS_ROUTINE TranslateAddress)
{
    ...
    // Second argument is DWORD on left, void* on right.
    ...
    if(!StackFrame->Reserved[0])
    {
        ...
        if(!ReadMemoryRoutine(hProcess,
                (StackFrame->AddrFrame.Offset + sizeof(DWORD)),
    ...
    else
    {
        ...
        if(!ReadMemoryRoutine(hProcess,
                ...
                &StackFrame->AddrFrame.Offset, sizeof(DWORD), NULL))
            return FALSE;
        if(!ReadMemoryRoutine(hProcess,
                (StackFrame->AddrFrame.Offset + sizeof(DWORD)),
                &StackFrame->AddrReturn.Offset, sizeof(DWORD), NULL))
    ...
    ReadMemoryRoutine(hProcess,
        (StackFrame->AddrFrame.Offset + 2*sizeof(DWORD)),
...
BOOL
StackDump::StackBackTrace(HANDLE hProcess, HANDLE hThread, PCONTEXT pContext)
{
    ...
    while ( 1 )
    {
        ...
        {
#define DLL_FIELD " %-14s"
            rprintf( _T(DLL_FIELD), strip_path(szModule));
            ...
            if(hModule != hPrevModule)
            {
                ...
                if((abfd = bfd_openr (szModule, NULL)))
                    if(bfd_check_format(abfd, bfd_object))
                    {
                        ...
                        /* If we are adjusting section VMA's, change them
                         * all now.  Changing the BFD information is a hack.
                         * However, we must do it, or bfd_find_nearest_line
                         * will not do the right thing.  */
/*
  The next line is the only thing in the file that requires internal.h
  and libcoff.h.
*/
                        if ((adjust_section_vma = (long) hModule -
...
#define SYM_FIELD " %-24s"
            if(!bSuccess && abfd && syms && symcount)
                if((bSuccess = BfdGetSymFromAddr(abfd, syms, symcount,
                        StackFrame.AddrPC.Offset, szSymName, 512)))
                {
                    /*
                    ...
                    */
                    ...
                    rprintf( _T(SYM_FIELD), szSymName);
                    
                    if(BfdGetLineFromAddr(abfd, syms, symcount,
                            StackFrame.AddrPC.Offset, szFileName, MAX_PATH,
                            &LineNumber))
                        rprintf( _T(" %s:%ld"), strip_path(szFileName),
                            LineNumber);
                }
            
            if(!bSuccess && bSymInitialized)
                if((bSuccess = ImagehlpGetSymFromAddr(hProcess,
                        StackFrame.AddrPC.Offset, szSymName, 512)))
                {
                    rprintf( _T(SYM_FIELD), szSymName);
                    ...
                    if(ImagehlpGetLineFromAddr(hProcess,
                            StackFrame.AddrPC.Offset, szFileName, MAX_PATH,
                            &LineNumber))
                        rprintf( _T(" %s:%ld"), strip_path(szFileName),
                            LineNumber);
            ...
            if(!bSuccess)
                if((bSuccess = PEGetSymFromAddr(hProcess,
                        StackFrame.AddrPC.Offset, szSymName, 512)))
                    rprintf( _T(SYM_FIELD), szSymName);
        }
...
void
StackDump::GenerateExceptionReport(PEXCEPTION_POINTERS pExceptionInfo)
{
...
/* Skip this, application will append date/time.
...
*/
    ...
    rprintf(_T("%s caused "),  GetModuleFileName(NULL, szModule,
        MAX_PATH) ? strip_path(szModule) : "Application");
    switch(pExceptionRecord->ExceptionCode)
    {
        ...
        default:
            ...
            */
            ...
            rprintf(_T("an Unknown [0x%lX] Exception"),
                pExceptionRecord->ExceptionCode);
            break;
    }

    // Now print information about where the fault occured
    rprintf(_T(" at location %08x"),
        (DWORD) pExceptionRecord->ExceptionAddress);
    if((hModule = (HMODULE) GetModuleBase(
            (DWORD) pExceptionRecord->ExceptionAddress)) &&
            GetModuleFileName(hModule, szModule, sizeof(szModule)))
        rprintf(_T(" in module %s"), strip_path(szModule));
...
//
LONG WINAPI
StackDump::TopLevelExceptionFilter(PEXCEPTION_POINTERS pExceptionInfo)
{
    ...
    case EXCEPTION_PRIV_INSTRUCTION:
        if(!bBeenHere)
        {
            ...
            if(fuOldErrorMode & SEM_NOGPFAULTERRORBOX)
            {
                ...
                wsprintf(szBuffer,
            _T("An unhandled exception ocurred\r\nSee %s for more details\r\n"),
                    szLogFileName);
