 ...
 *                                                                        *
 *========================================================================*
 $Id: inpptree.cc,v 2.120 2017/02/22 01:53:11 stevew Exp $
 *========================================================================*/
...
namespace {
    ...
    //
    const char *is_tran_func(const char **pstr)
    {
        if (!pstr)
            return (0);
        if (!tran_tab) {
...
{
    const char *bptr = *s;
    const char *name = is_tran_func(s);
    if (!name)
        return (0);
    ...
    case PTF_tINTERP:
        if (varnum < 0 && !pt_num_xvars) {
            ...
            if (p->p_valindx == PTF_tPULSE)
                ...
                np->p_func = &IFparseNode::PTtGpulseD;
            else if (p->p_valindx == PTF_tSIN)
                np->p_func = &IFparseNode::PTtSinD;
            else if (p->p_valindx == PTF_tSPULSE)
                np->p_func = &IFparseNode::PTtSpulseD;
            else if (p->p_valindx == PTF_tEXP)
                np->p_func = &IFparseNode::PTtExpD;
            else if (p->p_valindx == PTF_tSFFM)
                np->p_func = &IFparseNode::PTtSffmD;
            else if (p->p_valindx == PTF_tAM)
                np->p_func = &IFparseNode::PTtAmD;
            else if (p->p_valindx == PTF_tGAUSS)
                np->p_func = &IFparseNode::PTtGaussD;
            else if (p->p_valindx == PTF_tINTERP)
                np->p_func = &IFparseNode::PTtInterpD;
            return (np);
        }
        break;
...
{
    *error = OK;
    // The tran function string includes parameters enclosed in ().
    if (!string)
        return (false);
    ...
    {
        if ((astart - string) > 64)
            return (false);  // too ridiculous

        char *s = buf;
        for (const char *ss = string; ss < astart && !isspace(*ss); )
            *s++ = *ss++;
        *s = '\0';
    }
    astart++;
    ...
    {
        int i;
        for (i = 0; PTtFuncs[i].name; i++) {
            if (lstring::cieq(PTtFuncs[i].name, buf))
                break;
        }
        if (!PTtFuncs[i].name)
            return (false);
        ...
        p->p_type = PT_TFUNC;
        p->p_valname = PTtFuncs[i].name;
        p->p_valindx = PTtFuncs[i].number;
        p->p_func = PTtFuncs[i].funcptr;
    ...
    if (p->p_valindx == PTF_tINTERP) {
        ...
        sDataVec *d = Sp.VecGet(buf, ckt);
        if (!d) {
            *error = E_NOVEC;
            return (true);
        }
        sDataVec *scale = d->scale();
        if (!scale)
            scale = Sp.CurPlot()->scale();
        if (!scale || d->iscomplex() || scale->iscomplex()) {
            Errs()->add_error("bad scale for interp function");
            *error = E_NOVEC;
            return (true);
        }
        int len = d->length();
        ...
        p->v.td = new IFtranData(PTF_tINTERP, scale->realvec(), len,
            d->realvec());
        return (true);
    }
    ...
    for (;;) {
        if (!ptmp) {
            if (p->p_valindx == PTF_tPWL && !list_done) {
            ...
            if (!is_const(ptmp)) {
                if (lookingR) {
                    ...
                    lookingTD = false;
                    Errs()->add_error("TRAN function TD val not constant.");
                }
                else if (list_done)
                    break;
                else {
                    da.add(0.0);
                    Errs()->add_error("TRAN function arg %d not constant",
                        da.count());
                }
                *error = E_BADPARM;
                // Don't return on error here or below, calling the init
                // functions below avoids seg faults.
            }
            else {
                double r;
                ...
                if (err) {
                    if (lookingR) {
                        lookingR = false;
                        Errs()->add_error("TRAN function R eval failed: %s",
                            Sp.ErrorShort(err));
                    }
                    else if (lookingTD) {
                        lookingTD = false;
                        Errs()->add_error("TRAN function TD eval failed: %s",
                            Sp.ErrorShort(err));
                    }
                    else if (list_done)
                        break;
                        ...
                        da.add(0.0);
                        Errs()->add_error(
                            "TRAN function arg %d eval failed: %s",
                            da.count(), Sp.ErrorShort(err));
                    }
                    *error = E_BADPARM;
                }
                else {
...
again:
    ...
    case PTF_tAM:
    case PTF_tGAUSS:
        p->v.td = new IFtranData((PTftType)p->p_valindx, list, num);
        break;
    case PTF_tPWL:
        p->v.td = new IFtranData((PTftType)p->p_valindx, list, num,
            0, hasR, Rix, TDval);
