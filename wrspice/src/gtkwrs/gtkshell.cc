...
//
void
GTKtoolbar::PopUpShellDefs(ShowMode mode, int x, int y)
{
    if (mode == MODE_OFF) {
        if (!sh_shell)
            return;
    ...
    if (entry) {
        ...
        entry->xent()->setup(CP_DefHistLen, 1.0, 0, 0, 0);
    ...
    if (entry) {
        ...
        entry->ent = new xEnt(kw_int_func);
        entry->xent()->setup(DEF_WIDTH, 1.0, 0.0, 0.0, 0);
        entry->xent()->create_widgets(entry, tbuf);
    ...
    if (entry) {
        ...
        entry->ent = new xEnt(kw_int_func);
        entry->xent()->setup(DEF_HEIGHT, 1.0, 0.0, 0.0, 0);
        entry->xent()->create_widgets(entry, tbuf);
    ...
    if (entry) {
        ...
        gtk_table_attach(GTK_TABLE(form), entry->xent()->frame, 0, 4,
            entrycount, entrycount + 1,
            (GtkAttachOptions)(GTK_EXPAND | GTK_FILL | GTK_SHRINK),
            (GtkAttachOptions)0, 2, 2);
    }
...
namespace {
    ...
    sh_help_proc(GtkWidget *caller, void *client_data)
    {
        ...
        if (state)
            TB()->PopUpTBhelp(MODE_ON, parent, caller, TBH_SH);
        else
            TB()->PopUpTBhelp(MODE_OFF, 0, 0,TBH_SH);
    }
