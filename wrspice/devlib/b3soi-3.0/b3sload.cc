 ...
 *                                                                        *
 *========================================================================*
 $Id: b3sload.cc,v 2.14 2015/06/11 01:12:28 stevew Exp $
 *========================================================================*/
...
int
// SRW B3SOIdev::load(sGENmodel *genmod, sCKT *ckt)
...
// SRW - end
            ...
            ckt->rhsadd(here->B3SOIgNode, -(ceqgate + ceqqg + Igtoteq)); /* v3.0 */
            ckt->rhsadd(here->B3SOIdNodePrime, (ceqbd - cdreq - ceqqd) + Idtoteq); /* v3.0 */
            ckt->rhsadd(here->B3SOIsNodePrime, (cdreq + ceqbs + ceqqg
...
// SRW
            if (here->B3SOIadjoint)
            {
                ...
                (*mat->get_elem(here->B3SOIgNode) -= (ceqgate + ceqqg) + Igtoteq); /* v3.0 */
                (*mat->get_elem(here->B3SOIdNodePrime) += (ceqbd - cdreq - ceqqd) + Idtoteq); /* v3.0 */
                (*mat->get_elem(here->B3SOIsNodePrime) += (cdreq + ceqbs + ceqqg
                        + ceqqb + ceqqd + ceqqe) + Istoteq); /* v3.0 */
...
// SRW - end
            ...
            ckt->ldadd(here->B3SOIEbPtr, -(gcegb + gcedb + gcesb + gceeb));
            ckt->ldadd(here->B3SOIGbPtr, -(-gigb + gcggb + gcgdb + gcgsb + gcgeb - gIgtotb)); /* v3.0 */
            ckt->ldadd(here->B3SOIDPbPtr, -((-gddpb - Gmbs + gcdgb + gcddb + gcdeb + gcdsb) + gIdtotb)); /* v3.0 */

            ckt->ldadd(here->B3SOISPbPtr, -((-gsspb + Gmbs + gcsgb + gcsdb + gcseb + gcssb)
                                     + Gmin + gIstotb)); /* v3.0 */
            ...
            ckt->ldadd(here->B3SOIBbPtr, -gigb + gbbb - gcbgb - gcbdb - gcbsb - gcbeb + Gmin) ;
            ckt->ldadd(here->B3SOIEePtr, gceeb);

            ckt->ldadd(here->B3SOIGgPtr, gigg + gcggb + ckt->CKTgmin + gIgtotg); /* v3.0 */
            ckt->ldadd(here->B3SOIGdpPtr, gigd + gcgdb - ckt->CKTgmin + gIgtotd); /* v3.0 */
            ckt->ldadd(here->B3SOIGspPtr, gcgsb + gigs + gIgtots); /* v3.0 */

            ckt->ldadd(here->B3SOIDPgPtr, (Gm + gcdgb) + gddpg - ckt->CKTgmin - gIdtotg); /* v3.0 */
            ckt->ldadd(here->B3SOIDPdpPtr, (here->B3SOIdrainConductance
                                       + here->B3SOIgds + gddpdp
                                       + RevSum + gcddb) + ckt->CKTgmin - gIdtotd); /* v3.0 */
            ckt->ldadd(here->B3SOIDPspPtr, -((-gddpsp + here->B3SOIgds + FwdSum - gcdsb) + gIdtots)); /* v3.0 */

            ckt->ldadd(here->B3SOIDPdPtr, -here->B3SOIdrainConductance);

            ckt->ldadd(here->B3SOISPgPtr, gcsgb - Gm + gsspg - gIstotg); /* v3.0 */
            ckt->ldadd(here->B3SOISPdpPtr, -(here->B3SOIgds - gsspdp + RevSum - gcsdb + gIstotd)); /* v3.0 */
...
// SRW
            if (here->B3SOIadjoint)
            {
                ...
                *(adj->B3SOIGePtr) += gcgeb;
                ...
                (*(adj->B3SOIEbPtr) -= gcegb + gcedb + gcesb + gceeb);
                (*(adj->B3SOIGbPtr) -= -gigb + gcggb + gcgdb + gcgsb + gcgeb - gIgtotb); /* v3.0 */
                (*(adj->B3SOIDPbPtr) -= (-gddpb - Gmbs + gcdgb + gcddb + gcdeb + gcdsb) + gIdtotb); /* v3.0 */

                (*(adj->B3SOISPbPtr) -= (-gsspb + Gmbs + gcsgb + gcsdb + gcseb + gcssb)
                                        + Gmin + gIstotb); /* v3.0 */
                ...
                (*(adj->B3SOIBspPtr) += gcbsb + gbbsp - Gmin
                                        - gigs);
                (*(adj->B3SOIBbPtr) += -gigb + gbbb - gcbgb - gcbdb - gcbsb - gcbeb + Gmin) ;
                (*(adj->B3SOIEePtr) += gceeb);

                (*(adj->B3SOIGgPtr) += gigg + gcggb + ckt->CKTgmin + gIgtotg); /* v3.0 */
                (*(adj->B3SOIGdpPtr) += gigd + gcgdb - ckt->CKTgmin + gIgtotd); /* v3.0 */
                (*(adj->B3SOIGspPtr) += gcgsb + gigs + gIgtots); /* v3.0 */

                (*(adj->B3SOIDPgPtr) += (Gm + gcdgb) + gddpg - ckt->CKTgmin - gIdtotg); /* v3.0 */
                (*(adj->B3SOIDPdpPtr) += (here->B3SOIdrainConductance
                                          + here->B3SOIgds + gddpdp
                                          + RevSum + gcddb) + ckt->CKTgmin - gIdtotd); /* v3.0 */
                (*(adj->B3SOIDPspPtr) -= (-gddpsp + here->B3SOIgds + FwdSum - gcdsb) + gIdtots); /* v3.0 */

                (*(adj->B3SOIDPdPtr) -= here->B3SOIdrainConductance);

                (*(adj->B3SOISPgPtr) += gcsgb - Gm + gsspg - gIstotg); /* v3.0 */
                (*(adj->B3SOISPdpPtr) -= (here->B3SOIgds - gsspdp + RevSum - gcsdb + gIstotd)); /* v3.0 */
