 ...
 *                                                                        *
 *========================================================================*
 $Id: b4check.cc,v 1.5 2011/12/18 01:15:30 stevew Exp $
 *========================================================================*/
...
// SRW -- modified version checking
                ...
                ((here->BSIM4nf == 1.0) || ((here->BSIM4nf > 1.0) && (here->BSIM4sd > 0.0))) )
        {
            ...
            {
                ChkFprintf(fplog, "Warning: WLOD = %g is less than 0. Set to 0.0\n",model->BSIM4wlod);
                WarnPrintf("Warning: WLOD = %g is less than 0.\n",model->BSIM4wlod);
                model->BSIM4wlod = 0.0;
            }
            if (model->BSIM4kvsat < -1.0 )
            {
                ChkFprintf(fplog, "Warning: KVSAT = %g is too small; Reset to -1.0.\n",model->BSIM4kvsat);
                WarnPrintf("Warning: KVSAT = %g is is too small; Reset to -1.0.\n",model->BSIM4kvsat);
                model->BSIM4kvsat = -1.0;
            }
            if (model->BSIM4kvsat > 1.0)
            {
                ChkFprintf(fplog, "Warning: KVSAT = %g is too big; Reset to 1.0.\n",model->BSIM4kvsat);
                WarnPrintf("Warning: KVSAT = %g is too big; Reset to 1.0.\n",model->BSIM4kvsat);
                model->BSIM4kvsat = 1.0;
            }
