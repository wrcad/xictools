...
/***********************************************************************
 HiSIM v1.1.0
 File: hsm1eval1_0.c of HiSIM v1.1.0
...
#else
int HSM1evaluate1_0( sIN , pOT , pMS ) 
     HiSIM_input     sIN ;
     HiSIM_output    *pOT ;
     HiSIM_messenger *pMS ;
#endif
...
/*-----*/
double  qme_dlt = 1.0e-2 ;
double  eef_dlt = 1.0e-2 ;
double	sti_dlt = -3.0e-3 ;
double  pol_dlt = 1.0e-2 ;
...
/* SRW -- initialize all variables flagged as potentially used before
 * initialized by gcc.
 */
flg_noqi = 0;
Vgp = 0;
...
*-----------------*/
    ...
    if ( yn <= Lov ){
      Qgod = ( ( Vgs - Vds ) * Lov - T2 * yn3 / 3.0e0 ) * T1 ;
        Qgod_dVbs = ( (-1.0e0) * T2 * yn2 * yn_dVbs ) * T1 ;
        Qgod_dVds = ( (-1.0e0) * Lov - T2 * yn2 * yn_dVds ) * T1 ;
        Qgod_dVgs = ( Lov - T2 * yn2 * yn_dVgs ) * T1 ;
    }else{
      T4 = (Lov - yn ) * (Lov - yn ) ;
      T5 = T4 * (Lov - yn ) ;
...
*-----------------*/
    ...
    if ( sIN.coadov == 1 ) {
        ...
        pOT->cggb   += Cggo ;
	pOT->cgdb   = pOT->cgdb + Cgdo + (-1) * Cgdfring ;
        pOT->cgsb   += Cgso ;
...
*-----------------*/ 
    ...
    pOT->capgs  = - pOT->cgsb - Cgso + Cf ;
    pOT->capgd  = - pOT->cgdb - Cgdo + Cf ;
