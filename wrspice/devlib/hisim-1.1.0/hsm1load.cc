 ...
 *                                                                        *
 *========================================================================*
 $Id: hsm1load.cc,v 2.14 2015/06/11 01:12:29 stevew Exp $
 *========================================================================*/
...
extern "C" {
extern int HSM1evaluate1_0(HiSIM_input, HiSIM_output*, HiSIM_messenger*);
extern int HSM1evaluate1_1(HiSIM_input, HiSIM_output*, HiSIM_messenger*);
}
...
*/
      ...
      *(ckt->CKTstate0 + here->HSM1qbs) = sOT.qbs ;
      ...
      if ( (here->HSM1_off == 0) || !(ckt->CKTmode & MODEINITFIX) ) {
        if (Check == 1) {
          ...
          if (FABS(cdhat - Idtot) >= tol) ckt->incNoncon();  // SRW
          else {
            Ibtot = here->HSM1_ibs + here->HSM1_ibd - here->HSM1_isub;
            tol = ckt->CKTreltol * 
              MAX(FABS(cbhat), FABS(Ibtot)) + ckt->CKTabstol;
            if (FABS(cbhat - Ibtot) > tol) ckt->incNoncon();  // SRW
          }
        }
#endif /* NEWCONV */
      }
 ...
 line900:
    ...
    */
    ...
    ckt->rhsadd(here->HSM1gNode, -ceqqg);
    ckt->rhsadd(here->HSM1bNode, -(ceqbs + ceqbd + ceqqb));
    ckt->rhsadd(here->HSM1dNodePrime, ceqbd - cdreq - ceqqd);
    ckt->rhsadd(here->HSM1sNodePrime, cdreq 
    ...
    ckt->ldadd(here->HSM1GgPtr, gcggb);
    ...
    ckt->ldadd(here->HSM1GbPtr, -(gcggb + gcgdb + gcgsb));
    ckt->ldadd(here->HSM1GdpPtr, gcgdb);
    ckt->ldadd(here->HSM1GspPtr, gcgsb);
    ckt->ldadd(here->HSM1SspPtr, -here->HSM1sourceConductance);
    ckt->ldadd(here->HSM1BgPtr, gcbgb - here->HSM1_gbgs);
    ckt->ldadd(here->HSM1BdpPtr, gcbdb - here->HSM1_gbd + gbbdp);
    ckt->ldadd(here->HSM1BspPtr, gcbsb - here->HSM1_gbs + gbbsp);
    ckt->ldadd(here->HSM1DPdPtr, -here->HSM1drainConductance);
    ckt->ldadd(here->HSM1DPgPtr, gm + gcdgb + gbdpg);
    ckt->ldadd(here->HSM1DPbPtr, -(here->HSM1_gbd - gmbs + gcdgb + gcddb + gcdsb - gbdpb));
    ckt->ldadd(here->HSM1DPspPtr, -(here->HSM1_gds + FwdSum - gcdsb - gbdpsp));
    ckt->ldadd(here->HSM1SPgPtr, gcsgb - gm + gbspg);
    ckt->ldadd(here->HSM1SPsPtr, -here->HSM1sourceConductance);
    ckt->ldadd(here->HSM1SPbPtr, -(here->HSM1_gbs + gmbs + gcsgb + gcsdb + gcssb - gbspb));
    ckt->ldadd(here->HSM1SPdpPtr, -(here->HSM1_gds + RevSum - gcsdb - gbspdp));
