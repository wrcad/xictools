 ...
 *                                                                        *
 *========================================================================*
 $Id: vbicload.cc,v 1.5 2015/07/30 18:16:29 stevew Exp $
 *========================================================================*/
...
static int vbic_4T_it_cf_fj(
    double *p, double *Vbei, double *Vbex, double *Vbci, double *Vbep, double *Vbcp,
    double *Vrcx, double *Vbcx, double *Vrci, double *Vrbx, double *Vrbi, double *Vre,
    double *Vrbp, double *Vrs, double *Vbe, double *Vbc, double *Ibe, double *Ibe_Vbei,
    double *Ibex, double *Ibex_Vbex, double *Itzf, double *Itzf_Vbei, double *Itzf_Vbci,
    double *Itzr, double *Itzr_Vbci, double *Itzr_Vbei, double *Ibc, double *Ibc_Vbci,
    double *Ibc_Vbei, double *Ibep, double *Ibep_Vbep, double *Ircx, double *Ircx_Vrcx,
    double *Irci, double *Irci_Vrci, double *Irci_Vbci, double *Irci_Vbcx, double *Irbx,
    double *Irbx_Vrbx, double *Irbi, double *Irbi_Vrbi, double *Irbi_Vbei,
    double *Irbi_Vbci, double *Ire, double *Ire_Vre, double *Irbp, double *Irbp_Vrbp,
    double *Irbp_Vbep, double *Irbp_Vbci, double *Qbe, double *Qbe_Vbei,
    double *Qbe_Vbci, double *Qbex, double *Qbex_Vbex, double *Qbc, double *Qbc_Vbci,
    double *Qbcx, double *Qbcx_Vbcx, double *Qbep, double *Qbep_Vbep, double *Qbep_Vbci,
    double *Qbeo, double *Qbeo_Vbe, double *Qbco, double *Qbco_Vbc, double *Ibcp,
    double *Ibcp_Vbcp, double *Iccp, double *Iccp_Vbep, double *Iccp_Vbci,
    double *Iccp_Vbcp, double *Irs, double *Irs_Vrs, double *Qbcp, double *Qbcp_Vbcp,
    double *SCALE);
//
...
//
int
// SRW VBICdev::load(sGENmodel *genmod, sCKT *ckt)
VBICdev::load(sGENinstance *in_inst, sCKT *ckt)
{
// SRW    sVBICmodel *model = static_cast<sVBICmodel*>(genmod);
// SRW    sVBICinstance *here;
    sVBICinstance *here = (sVBICinstance*)in_inst;
    sVBICmodel *model = (sVBICmodel*)here->GENmodPtr;
...
//            if (here->VBICowner != ARCHme) continue;
            ...
            if(ckt->CKTmode & MODEINITSMSIG) {
                 ...
                 */
                ichk1 = 1;
                Vbei = DEV.pnjlim(Vbei,*(ckt->CKTstate0 + here->VBICvbei),vt,
                        here->VBICtVcrit,&icheck);
                Vbex = DEV.pnjlim(Vbex,*(ckt->CKTstate0 + here->VBICvbex),vt,
                        here->VBICtVcrit,&ichk1);
                Vbci = DEV.pnjlim(Vbci,*(ckt->CKTstate0 + here->VBICvbci),vt,
                        here->VBICtVcrit,&ichk2);
                Vbcx = DEV.pnjlim(Vbcx,*(ckt->CKTstate0 + here->VBICvbcx),vt,
                        here->VBICtVcrit,&ichk3);
                Vbep = DEV.pnjlim(Vbep,*(ckt->CKTstate0 + here->VBICvbep),vt,
                        here->VBICtVcrit,&ichk4);
                Vbcp = DEV.pnjlim(Vbcp,*(ckt->CKTstate0 + here->VBICvbcp),vt,
...
load:
            ...
            rhs_current = model->VBICtype * (*(ckt->CKTstate0 + here->VBICcqbeo) - 
                    Vbe * gqbeo);
            ckt->rhsadd(here->VBICbaseNode, -rhs_current);
            ckt->rhsadd(here->VBICemitNode, rhs_current);
            ckt->ldadd(here->VBICbaseBasePtr,  gqbeo);
            ckt->ldadd(here->VBICemitEmitPtr,  gqbeo);
            ckt->ldadd(here->VBICbaseEmitPtr, -gqbeo);
            ckt->ldadd(here->VBICemitBasePtr, -gqbeo);

            rhs_current = model->VBICtype * (*(ckt->CKTstate0 + here->VBICcqbco) - 
                    Vbc * gqbco);
            ckt->rhsadd(here->VBICbaseNode, -rhs_current);
            ckt->rhsadd(here->VBICcollNode, rhs_current);
            ckt->ldadd(here->VBICbaseBasePtr,  gqbco);
            ckt->ldadd(here->VBICcollCollPtr,  gqbco);
            ckt->ldadd(here->VBICbaseCollPtr, -gqbco);
            ckt->ldadd(here->VBICcollBasePtr, -gqbco);

            ckt->rhsadd(here->VBICbaseBINode, -cbcx);
            ckt->rhsadd(here->VBICcollCXNode, cbcx);
            ckt->ldadd(here->VBICbaseBIBaseBIPtr,  gbcx);
            ckt->ldadd(here->VBICcollCXCollCXPtr,  gbcx);
            ckt->ldadd(here->VBICbaseBICollCXPtr, -gbcx);
            ckt->ldadd(here->VBICcollCXBaseBIPtr, -gbcx);
...
*/
            ...
            ckt->rhsadd(here->VBICbaseBINode, -rhs_current);
            ckt->ldadd(here->VBICbaseBIBaseBIPtr,  Ibe_Vbei);
            ckt->ldadd(here->VBICbaseBIEmitEIPtr, -Ibe_Vbei);
            ckt->rhsadd(here->VBICemitEINode, rhs_current);
            ckt->ldadd(here->VBICemitEIBaseBIPtr, -Ibe_Vbei);
            ckt->ldadd(here->VBICemitEIEmitEIPtr,  Ibe_Vbei);
/*
c           Stamp element: Ibex
*/
            rhs_current = model->VBICtype * (Ibex - Ibex_Vbex*Vbex);
            ckt->rhsadd(here->VBICbaseBXNode, -rhs_current);
            ckt->ldadd(here->VBICbaseBXBaseBXPtr,  Ibex_Vbex);
            ckt->ldadd(here->VBICbaseBXEmitEIPtr, -Ibex_Vbex);
            ckt->rhsadd(here->VBICemitEINode, rhs_current);
            ckt->ldadd(here->VBICemitEIBaseBXPtr, -Ibex_Vbex);
            ckt->ldadd(here->VBICemitEIEmitEIPtr,  Ibex_Vbex);

/*
c           Stamp element: Itzf
*/
            rhs_current = model->VBICtype * (Itzf - Itzf_Vbei*Vbei - Itzf_Vbci*Vbci);
            ckt->rhsadd(here->VBICcollCINode, -rhs_current);
            ckt->ldadd(here->VBICcollCIBaseBIPtr,  Itzf_Vbei);
            ckt->ldadd(here->VBICcollCIEmitEIPtr, -Itzf_Vbei);
            ckt->ldadd(here->VBICcollCIBaseBIPtr,  Itzf_Vbci);
            ckt->ldadd(here->VBICcollCICollCIPtr, -Itzf_Vbci);
            ckt->rhsadd(here->VBICemitEINode, rhs_current);
            ckt->ldadd(here->VBICemitEIBaseBIPtr, -Itzf_Vbei);
            ckt->ldadd(here->VBICemitEIEmitEIPtr,  Itzf_Vbei);
            ckt->ldadd(here->VBICemitEIBaseBIPtr, -Itzf_Vbci);
            ckt->ldadd(here->VBICemitEICollCIPtr,  Itzf_Vbci);
/*
c           Stamp element: Itzr
*/
            rhs_current = model->VBICtype * (Itzr - Itzr_Vbci*Vbci - Itzr_Vbei*Vbei);
            ckt->rhsadd(here->VBICemitEINode, -rhs_current);
            ckt->ldadd(here->VBICemitEIBaseBIPtr,  Itzr_Vbci);
            ckt->ldadd(here->VBICemitEICollCIPtr, -Itzr_Vbci);
            ckt->ldadd(here->VBICemitEIBaseBIPtr,  Itzr_Vbei);
            ckt->ldadd(here->VBICemitEIEmitEIPtr, -Itzr_Vbei);
            ckt->rhsadd(here->VBICcollCINode, rhs_current);
            ckt->ldadd(here->VBICcollCIBaseBIPtr, -Itzr_Vbci);
            ckt->ldadd(here->VBICcollCICollCIPtr,  Itzr_Vbci);
            ckt->ldadd(here->VBICcollCIBaseBIPtr, -Itzr_Vbei);
            ckt->ldadd(here->VBICcollCIEmitEIPtr,  Itzr_Vbei);
/*
c           Stamp element: Ibc
*/
            rhs_current = model->VBICtype * (Ibc - Ibc_Vbci*Vbci - Ibc_Vbei*Vbei);
            ckt->rhsadd(here->VBICbaseBINode, -rhs_current);
            ckt->ldadd(here->VBICbaseBIBaseBIPtr,  Ibc_Vbci);
            ckt->ldadd(here->VBICbaseBICollCIPtr, -Ibc_Vbci);
            ckt->ldadd(here->VBICbaseBIBaseBIPtr,  Ibc_Vbei);
            ckt->ldadd(here->VBICbaseBIEmitEIPtr, -Ibc_Vbei);
            ckt->rhsadd(here->VBICcollCINode, rhs_current);
            ckt->ldadd(here->VBICcollCIBaseBIPtr, -Ibc_Vbci);
            ckt->ldadd(here->VBICcollCICollCIPtr,  Ibc_Vbci);
            ckt->ldadd(here->VBICcollCIBaseBIPtr, -Ibc_Vbei);
            ckt->ldadd(here->VBICcollCIEmitEIPtr,  Ibc_Vbei);
/*
c           Stamp element: Ibep
*/
            rhs_current = model->VBICtype * (Ibep - Ibep_Vbep*Vbep);
            ckt->rhsadd(here->VBICbaseBXNode, -rhs_current);
            ckt->ldadd(here->VBICbaseBXBaseBXPtr,  Ibep_Vbep);
            ckt->ldadd(here->VBICbaseBXBaseBPPtr, -Ibep_Vbep);
            ckt->rhsadd(here->VBICbaseBPNode, rhs_current);
            ckt->ldadd(here->VBICbaseBPBaseBXPtr, -Ibep_Vbep);
            ckt->ldadd(here->VBICbaseBPBaseBPPtr,  Ibep_Vbep);
/*
c           Stamp element: Ircx
*/
            ckt->ldadd(here->VBICcollCollPtr,  Ircx_Vrcx);
            ckt->ldadd(here->VBICcollCXCollCXPtr,  Ircx_Vrcx);
            ckt->ldadd(here->VBICcollCXCollPtr,  -Ircx_Vrcx);
            ckt->ldadd(here->VBICcollCollCXPtr,  -Ircx_Vrcx);
/*
c           Stamp element: Irci
*/
            rhs_current = model->VBICtype * (Irci - Irci_Vrci*Vrci - Irci_Vbci*Vbci - Irci_Vbcx*Vbcx);
            ckt->rhsadd(here->VBICcollCXNode, -rhs_current);
            ckt->ldadd(here->VBICcollCXCollCXPtr,  Irci_Vrci);
            ckt->ldadd(here->VBICcollCXCollCIPtr, -Irci_Vrci);
            ckt->ldadd(here->VBICcollCXBaseBIPtr,  Irci_Vbci);
            ckt->ldadd(here->VBICcollCXCollCIPtr, -Irci_Vbci);
            ckt->ldadd(here->VBICcollCXBaseBIPtr,  Irci_Vbcx);
            ckt->ldadd(here->VBICcollCXCollCXPtr, -Irci_Vbcx);
            ckt->rhsadd(here->VBICcollCINode, rhs_current);
            ckt->ldadd(here->VBICcollCICollCXPtr, -Irci_Vrci);
            ckt->ldadd(here->VBICcollCICollCIPtr,  Irci_Vrci);
            ckt->ldadd(here->VBICcollCIBaseBIPtr, -Irci_Vbci);
            ckt->ldadd(here->VBICcollCICollCIPtr,  Irci_Vbci);
            ckt->ldadd(here->VBICcollCIBaseBIPtr, -Irci_Vbcx);
            ckt->ldadd(here->VBICcollCICollCXPtr,  Irci_Vbcx);
/*
c           Stamp element: Irbx
*/
            ckt->ldadd(here->VBICbaseBasePtr,  Irbx_Vrbx);
            ckt->ldadd(here->VBICbaseBXBaseBXPtr,  Irbx_Vrbx);
            ckt->ldadd(here->VBICbaseBXBasePtr, -Irbx_Vrbx);
            ckt->ldadd(here->VBICbaseBaseBXPtr, -Irbx_Vrbx);
/*
c           Stamp element: Irbi
*/
            rhs_current = model->VBICtype * (Irbi - Irbi_Vrbi*Vrbi - Irbi_Vbei*Vbei - Irbi_Vbci*Vbci);
            ckt->rhsadd(here->VBICbaseBXNode, -rhs_current);
            ckt->ldadd(here->VBICbaseBXBaseBXPtr,  Irbi_Vrbi);
            ckt->ldadd(here->VBICbaseBXBaseBIPtr, -Irbi_Vrbi);
            ckt->ldadd(here->VBICbaseBXBaseBIPtr,  Irbi_Vbei);
            ckt->ldadd(here->VBICbaseBXEmitEIPtr, -Irbi_Vbei);
            ckt->ldadd(here->VBICbaseBXBaseBIPtr,  Irbi_Vbci);
            ckt->ldadd(here->VBICbaseBXCollCIPtr, -Irbi_Vbci);
            ckt->rhsadd(here->VBICbaseBINode, rhs_current);
            ckt->ldadd(here->VBICbaseBIBaseBXPtr, -Irbi_Vrbi);
            ckt->ldadd(here->VBICbaseBIBaseBIPtr,  Irbi_Vrbi);
            ckt->ldadd(here->VBICbaseBIBaseBIPtr, -Irbi_Vbei);
            ckt->ldadd(here->VBICbaseBIEmitEIPtr,  Irbi_Vbei);
            ckt->ldadd(here->VBICbaseBIBaseBIPtr, -Irbi_Vbci);
            ckt->ldadd(here->VBICbaseBICollCIPtr,  Irbi_Vbci);
/*
c           Stamp element: Ire
*/
            ckt->ldadd(here->VBICemitEmitPtr,  Ire_Vre);
            ckt->ldadd(here->VBICemitEIEmitEIPtr,  Ire_Vre);
            ckt->ldadd(here->VBICemitEIEmitPtr, -Ire_Vre);
            ckt->ldadd(here->VBICemitEmitEIPtr, -Ire_Vre);
/*
c           Stamp element: Irbp
*/
            rhs_current = model->VBICtype * (Irbp - Irbp_Vrbp*Vrbp - Irbp_Vbep*Vbep - Irbp_Vbci*Vbci);
            ckt->rhsadd(here->VBICbaseBPNode, -rhs_current);
            ckt->ldadd(here->VBICbaseBPBaseBPPtr,  Irbp_Vrbp);
            ckt->ldadd(here->VBICbaseBPCollCXPtr, -Irbp_Vrbp);
            ckt->ldadd(here->VBICbaseBPBaseBXPtr,  Irbp_Vbep);
            ckt->ldadd(here->VBICbaseBPBaseBPPtr, -Irbp_Vbep);
            ckt->ldadd(here->VBICbaseBPBaseBIPtr,  Irbp_Vbci);
            ckt->ldadd(here->VBICbaseBPCollCIPtr, -Irbp_Vbci);
            ckt->rhsadd(here->VBICcollCXNode, rhs_current);
            ckt->ldadd(here->VBICcollCXBaseBPPtr, -Irbp_Vrbp);
            ckt->ldadd(here->VBICcollCXCollCXPtr,  Irbp_Vrbp);
            ckt->ldadd(here->VBICcollCXBaseBXPtr, -Irbp_Vbep);
            ckt->ldadd(here->VBICcollCXBaseBPPtr,  Irbp_Vbep);
            ckt->ldadd(here->VBICcollCXBaseBIPtr, -Irbp_Vbci);
            ckt->ldadd(here->VBICcollCXCollCIPtr,  Irbp_Vbci);
/*
c           Stamp element: Ibcp
*/
            rhs_current = model->VBICtype * (Ibcp - Ibcp_Vbcp*Vbcp);
            ckt->rhsadd(here->VBICsubsSINode, -rhs_current);
            ckt->ldadd(here->VBICsubsSISubsSIPtr,  Ibcp_Vbcp);
            ckt->ldadd(here->VBICsubsSIBaseBPPtr, -Ibcp_Vbcp);
            ckt->rhsadd(here->VBICbaseBPNode, rhs_current);
            ckt->ldadd(here->VBICbaseBPSubsSIPtr, -Ibcp_Vbcp);
            ckt->ldadd(here->VBICbaseBPBaseBPPtr,  Ibcp_Vbcp);
/*
c           Stamp element: Iccp
*/
            rhs_current = model->VBICtype * (Iccp - Iccp_Vbep*Vbep - Iccp_Vbci*Vbci - Iccp_Vbcp*Vbcp);
            ckt->rhsadd(here->VBICbaseBXNode, -rhs_current);
            ckt->ldadd(here->VBICbaseBXBaseBXPtr,  Iccp_Vbep);
            ckt->ldadd(here->VBICbaseBXBaseBPPtr, -Iccp_Vbep);
            ckt->ldadd(here->VBICbaseBXBaseBIPtr,  Iccp_Vbci);
            ckt->ldadd(here->VBICbaseBXCollCIPtr, -Iccp_Vbci);
            ckt->ldadd(here->VBICbaseBXSubsSIPtr,  Iccp_Vbcp);
            ckt->ldadd(here->VBICbaseBXBaseBPPtr, -Iccp_Vbcp);
            ckt->rhsadd(here->VBICsubsSINode, rhs_current);
            ckt->ldadd(here->VBICsubsSIBaseBXPtr, -Iccp_Vbep);
            ckt->ldadd(here->VBICsubsSIBaseBPPtr,  Iccp_Vbep);
            ckt->ldadd(here->VBICsubsSIBaseBIPtr, -Iccp_Vbci);
            ckt->ldadd(here->VBICsubsSICollCIPtr,  Iccp_Vbci);
            ckt->ldadd(here->VBICsubsSISubsSIPtr, -Iccp_Vbcp);
            ckt->ldadd(here->VBICsubsSIBaseBPPtr,  Iccp_Vbcp);
/*
c           Stamp element: Irs
*/
            ckt->ldadd(here->VBICsubsSubsPtr,  Irs_Vrs);
            ckt->ldadd(here->VBICsubsSISubsSIPtr,  Irs_Vrs);
            ckt->ldadd(here->VBICsubsSISubsPtr, -Irs_Vrs);
            ckt->ldadd(here->VBICsubsSubsSIPtr, -Irs_Vrs);
