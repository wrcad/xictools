...
/***********************************************************************
 ...
 VERSION : HiSIM 1.2.0
 FILE : hsm1eval102.c of HiSIM 1.2.0
...
*         Vth*, dVth*, dPpg, Qnm, Qbm, Igate, Igidl. 
...
*-----------------*/
    ...
    T4 = sIN.sc1 + sIN.sc3 * Psum / Leff ;
    T4_dVbs = sIN.sc3 * Psum_dVbs / Leff ;
    T4_dVds = sIN.sc3 * Psum_dVds / Leff ;
...
*-----------------*/
      ...
      T8 = Ec * Ec + 2.0e0 / C_ESI * q_Nsub * ( Psdl - Psl ) + 1.0e3 ;
...
*-----------------*/
    ...
    E1_dVds = E1 * ( ( sIN.gidl3 * Vdsz_dVds 
		       - Vgsz_dVds - dVth_dVds + dPpg_dVds) / T1 
		     - Tox_dVds / Tox ) ;
    E1_dVgs = E1 * ( ( -1.0 - dVth_dVgs + dPpg_dVgs) / T1 - Tox_dVgs / Tox ) ;
    ...
    Igidl_dVbs = T3 * Egidl_dVbs ;
    Igidl_dVds = T3 * Egidl_dVds ;
    Igidl_dVgs = T3 * Egidl_dVgs ;
...
*-----------------*/
    ...
    if ( pOT->cbsb > -1e-27 ) { pOT->cbsb = 0e0 ; }
 ...
 */
    ...
    T1 = 2 * C_ESI * Weff / C_Pi ;
    T2 = C_Pi * C_EOX / 2 / C_ESI ;
    T3 = sIN.xj / sIN.tox * sin( T2 ) ;
    Cgdf = T1 * log( 1.0e0 + T3 ) ;

    T4 = (-1) * ( pOT->cgdb ) ;
    T5 = 2 * T4 / Cox / Weff / Leff ;
    Cgdfring = Cgdf * ( 1.0e0 - T5 ) ;
...
*-----------------*/
    ...
    if ( sIN.coadov == 1 ) {
        ...
        pOT->cggb   += Cggo ;
	pOT->cgdb   += Cgdo + (-1) * Cgdfring ;
        pOT->cgsb   += Cgso ;

        pOT->cdgb   += - Qgod_dVgse - Cf ;
        pOT->cddb   += - Qgod_dVdse + Cf ;
        pOT->cdsb   += Qgod_dVbse + Qgod_dVdse + Qgod_dVgse ;
...
*-----------------*/ 
    ...
    pOT->ggbs   = Igate_dVbse ;
    pOT->ggds   = Igate_dVdse ;
    pOT->gggs   = Igate_dVgse ;
 
    pOT->igidl   = Igidl ;

    pOT->ggidlbs   = Igidl_dVbse ;
    pOT->ggidlds   = Igidl_dVdse ;
    pOT->ggidlgs   = Igidl_dVgse ;
