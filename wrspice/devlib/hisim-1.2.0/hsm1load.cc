 ...
 *                                                                        *
 *========================================================================*
 $Id: hsm1load.cc,v 1.6 2015/06/11 01:12:29 stevew Exp $
 *========================================================================*/
...
extern "C" {
extern int HSM1evaluate102(HiSIM_input, HiSIM_output*, HiSIM_messenger*);
extern int HSM1evaluate112(HiSIM_input, HiSIM_output*, HiSIM_messenger*);
extern int HSM1evaluate120(HiSIM_input, HiSIM_output*, HiSIM_messenger*);
}
...
int
// SRW HSM1dev::load(sGENmodel *genmod, sCKT *ckt)
HSM1dev::load(sGENinstance *in_inst, sCKT *ckt)
{
// SRW    sHSM1model *model = static_cast<sHSM1model*>(genmod);
// SRW    sHSM1instance *here;
    sHSM1instance *here = (sHSM1instance*)in_inst;
    sHSM1model *model = (sHSM1model*)here->GENmodPtr;
...
*/
      ...
      if ( ckt->CKTmode & MODEINITSMSIG ) {
        ...
        if (here->HSM1_mode >= 0) {
          ...
          cbhat = Ibtot + here->HSM1_gbs * delvbs
            ...
            + here->HSM1_gigislsd * delvds;
          Igbtot = here->HSM1_igb;
          cgbhat = here->HSM1_igb + here->HSM1_gigbg * delvgd 
        ...
        if(*(ckt->CKTstate0 + here->HSM1vds) >= 0.0) {
          vgs = DEV.fetlim(vgs, *(ckt->CKTstate0 + here->HSM1vgs), von);
          vds = vgs - vgd;
          vds = DEV.limvds(vds, *(ckt->CKTstate0 + here->HSM1vds));
          vgd = vgs - vds;
        } 
        else {
        ...
        if (vds >= 0.0) {
          ...
          vbs = DEV.pnjlim(vbs,*(ckt->CKTstate0 + here->HSM1vbs),
          ...
          vbd = DEV.pnjlim(vbd,*(ckt->CKTstate0 + here->HSM1vbd),
                          CONSTvt0,vcrit,&Check);
          vbs = vbd + vds;
        }
      }
 ...
 line900:
    ...
    */
    ...
    ckt->rhsadd(here->HSM1gNode, -(ceqqg + Igtoteq));
    ckt->rhsadd(here->HSM1bNode, -(ceqbs + ceqbd + ceqqb - Ibtoteq));
    ckt->rhsadd(here->HSM1dNodePrime, (ceqbd - cdreq - ceqqd + Idtoteq));
    ckt->rhsadd(here->HSM1sNodePrime,
      (cdreq + ceqbs + ceqqg + ceqqb + ceqqd + Istoteq));
    
    /*
     *  load y matrix
     */

    ckt->ldadd(here->HSM1DdPtr, here->HSM1drainConductance);
    ckt->ldadd(here->HSM1GgPtr, gcggb + gIgtotg);
    ckt->ldadd(here->HSM1SsPtr, here->HSM1sourceConductance);
    ckt->ldadd(here->HSM1BbPtr, here->HSM1_gbd + here->HSM1_gbs
      - gcbgb - gcbdb - gcbsb - here->HSM1_gbbs - gIbtotb);
    ckt->ldadd(here->HSM1DPdpPtr, here->HSM1drainConductance
      + here->HSM1_gds + here->HSM1_gbd + RevSum + gcddb + gbdpdp - gIdtotd);
    ckt->ldadd(here->HSM1SPspPtr, here->HSM1sourceConductance
      + here->HSM1_gds + here->HSM1_gbs + FwdSum + gcssb + gbspsp - gIstots);
    ckt->ldadd(here->HSM1DdpPtr, -here->HSM1drainConductance);
    ckt->ldadd(here->HSM1GbPtr, -(gcggb + gcgdb + gcgsb - gIgtotb));
    ckt->ldadd(here->HSM1GdpPtr, gcgdb + gIgtotd);
    ckt->ldadd(here->HSM1GspPtr, gcgsb + gIgtots);
    ckt->ldadd(here->HSM1SspPtr, -here->HSM1sourceConductance);
    ckt->ldadd(here->HSM1BgPtr, gcbgb - here->HSM1_gbgs - gIbtotg);
    ckt->ldadd(here->HSM1BdpPtr, gcbdb - here->HSM1_gbd + gbbdp - gIbtotd);
    ckt->ldadd(here->HSM1BspPtr, gcbsb - here->HSM1_gbs + gbbsp - gIbtots);
    ckt->ldadd(here->HSM1DPdPtr, -here->HSM1drainConductance);
    ckt->ldadd(here->HSM1DPgPtr, gm + gcdgb + gbdpg - gIdtotg);
    ckt->ldadd(here->HSM1DPbPtr, -(here->HSM1_gbd - gmbs + gcdgb + gcddb + gcdsb - gbdpb + gIdtotb));
    ckt->ldadd(here->HSM1DPspPtr, -(here->HSM1_gds + FwdSum - gcdsb - gbdpsp + gIdtots));
    ckt->ldadd(here->HSM1SPgPtr, gcsgb - gm + gbspg - gIstotg);
    ckt->ldadd(here->HSM1SPsPtr, -here->HSM1sourceConductance);
    ckt->ldadd(here->HSM1SPbPtr, -(here->HSM1_gbs + gmbs + gcsgb + gcsdb + gcssb - gbspb + gIstotb));
    ckt->ldadd(here->HSM1SPdpPtr, -(here->HSM1_gds + RevSum - gcsdb - gbspdp + gIstotd));

    /* stamp GIDL */
    ckt->ldadd(here->HSM1DPdpPtr, here->HSM1_gigidlds);
    ckt->ldadd(here->HSM1DPgPtr, here->HSM1_gigidlgs);
    ckt->ldadd(here->HSM1DPspPtr, -(here->HSM1_gigidlgs + 
                             here->HSM1_gigidlds + here->HSM1_gigidlbs));
    ckt->ldadd(here->HSM1DPbPtr, here->HSM1_gigidlbs);
    ckt->ldadd(here->HSM1BdpPtr, -here->HSM1_gigidlds);
    ckt->ldadd(here->HSM1BgPtr, -here->HSM1_gigidlgs);
    ckt->ldadd(here->HSM1BspPtr, (here->HSM1_gigidlgs + 
                             here->HSM1_gigidlds + here->HSM1_gigidlbs));
    ckt->ldadd(here->HSM1BbPtr, -here->HSM1_gigidlbs);
    /* stamp GISL */
    ckt->ldadd(here->HSM1SPdpPtr, -(here->HSM1_gigislsd + 
                             here->HSM1_gigislgd + here->HSM1_gigislbd));
    ckt->ldadd(here->HSM1SPgPtr, here->HSM1_gigislgd);
    ckt->ldadd(here->HSM1SPspPtr, here->HSM1_gigislsd);
    ckt->ldadd(here->HSM1SPbPtr, here->HSM1_gigislbd);
    ckt->ldadd(here->HSM1BdpPtr, (here->HSM1_gigislgd + 
                             here->HSM1_gigislsd + here->HSM1_gigislbd));
    ckt->ldadd(here->HSM1BgPtr, -here->HSM1_gigislgd);
    ckt->ldadd(here->HSM1BspPtr, -here->HSM1_gigislsd);
    ckt->ldadd(here->HSM1BbPtr, -here->HSM1_gigislbd);
