 ...
 *                                                                        *
 *========================================================================*
 $Id: b4set.cc,v 1.6 2016/09/26 01:47:50 stevew Exp $
 *========================================================================*/
...
// SRW - encapsulate the matrix element allocation.
#undef TSTALLOC
#define TSTALLOC(ptr, first, second) \
    if ((here->ptr = ckt->alloc(here->first, here->second)) == 0) \
        return (E_NOMEM); \
    if (here->BSIM4adjoint) here->BSIM4adjoint->ptr = \
        here->BSIM4adjoint->matrix->get_elem(here->first, here->second);

namespace
{
    int get_node_ptr(sCKT *ckt, sBSIM4model *model, sBSIM4instance *here)
    {
...
int
BSIM4dev::setup(sGENmodel *genmod, sCKT *ckt, int *states)
{
...
//    for (job = ((TSKtask *)ft_curckt->ci_curTask)->jobs;job;job = job->JOBnextJob) {
    for (sJOB *job = ckt->CKTcurTask->TSKjobs; job; job = job->JOBnextJob)
    {
//        if(strcmp(job->JOBname,"Noise Analysis")==0) {
        if (strcmp((const char*)job->JOBname, "NOISE") == 0)
        {
            noiseAnalGiven = 1;
            break;
        }
...
// SRW, added RGEOMODE as model param
        ...
        if (!model->BSIM4lk1Given)
            /* SRW - error here    model->BSIM4lkt1 = 0.0;  */
            model->BSIM4lk1 = 0.0; 
...
//SRW
            ...
            if (!here->BSIM4lGiven)
//SRW                here->BSIM4l = 5.0e-6;
                here->BSIM4l = ckt->mos_default_l();
//SRW            if (!here->BSIM4wGiven)
//SRW                here->BSIM4w = 5.0e-6;
            if (!here->BSIM4nfGiven)
                here->BSIM4nf = 1.0;
//SRW---
// Added wf (finger width) parameter.
            if (!here->BSIM4wGiven)
            {
                if (here->BSIM4wfGiven)
                    here->BSIM4w = here->BSIM4nf * here->BSIM4wf;
                else
                    here->BSIM4w = ckt->mos_default_w();
            }
            if (!here->BSIM4wfGiven)
                here->BSIM4wf = here->BSIM4w/here->BSIM4nf;
            else if (fabs(here->BSIM4w - here->BSIM4nf * here->BSIM4wf) >
                     1e-12)
            {
                printf("Warning: W and WF*NF differ, ignoring WF.\n");
                here->BSIM4wf = here->BSIM4w/here->BSIM4nf;
            }
//SRW---
...
//SRW                here->BSIM4drainArea = 0.0;
                here->BSIM4drainArea = ckt->mos_default_ad();
...
//SRW                here->BSIM4sourceArea = 0.0;
                here->BSIM4sourceArea = ckt->mos_default_as();
...
// SRW
            if (ckt->CKTcurTask->TSKuseAdjoint && !here->BSIM4adjoint)
                here->BSIM4adjoint = new BSIM4adj;
...
// SRW
#define BSIM4model sBSIM4model
#define BSIM4instance sBSIM4instance

int
BSIM4dev::unsetup(sGENmodel *inModel, sCKT*)
{
#ifndef HAS_BATCHSIM
    BSIM4model *model;
    BSIM4instance *here;
...
//
int
BSIM4dev::resetup(sGENmodel *inModel, sCKT *ckt)
{
    for (sBSIM4model *model = (sBSIM4model*)inModel; model;
            model = model->next())
    {
        for (sBSIM4instance *here = model->inst(); here;
                here = here->next())
        {
