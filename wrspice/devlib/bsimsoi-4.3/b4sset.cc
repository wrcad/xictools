 ...
 *                                                                        *
 *========================================================================*
 $Id: b4sset.cc,v 1.14 2016/09/26 01:47:54 stevew Exp $
 *========================================================================*/
...
#define TSTALLOC(ptr, first, second) \
    if ((here->ptr = ckt->alloc(here->first, here->second)) == 0) \
        return (E_NOMEM); \
    if (here->B4SOIadjoint) here->B4SOIadjoint->ptr = \
        here->B4SOIadjoint->matrix->get_elem(here->first, here->second);

namespace
{
    int get_node_ptr(sCKT *ckt, sB4SOImodel *model, sB4SOIinstance *here)
    {
...
int
B4SOIdev::setup(sGENmodel *genmod, sCKT *ckt, int *states)
{
...
//SRW
                here->B4SOIdrainArea = ckt->mos_default_ad();
            ...
            if (!here->B4SOIlGiven)
//SRW
                here->B4SOIl = ckt->mos_default_l();
            if (!here->B4SOIsourceAreaGiven)
//SRW
                here->B4SOIsourceArea = ckt->mos_default_as();
...
// SRW - braces added
            ...
            */
            ...
            if (ckt->CKTcurTask->TSKuseAdjoint && !here->B4SOIadjoint)
                here->B4SOIadjoint = new B4SOIadj;
...
// SRW
#define B4SOImodel sB4SOImodel
#define B4SOIinstance sB4SOIinstance

int
B4SOIdev::unsetup(sGENmodel *inModel, sCKT*)
{
#ifndef HAS_BATCHSIM
    B4SOImodel *model;
    B4SOIinstance *here;
...
//
int
B4SOIdev::resetup(sGENmodel *inModel, sCKT *ckt)
{
    for (sB4SOImodel *model = (sB4SOImodel*)inModel; model;
            model = model->next())
    {
        for (sB4SOIinstance *here = model->inst(); here;
                here = here->next())
        {
