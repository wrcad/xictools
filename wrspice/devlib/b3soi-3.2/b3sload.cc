 ...
 *                                                                        *
 *========================================================================*
 $Id: b3sload.cc,v 1.7 2015/06/11 01:12:28 stevew Exp $
 *========================================================================*/
...
Modified by Pin Su and Hui Wan 03/2/28
Modified by Pin Su 03/4/20
Modified by Pin Su and Hui Wan 03/07/30
Modified by Hui Wan 04/02/24
**********/
...
int
// SRWB3SOIdev::load(sGENmodel *genmod, sCKT *ckt)
B3SOIdev::load(sGENinstance *in_inst, sCKT *ckt)
{
// SRW    sB3SOImodel *model = static_cast<sB3SOImodel*>(genmod);
// SRW    sB3SOIinstance *here;
    sB3SOIinstance *here = (sB3SOIinstance*)in_inst;
    sB3SOImodel *model = (sB3SOImodel*)here->GENmodPtr;
...
//                    vb = here->B3SOIvbsusr + vs;
                ...
                vgge = *(ckt->CKTrhsOld + here->B3SOIgNodeExt);
                vggm = *(ckt->CKTrhsOld + here->B3SOIgNodeMid);

                vges = *(ckt->CKTstate0 + here->B3SOIvges);
                vgms = *(ckt->CKTstate0 + here->B3SOIvgms);
                /* v3.1 added for RF by wanh end*/
            }
...
//                    vb = here->B3SOIvbsusr + vs;
                ...
                vgge = *(ckt->CKTrhsOld + here->B3SOIgNodeExt);
                vggm = *(ckt->CKTrhsOld + here->B3SOIgNodeMid);
                vges = *(ckt->CKTstate1 + here->B3SOIvges);
                vgms = *(ckt->CKTstate1 + here->B3SOIvgms);
                /* v3.1 added for RF by wanh end*/
            ...
            else
            {
                ...
                if ((ckt->CKTmode & MODEINITPRED))
                {
                    ...
                    *(ckt->CKTstate0 + here->B3SOIve) = *(ckt->CKTstate1 + here->B3SOIve);
                    ...
                    /* v3.1 wanh added for RF */
                    *(ckt->CKTstate0 + here->B3SOIvgge) = *(ckt->CKTstate1 + here->B3SOIvgge);
                    *(ckt->CKTstate0 + here->B3SOIvggm) = *(ckt->CKTstate1 + here->B3SOIvggm);
                    *(ckt->CKTstate0 + here->B3SOIvges) =
                        *(ckt->CKTstate1 + here->B3SOIvges);
                    vges = (1.0 + xfact)* (*(ckt->CKTstate1 + here->B3SOIvges))
                           - (xfact * (*(ckt->CKTstate2 + here->B3SOIvges)));
                    *(ckt->CKTstate0 + here->B3SOIvgms) =
                        *(ckt->CKTstate1 + here->B3SOIvgms);
                    vgms = (1.0 + xfact)* (*(ckt->CKTstate1 + here->B3SOIvgms))
                           - (xfact * (*(ckt->CKTstate2 + here->B3SOIvgms)));
                    /* v3.1 added for RF by wanh end */
...
//                    vb = model->B3SOItype * vbs + vs;
                    ...
                    /* v3.1 wanh added for RF */
                    vgge = model->B3SOItype * vges + vs;
                    vggm = model->B3SOItype * vgms + vs;
                    /* v3.1 wanh added for RF end */
                ...
                else
                {
                    ...
                    /* v3.1 wanh added for RF */
                    vgge = B3SOIlimit(*(ckt->CKTrhsOld + here->B3SOIgNodeExt),
                                      *(ckt->CKTstate0 + here->B3SOIvgge), 3.0, &Check);

                    vggm = B3SOIlimit(*(ckt->CKTrhsOld + here->B3SOIgNodeMid),
                                      *(ckt->CKTstate0 + here->B3SOIvggm), 3.0, &Check);
                    /* v3.1 wanh added for RF end */
                    ...
                    /* v3.1 wanh added for RF */
                    vges = model->B3SOItype * (vgge - vs);
                    vgms = model->B3SOItype * (vggm - vs);
                    /* v3.1 wanh added for RF end */
                ...
                /* v3.1 wanh added for RF */
//                vgedo = *(ckt->CKTstate0 + here->B3SOIvges)
//                        - *(ckt->CKTstate0 + here->B3SOIvds);
//                vgmdo = *(ckt->CKTstate0 + here->B3SOIvgms)
//                        - *(ckt->CKTstate0 + here->B3SOIvds);
                vged = vges - vds;
                vgmd = vgms - vds;
//                delvged = vged - vgedo;
//                delvgmd = vgmd - vgmdo;
                ...
                /* v3.1 wanh added for RF */
                delvges = vges - *(ckt->CKTstate0 + here->B3SOIvges);
                delvgms = vgms - *(ckt->CKTstate0 + here->B3SOIvgms);
                /* v3.1 wanh added for RF end */
                                ...
                                if ((FABS(delves) < (ckt->CKTreltol * MAX(FABS(ves),
                                    ...
                                    if ( (here->B3SOIbodyMod == 0) || (here->B3SOIbodyMod == 2) ||
                                        ...
                                        if ( (here->B3SOItempNode == 0)  ||
                                            ...
                                            /* v3.1 wanh added for RF */
                                            if ((here->B3SOIrgateMod == 0) || (here->B3SOIrgateMod == 1)
                                                    || (FABS(delvges) < (ckt->CKTreltol * MAX(FABS(vges),
                                                                         FABS(*(ckt->CKTstate0 + here->B3SOIvges))) + ckt->CKTvoltTol)))
                                                if ((here->B3SOIrgateMod != 3) || (FABS(delvgms) < (ckt->CKTreltol
                                                                                   * MAX(FABS(vgms), FABS(*(ckt->CKTstate0 + here->B3SOIvgms)))
                                                                                   + ckt->CKTvoltTol)))
                                                    /* v3.1 wanh added for RF end */
                                                    ...
                                                    if ((FABS(delvds) < (ckt->CKTreltol * MAX(FABS(vds),
                                                        ...
                                                        if ((FABS(cdhat - here->B3SOIcd) < ckt->CKTreltol
                                                            ...
                                                            if ((here->B3SOIsoiMod == 2) ||       /* v3.2 */
                                                                    (FABS(cbhat - here->B3SOIcb) < ckt->CKTreltol
...
//                von = here->B3SOIvon;
                ...
                if (*(ckt->CKTstate0 + here->B3SOIvds) >= 0.0)
                {
                    ...
                    else if ((here->B3SOIrgateMod == 1) || (here->B3SOIrgateMod == 2))
                    {
                        vged = vges - vds;
                    }
                    /* v3.1 wanh added for RF end*/
                ...
                else
                {
                    ...
                    /* added for RF by wanh on 09/23/02 */
                    if (here->B3SOIrgateMod == 3)
                    {
                        vges = vged + vds;
                        vgms = vgmd + vds;
                    }
                    if ((here->B3SOIrgateMod == 1) || (here->B3SOIrgateMod == 2))
                    {
                        vges = vged + vds;
                    }
                    /* added for RF by wanh on 09/23/02 end*/
...
//                    vb = model->B3SOItype * vbs + vd;
            ...
            if (selfheat)
            {
                ...
                T7 = pParam->B3SOIxbjt * T4 / pParam->B3SOIndiode;
                dT7_dT = pParam->B3SOIxbjt * dT4_dT / pParam->B3SOIndiode;
                DEXP(T7, T0, dT0_dT7);
                dT0_dT = dT0_dT7 * dT7_dT;

                if (pParam->B3SOIxbjt == pParam->B3SOIxdif)
                {
                    T1 = T0;
                    dT1_dT = dT0_dT;
                }
                ...
                {
                    T7 = pParam->B3SOIxdif * T4 / pParam->B3SOIndiode;
                    dT7_dT = pParam->B3SOIxdif * dT4_dT / pParam->B3SOIndiode;
                    DEXP(T7, T1, dT1_dT7);
                    dT1_dT = dT1_dT7 * dT7_dT;
                }

                T7 = pParam->B3SOIxrec * T4 / pParam->B3SOInrecf0;
                dT7_dT = pParam->B3SOIxrec * dT4_dT / pParam->B3SOInrecf0;
                DEXP(T7, T2, dT2_dT7);
                ...
                T7 = pParam->B3SOIxtun * T3;
                dT7_dT = pParam->B3SOIxtun * T8;
                DEXP(T7, T0, dT0_dT7);
                dT0_dT = dT0_dT7 * dT7_dT;
            ...
            /* v3.1 bug fix */
            wdiosCV_NoSwap = pParam->B3SOIwdiosCV;
            wdiodCV_NoSwap = pParam->B3SOIwdiodCV;
            ...
            */
            ...
            if (here->B3SOIsoiMod == 0) /* BSIMPD */ /* v3.2 */
            {
                Vbsmos = Vbs;
                dVbsmos_dVg = 0.0;
                dVbsmos_dVd = 0.0;
            ...
            else /* soiMod = 1 or 2: adding FD module on top of BSIMPD */
            {
                ...
                T2 = T1 * (vbi - phi);
                T3 = 0.5 * pParam->B3SOIqsi / model->B3SOIcsi; /* v3.2 */
                ...
                T3 = 0.5 * (T1 + T2);
                T4 = T3 * model->B3SOIcsi / pParam->B3SOIqsi; /* v3.2 */
...
//                dPhis_dVb = -1; /* w.r.t Vbs0mos */
                ...
                if (here->B3SOIsoiMod == 2) /* v3.2 */ /* v3.1 ideal FD: Vbsitf is pinned at Vbs0 */
                {
            ...
            here->B3SOIvbseff = Vbseff; /* SPICE sol. */
...
//            dPhis_dVb = -1;
            ...
            here->B3SOInstar = model->B3SOIvtm / Charge_q *
                               (model->B3SOIcox + EPSSI / Xdep + pParam->B3SOIcit);
            /* Calculate n */
            ...
            here->B3SOIAbulk = Abulk; /*v3.2 for noise */
...
//            Gmc = 0.0;
            ...
            if (here->B3SOIsoiMod != 2) /* v3.2 */
            {
                ...
                NVtmf = 0.026 * pParam->B3SOInrecf0
                        * (1 + pParam->B3SOIntrecf * (TempRatio - 1));
                NVtmr = 0.026 * pParam->B3SOInrecr0 /* v2.2.2 bug fix */
                        * (1 + pParam->B3SOIntrecr * (TempRatio - 1));
                if (selfheat)
                {
                    dNVtmf_dT = pParam->B3SOInrecf0 * 0.026
                                * pParam->B3SOIntrecf * dTempRatio_dT;
                    dNVtmr_dT = pParam->B3SOInrecr0 * 0.026 /* v2.2.2 bug fix */
                                * pParam->B3SOIntrecr * dTempRatio_dT;
                }
                else
            ...
            {
                here->B3SOIigidl= Idgidl = Gdgidld = Gdgidlg = 0.0;
                Isgidl = Gsgidlg = 0;
            ...
            if ((model->B3SOIigbMod!= 0) && (here->B3SOIsoiMod != 2))  /* v3.2 */
                /* v3.1: the Igb calculation is skipped for the ideal FD mode */
            {
                ...
                T3 = pParam->B3SOIalphaGB1;
                T4 = pParam->B3SOIbetaGB1;
                ...
                T3 = pParam->B3SOIalphaGB2;
                T4 = pParam->B3SOIbetaGB2;
            ...
            if (here->B3SOIsoiMod != 2) /* v3.2 */
            {
                ...
                {
                    if (pParam->B3SOIrbody < 1e-3)  /* 3.2 bug fix */
                    {
                        if (here->B3SOIrbodyext <= 1e-3) /* 3.2 bug fix */
            ...
            else /* v3.1 soiMod=2: ideal FD */
            {
                ...
                here->B3SOIgbpbs = 0.0;
                here->B3SOIgbpps = here->B3SOIgbpT = here->B3SOIcbodcon = 0.0;
                Gbpbs = Gbpps = 0.0;
            }
            /* v3.1 */
            ...
            {
                T9 = pParam->B3SOIxrcrg2 * model->B3SOIvtm;
                T0 = T9 *beta;
                dT0_dVd = (dbeta_dVd + dbeta_dVg * dVgsteff_dVd) * T9;
                dT0_dVb = (dbeta_dVb + dbeta_dVg * dVgsteff_dVb) * T9;
                dT0_dVg = dbeta_dVg * T9;

                here->B3SOIgcrg = pParam->B3SOIxrcrg1 * (T0 + Ids);
                here->B3SOIgcrgd = pParam->B3SOIxrcrg1 * (dT0_dVd +tmp1);
                here->B3SOIgcrgb = pParam->B3SOIxrcrg1 * (dT0_dVb +tmp2)
                                   * dVbseff_dVb;
                here->B3SOIgcrgg = pParam->B3SOIxrcrg1 * (dT0_dVg +tmp3)
                                   * dVgsteff_dVg;

                if (here->B3SOIrgateMod == 2)
                {
                    T10 = here->B3SOIgrgeltd * here->B3SOIgrgeltd;
                    T11 = here->B3SOIgrgeltd + here->B3SOIgcrg;
                    here->B3SOIgcrg = here->B3SOIgrgeltd * here->B3SOIgcrg / T11;
                    T12 = T10 / T11 /T11;
                    here->B3SOIgcrgg *= T12;
                    here->B3SOIgcrgd *= T12;
                    here->B3SOIgcrgb *= T12;
                ...
                here->B3SOIgcrgs = -(here->B3SOIgcrgg + here->B3SOIgcrgd
                                     + here->B3SOIgcrgb);
            }
            ...
            else
            {
                ...
                noff = n * pParam->B3SOInoff;
                dnoff_dVd = pParam->B3SOInoff * dn_dVd;
                dnoff_dVb = pParam->B3SOInoff * dn_dVb;

                if ((VgstNVt > -EXPL_THRESHOLD) && (VgstNVt < EXPL_THRESHOLD))
                {
                    ExpVgst *= ExpVgst;
                    ExpVgst *= exp( -(pParam->B3SOIdelvt / (noff * Vtm)));
                    Vgsteff = noff * Vtm * log(1.0 + ExpVgst);
                    T0 = ExpVgst / (1.0 + ExpVgst);
                    T1 = -T0 * (dVth_dVb + (Vgst-pParam->B3SOIdelvt) / noff * dnoff_dVb)
                ...
                if (model->B3SOIcapMod == 2)
                {
                    ...
                    /* v3.1 */
                    if (here->B3SOIsoiMod == 2) /* v3.2 */ /* ideal FD */
                    {
...
//                    dVdsatCV_dVb = -VdsatCV * dAbulkCV_dVb / AbulkCV;
                    ...
                    */
                    ...
                    here->B3SOIqinv = -qinv; /* for noise v3.2 */
                ...
                else if (model->B3SOIcapMod == 3)
                {
                    ...
                    Cox = 3.453133e-11 / (model->B3SOItox - model->B3SOIdtoxcv); /* v2.2.3 */
...
//                    dDeltaPhi_dVb = dDeltaPhi_dVg * dVgsteff_dVb;
                    ...
                    if (here->B3SOIsoiMod == 2) /* v3.2 */ /* ideal FD */
                    {
// SRW                            qbulk = Cbg1 = Cbd1 = Cbb1 = Cbg1 = 0;
                        qbulk = Cbd1 = Cbb1 = Cbg1 = 0;
                    }
                    else /* soiMod = 0 or 1 */
                    {
                        T7 = 1.0 - AbulkCV;
...
finished: /* returning Values to Calling Routine */
            ...
            *(ckt->CKTstate0 + here->B3SOIdeltemp) = delTemp;
            ...
            /* v3.1 wanh added for RF */
            *(ckt->CKTstate0 + here->B3SOIvgge) = vgge;
            *(ckt->CKTstate0 + here->B3SOIvggm) = vggm;
            *(ckt->CKTstate0 + here->B3SOIvges) = vges;
            *(ckt->CKTstate0 + here->B3SOIvgms) = vgms;
            /* v3.1 wanh added for RF end*/
...
line755:
            ...
            T0 = vgd + DELTA_1;
            if (here->B3SOIrgateMod == 3) T0 = vgmd + DELTA_1; /* v3.2 bug fix */
            T1 = sqrt(T0 * T0 + 4.0 * DELTA_1);
            T2 = 0.5 * (T0 - T1);

            /* v2.2.3 bug fix */
            T3 = wdiodCV_NoSwap * pParam->B3SOIcgdl; /* v3.1 bug fix */
            ...
            if (here->B3SOIrgateMod == 3)
            {
                qgdo = (pParam->B3SOIcgdo + T3) * vgmd - T3 * (T2
                        + 0.5 * pParam->B3SOIckappa * (T4 - 1.0));
            }     /* v3.2 bug fix */

            T0 = vgs + DELTA_1;
            if (here->B3SOIrgateMod == 3) T0 = vgms + DELTA_1; /* v3.2 bug fix */
            T1 = sqrt(T0 * T0 + 4.0 * DELTA_1);
            T2 = 0.5 * (T0 - T1);

            /* v2.2.3 bug fix */
            T3 = wdiosCV_NoSwap * pParam->B3SOIcgsl; /* v3.1 bug fix */
            ...
            if (here->B3SOIrgateMod == 3)
            {
                qgso = (pParam->B3SOIcgso + T3) * vgms - T3 * (T2
                        + 0.5 * pParam->B3SOIckappa * (T4 - 1.0));
            }     /* v3.2 bug fix */
            ...
            if (here->B3SOImode > 0)
            {
                ...
                if (here->B3SOIrgateMod == 3)
                {
                    gcgmgmb = (cgdo + cgso + pParam->B3SOIcgeo) * ag0;

                    gcgmdb = -cgdo * ag0;
                    gcgmsb = -cgso * ag0;
                    gcgmeb = -pParam->B3SOIcgeo * ag0;
                    gcdgmb = gcgmdb;
                    gcsgmb = gcgmsb;
                    gcegmb = gcgmeb;

                    gcggb = here->B3SOIcggb * ag0;
                    gcgdb = here->B3SOIcgdb * ag0;
                    gcgsb = here->B3SOIcgsb * ag0;
                    gcgeb = 0 ;/*v3.1 wanh changed*/
                    gcgbb = -(gcggb + gcgdb + gcgsb + gcgeb);

                    gcdgb = here->B3SOIcdgb * ag0;
                    gcegb = gcgeb; /*v3.1 wanh added*/
                    gcsgb = -(here->B3SOIcggb + here->B3SOIcbgb + here->B3SOIcdgb) * ag0 - gcegb;
                    ...
                    qgme = pParam->B3SOIcgeo * vgme;
                    qgmid = qgdo + qgso + qgme;
                    qgate += qge;
                    qbody -= 0;
                    qdrn += here->B3SOIqde - qgd;
                    qsub -= qgme + here->B3SOIqse + here->B3SOIqde;
                ...
                else
                {
                    ...
                    gcsgb = -(here->B3SOIcggb + here->B3SOIcbgb
                              + here->B3SOIcdgb + cgso) * ag0;
                    gcbgb = here->B3SOIcbgb * ag0;
                ...
                gcssb = (cgso + here->B3SOIgcse - (here->B3SOIcgsb + here->B3SOIcbsb + here->B3SOIcdsb)) * ag0;
                gcseb = -(here->B3SOIgcse + here->B3SOIcbeb + here->B3SOIcdeb + here->B3SOIceeb) * ag0;
                gcsT = - model->B3SOItype * (here->B3SOIcgT + here->B3SOIcbT + here->B3SOIcdT + here->B3SOIceT) * ag0;
...
//                dxpart = 0.4;
            ...
            else
            {
                if (here->B3SOIrgateMod == 3)
                {
                    ...
                    gcggb = here->B3SOIcggb * ag0;
                    gcgsb = here->B3SOIcgdb * ag0;
                    gcgdb = here->B3SOIcgsb * ag0;
                    gcgeb = 0; /* v3.1 wanh changed */
                    gcgbb = -(gcggb + gcgdb + gcgsb + gcgeb); /* v3.1 wanh added gcgeb */

                    gcsgb = here->B3SOIcdgb * ag0;
                    gcegb = gcgeb; /* v3.1 wanh added */
                    gcdgb = -(here->B3SOIcggb + here->B3SOIcbgb
                              + here->B3SOIcdgb) * ag0 - gcegb; /*v3.1 wanh added gcegb*/
                    gcbgb = here->B3SOIcbgb * ag0;
                ...
                else
                {
                    ...
                    gcdgb = -(here->B3SOIcggb + here->B3SOIcbgb + here->B3SOIcdgb + cgdo) * ag0;
                    gcdgmb = gcsgmb = gcegmb = 0.0;
                    gcgmdb = gcgmsb = gcgmeb = 0.0;
                ...
                /*                  gcegb = (-pParam->B3SOIcgeo) * ag0; V3.2 bug fix */
                gcesb = (- here->B3SOIgcse) * ag0;
                gcedb = (- here->B3SOIgcde) * ag0;
...
//                sxpart = 0.4;
            ...
            *(ckt->CKTstate0 + here->B3SOIqb) = qbody;
            ...
            if (here->B3SOIrgateMod == 3)
                *(ckt->CKTstate0 + here->B3SOIqgmid) = qgmid;
            /* 3.1 bug fix */
            ...
            if (ckt->CKTmode & MODEINITTRAN)
            {
                ...
                *(ckt->CKTstate1 + here->B3SOIqth) =
                ...
                if (here->B3SOIrgateMod == 3)
                    *(ckt->CKTstate1 + here->B3SOIqgmid) =
                        *(ckt->CKTstate0 + here->B3SOIqgmid);
                /*3.1 bug fix*/
            ...
            if (here->B3SOIrgateMod == 3)
            {
                error = NIintegrate(ckt, &geq, &ceq, 0.0, here->B3SOIqgmid);
                if (error) return(error);
            }     /*3.1 bug fix*/
...
line860:
            ...
            ceqqd = cqdrn - gcdgb * vgb + gcddb * vbd + gcdsb * vbs
                    - gcdeb * veb - gcdT * delTemp -gcdgmb * vgmb;/* 3.2 bug fix */
            ...
            /* v3.1 wanh added for RF */
            if (here->B3SOIrgateMod == 3)
                ceqqgmid = *(ckt->CKTstate0 + here->B3SOIcqgmid)
            ...
            if (ckt->CKTmode & MODEINITTRAN)
            {
                ...
                *(ckt->CKTstate1 + here->B3SOIcqth) =
                ...
                /* v3.1 wanh added for RF */
                if (here->B3SOIrgateMod == 3)
                    *(ckt->CKTstate1 + here->B3SOIcqgmid) =
                        *(ckt->CKTstate0 + here->B3SOIcqgmid);
                /* v3.1 wanh added for RF end */
...
#define MSGN(x) (model->B3SOItype > 0 ? x : -(x))

            *(ckt->CKTstate0 + here->B3SOIa_cd) = MSGN(here->B3SOIcd);
            *(ckt->CKTstate0 + here->B3SOIa_cbs) = MSGN(here->B3SOIcjs);
            *(ckt->CKTstate0 + here->B3SOIa_cbd) = MSGN(here->B3SOIcjd);
            *(ckt->CKTstate0 + here->B3SOIa_cggb) = MSGN(here->B3SOIcggb);
            *(ckt->CKTstate0 + here->B3SOIa_cgdb) = MSGN(here->B3SOIcgdb);
            *(ckt->CKTstate0 + here->B3SOIa_cgsb) = MSGN(here->B3SOIcgsb);
            *(ckt->CKTstate0 + here->B3SOIa_cdgb) = MSGN(here->B3SOIcdgb);
            *(ckt->CKTstate0 + here->B3SOIa_cddb) = MSGN(here->B3SOIcddb);
            *(ckt->CKTstate0 + here->B3SOIa_cdsb) = MSGN(here->B3SOIcdsb);
            *(ckt->CKTstate0 + here->B3SOIa_cbgb) = MSGN(here->B3SOIcbgb);
            *(ckt->CKTstate0 + here->B3SOIa_cbdb) = MSGN(here->B3SOIcbdb);
            *(ckt->CKTstate0 + here->B3SOIa_cbsb) = MSGN(here->B3SOIcbsb);
            *(ckt->CKTstate0 + here->B3SOIa_von) = MSGN(here->B3SOIvon);
            *(ckt->CKTstate0 + here->B3SOIa_vdsat) = MSGN(here->B3SOIvdsat);

            *(ckt->CKTstate0 + here->B3SOIa_gm) = here->B3SOIgm;
            *(ckt->CKTstate0 + here->B3SOIa_gds) = here->B3SOIgds;
            *(ckt->CKTstate0 + here->B3SOIa_gmbs) = here->B3SOIgmbs;
            *(ckt->CKTstate0 + here->B3SOIa_gbd) = here->B3SOIgjdb;
            *(ckt->CKTstate0 + here->B3SOIa_gbs) = here->B3SOIgjsb;
// SRW - end

            /* v3.1 */
            if (here->B3SOIsoiMod != 2) /* v3.2 */
                ckt->rhsadd(here->B3SOIbNode, -(ceqbody + ceqqb));

            /* v3.1 wanh added ceqgcrg for RF */
            ckt->rhsadd(here->B3SOIgNode, -(ceqgate + ceqqg
                                                  + Igtoteq - ceqgcrg)); /* v3.1 */
            /* v3.1 wanh added ceqgcrg for RF end */

            ckt->rhsadd(here->B3SOIdNodePrime, (ceqbd - cdreq
                    - ceqqd) + Idtoteq); /* v3.0 */
            /* v3.1 wanh added ceqqgmid for RF */
            ckt->rhsadd(here->B3SOIsNodePrime, (cdreq + ceqbs
                    + ceqqg
                    + ceqqb + ceqqd + ceqqe) + Istoteq + ceqqgmid); /* v3.0 */
            /* v3.1 wanh added ceqqgmid for RF end */

            ckt->rhsadd(here->B3SOIeNode, -ceqqe);

            /* v3.1 wanh added for RF */
            if (here->B3SOIrgateMod == 2)
                ckt->rhsadd(here->B3SOIgNodeExt, -ceqgcrg);
            else if (here->B3SOIrgateMod == 3)
                ckt->rhsadd(here->B3SOIgNodeMid, -(ceqqgmid + ceqgcrg));
            /* v3.1 wanh added for RF end */

            if (here->B3SOIbodyMod == 1)
            {
                ckt->rhsadd(here->B3SOIpNode, ceqbodcon);
            }

            if (selfheat)
            {
                ckt->rhsadd(here->B3SOItempNode, -(ceqth + ceqqth));
            }
...
// SRW
            ...
            {
                dvaMatrix *mat = here->B3SOIadjoint->matrix;
                mat->clear();

                if (here->B3SOIsoiMod != 2) /* v3.2 */
                    (*mat->get_elem(here->B3SOIbNode) -= (ceqbody + ceqqb));

                /* v3.1 wanh added ceqgcrg for RF */
                (*mat->get_elem(here->B3SOIgNode) -= (ceqgate + ceqqg)
                                                     + Igtoteq - ceqgcrg); /* v3.1 */
                /* v3.1 wanh added ceqgcrg for RF end */

                (*mat->get_elem(here->B3SOIdNodePrime) += (ceqbd - cdreq
                        - ceqqd) + Idtoteq); /* v3.0 */
                /* v3.1 wanh added ceqqgmid for RF */
                (*mat->get_elem(here->B3SOIsNodePrime) += (cdreq + ceqbs
                        + ceqqg
                        + ceqqb + ceqqd + ceqqe) + Istoteq + ceqqgmid); /* v3.0 */
                /* v3.1 wanh added ceqqgmid for RF end */

                (*mat->get_elem(here->B3SOIeNode) -= ceqqe);

                /* v3.1 wanh added for RF */
                if (here->B3SOIrgateMod == 2)
                    (*mat->get_elem(here->B3SOIgNodeExt) -= ceqgcrg);
                else if (here->B3SOIrgateMod == 3)
                    (*mat->get_elem(here->B3SOIgNodeMid) -= ceqqgmid + ceqgcrg);
                /* v3.1 wanh added for RF end */

                if (here->B3SOIbodyMod == 1)
                {
                    (*mat->get_elem(here->B3SOIpNode) += ceqbodcon);
                }

                if (selfheat)
                {
                    (*mat->get_elem(here->B3SOItempNode) -= ceqth + ceqqth);
                }
            }
// SRW - end
            ...
            if (here->B3SOIrgateMod == 1)
            {
                ckt->ldadd(here->B3SOIGEgePtr, geltd);
                ckt->ldadd(here->B3SOIGgePtr, -geltd);
                ckt->ldadd(here->B3SOIGEgPtr, -geltd);
            }
            else if (here->B3SOIrgateMod == 2)
            {
                ckt->ldadd(here->B3SOIGEgePtr, gcrg);
                ckt->ldadd(here->B3SOIGEgPtr, gcrgg);
                ckt->ldadd(here->B3SOIGEdpPtr, gcrgd);
                ckt->ldadd(here->B3SOIGEspPtr, gcrgs);
                ckt->ldadd(here->B3SOIGgePtr, -gcrg);
                if (here->B3SOIsoiMod !=2) /* v3.2 */
                    ckt->ldadd(here->B3SOIGEbPtr, gcrgb);
            }
            else if (here->B3SOIrgateMod == 3)
            {
                ckt->ldadd(here->B3SOIGEgePtr, geltd);
                ckt->ldadd(here->B3SOIGEgmPtr, -geltd);
                ckt->ldadd(here->B3SOIGMgePtr, -geltd);
                ckt->ldadd(here->B3SOIGMgmPtr, geltd + gcrg + gcgmgmb);

                ckt->ldadd(here->B3SOIGMdpPtr, gcrgd + gcgmdb);
                ckt->ldadd(here->B3SOIGMgPtr, gcrgg);
                ckt->ldadd(here->B3SOIGMspPtr, gcrgs + gcgmsb);
                ckt->ldadd(here->B3SOIGMePtr, gcgmeb);
                if (here->B3SOIsoiMod !=2) /* v3.2 */
                    ckt->ldadd(here->B3SOIGMbPtr, gcrgb);

                ckt->ldadd(here->B3SOIDPgmPtr, gcdgmb);
                ckt->ldadd(here->B3SOIGgmPtr, -gcrg);
                ckt->ldadd(here->B3SOISPgmPtr, gcsgmb);
                ckt->ldadd(here->B3SOIEgmPtr, gcegmb);
            }
            /* v3.1 wanh added for RF end*/
            ...
            {
                ckt->ldadd(here->B3SOIDPePtr, Gme + gddpe);
                ckt->ldadd(here->B3SOISPePtr, gsspe - Gme);

                if (here->B3SOIsoiMod != 2) /* v3.2 */
                {
                    ckt->ldadd(here->B3SOIGePtr, gige);
                    ckt->ldadd(here->B3SOIBePtr, -gige);
                }
            }

            ckt->ldadd(here->B3SOIEdpPtr, gcedb);
            ckt->ldadd(here->B3SOIEspPtr, gcesb);
            ckt->ldadd(here->B3SOIDPePtr, gcdeb);
            ckt->ldadd(here->B3SOISPePtr, gcseb);
            ckt->ldadd(here->B3SOIEgPtr, gcegb);
            ckt->ldadd(here->B3SOIGePtr, gcgeb);

            /* v3.1 */
            if (here->B3SOIsoiMod != 2) /* v3.2 */
            {
                ckt->ldadd(here->B3SOIEbPtr, -(gcegb + gcedb + gcesb + gceeb + gcegmb)); /* 3.2 bug fix */

                /* v3.1 wanh changed GbPtr for RF */
                if ((here->B3SOIrgateMod == 0) || (here->B3SOIrgateMod == 1))
                    ckt->ldadd(here->B3SOIGbPtr, -(-gigb + gcggb + gcgdb + gcgsb
                                            + gcgeb - gIgtotb));
                else /* v3.1 wanh added for rgateMod = 2 or 3 */
                    ckt->ldadd(here->B3SOIGbPtr, gigb + gcgbb +gIgtotb - gcrgb);


                ckt->ldadd(here->B3SOIDPbPtr, -((-gddpb - Gmbs + gcdgb + gcddb
                                          + gcdeb + gcdsb) + gcdgmb
                                         + gIdtotb)); /* v3.2 bug fix */

                ckt->ldadd(here->B3SOISPbPtr, -((-gsspb + Gmbs + gcsgb + gcsdb
                                          + gcseb + gcssb) + gcsgmb
                                         + Gmin + gIstotb));
                /* v3.2 bug fix */

                ckt->ldadd(here->B3SOIBePtr, gbbe + gcbeb); /* v3.0 */
                ckt->ldadd(here->B3SOIBgPtr, -gigg + gcbgb + gbbg);
                ckt->ldadd(here->B3SOIBdpPtr, -gigd + gcbdb + gbbdp );
                ckt->ldadd(here->B3SOIBspPtr, gcbsb + gbbsp - Gmin - gigs);
                ckt->ldadd(here->B3SOIBbPtr, -gigb + gbbb - gcbgb - gcbdb
                                        - gcbsb - gcbeb + Gmin) ;
            }
            /* v3.1 */
            ...
            ckt->ldadd(here->B3SOIEePtr, gceeb);

            if (here->B3SOIrgateMod == 0)
            {
                ckt->ldadd(here->B3SOIGgPtr, gigg + gcggb + Gmin
                                        + gIgtotg);
                ckt->ldadd(here->B3SOIGdpPtr, gigd + gcgdb - Gmin
                                         + gIgtotd);
                ckt->ldadd(here->B3SOIGspPtr, gcgsb + gigs + gIgtots);
            }
            else if (here->B3SOIrgateMod == 1) /* v3.1 for RF by wanh */
            {
                ckt->ldadd(here->B3SOIGgPtr, gigg + gcggb + Gmin
                                       + gIgtotg + geltd);
                ckt->ldadd(here->B3SOIGdpPtr, gigd + gcgdb - Gmin
            ...
            else /* v3.1 for RF by wanh rgateMod == 2 or 3 */
            {
                ckt->ldadd(here->B3SOIGgPtr, gigg + gcggb + Gmin
                                       + gIgtotg - gcrgg);
                ckt->ldadd(here->B3SOIGdpPtr, gigd + gcgdb - Gmin
                                        + gIgtotd - gcrgd);
                ckt->ldadd(here->B3SOIGspPtr, gcgsb + gigs + gIgtots - gcrgs);
            }


            ckt->ldadd(here->B3SOIDPgPtr, (Gm + gcdgb) + gddpg - Gmin
                                     - gIdtotg); /* v3.0 */
            ckt->ldadd(here->B3SOIDPdpPtr, (here->B3SOIdrainConductance
                                       + here->B3SOIgds + gddpdp
                                       + RevSum + gcddb) + Gmin
                                      - gIdtotd); /* v3.0 */
            ckt->ldadd(here->B3SOIDPspPtr, -((-gddpsp + here->B3SOIgds + FwdSum
                                       - gcdsb) + gIdtots)); /* v3.0 */

            ckt->ldadd(here->B3SOIDPdPtr, -here->B3SOIdrainConductance);

            ckt->ldadd(here->B3SOISPgPtr, gcsgb - Gm + gsspg - gIstotg); /* v3.0 */
            ckt->ldadd(here->B3SOISPdpPtr, -(here->B3SOIgds - gsspdp + RevSum
                                       - gcsdb + gIstotd)); /* v3.0 */

            ckt->ldadd(here->B3SOISPspPtr, (here->B3SOIsourceConductance
                                       + here->B3SOIgds + gsspsp
                                       + FwdSum + gcssb)
                                      + Gmin - gIstots); /* v3.0 */

            ckt->ldadd(here->B3SOISPsPtr, -here->B3SOIsourceConductance);


            ckt->ldadd(here->B3SOIDdPtr, here->B3SOIdrainConductance);
            ckt->ldadd(here->B3SOIDdpPtr, -here->B3SOIdrainConductance);


            ckt->ldadd(here->B3SOISsPtr, here->B3SOIsourceConductance);
            ckt->ldadd(here->B3SOISspPtr, -here->B3SOIsourceConductance);


            if (here->B3SOIbodyMod == 1)
            {
                ckt->ldadd(here->B3SOIBpPtr, -gppp);
                ckt->ldadd(here->B3SOIPbPtr, gppb);
                ckt->ldadd(here->B3SOIPpPtr, gppp);
            }


            if (selfheat)
            {
                ckt->ldadd(here->B3SOIDPtempPtr, GmT + gddpT + gcdT);
                ckt->ldadd(here->B3SOISPtempPtr, -GmT + gsspT + gcsT);
                ckt->ldadd(here->B3SOIBtempPtr, gbbT + gcbT - gigT);
                ckt->ldadd(here->B3SOIEtempPtr, gceT);
                ckt->ldadd(here->B3SOIGtempPtr, gcgT + gigT);
                ckt->ldadd(here->B3SOITemptempPtr, gTtt  + 1/pParam->B3SOIrth + gcTt);
                ckt->ldadd(here->B3SOITempgPtr, gTtg);
                ckt->ldadd(here->B3SOITempbPtr, gTtb);
                ckt->ldadd(here->B3SOITempdpPtr, gTtdp);
                ckt->ldadd(here->B3SOITempspPtr, gTtsp);

                /* v3.0 */
                if (here->B3SOIsoiMod != 0) /* v3.2 */
                    ckt->ldadd(here->B3SOITempePtr, gTte);

            }

            if (here->B3SOIdebugMod != 0)
            {
                ckt->ldadd(here->B3SOIVbsPtr, 1.0);
                ckt->ldadd(here->B3SOIIdsPtr, 1.0);
                ckt->ldadd(here->B3SOIIcPtr, 1.0);
                ckt->ldadd(here->B3SOIIbsPtr, 1.0);
                ckt->ldadd(here->B3SOIIbdPtr, 1.0);
                ckt->ldadd(here->B3SOIIiiPtr, 1.0);
                ckt->ldadd(here->B3SOIIgPtr, 1.0);
                ckt->ldadd(here->B3SOIGiggPtr, 1.0);
                ckt->ldadd(here->B3SOIGigdPtr, 1.0);
                ckt->ldadd(here->B3SOIGigbPtr, 1.0);
                ckt->ldadd(here->B3SOIIgidlPtr, 1.0);
                ckt->ldadd(here->B3SOIItunPtr, 1.0);
                ckt->ldadd(here->B3SOIIbpPtr, 1.0);
                ckt->ldadd(here->B3SOICbgPtr, 1.0);
                ckt->ldadd(here->B3SOICbbPtr, 1.0);
                ckt->ldadd(here->B3SOICbdPtr, 1.0);
                ckt->ldadd(here->B3SOIQbfPtr, 1.0);
                ckt->ldadd(here->B3SOIQjsPtr, 1.0);
                ckt->ldadd(here->B3SOIQjdPtr, 1.0);
...
// SRW
            if (here->B3SOIadjoint)
            {
                ...
                else if (here->B3SOIrgateMod == 3)
                {
                    ...
                    *(adj->B3SOIGMgmPtr) += geltd + gcrg + gcgmgmb;

                    *(adj->B3SOIGMdpPtr) += gcrgd + gcgmdb;
                    *(adj->B3SOIGMgPtr) += gcrgg;
                    *(adj->B3SOIGMspPtr) += gcrgs + gcgmsb;
                    *(adj->B3SOIGMePtr) += gcgmeb;
                    if (here->B3SOIsoiMod !=2) /* v3.2 */
                        *(adj->B3SOIGMbPtr) += gcrgb;

                    *(adj->B3SOIDPgmPtr) += gcdgmb;
                    *(adj->B3SOIGgmPtr) -= gcrg;
                    *(adj->B3SOISPgmPtr) += gcsgmb;
                    *(adj->B3SOIEgmPtr) += gcegmb;
                }
                /* v3.1 wanh added for RF end*/
                ...
                *(adj->B3SOIEdpPtr) += gcedb;
                *(adj->B3SOIEspPtr) += gcesb;
                *(adj->B3SOIDPePtr) += gcdeb;
                *(adj->B3SOISPePtr) += gcseb;
                *(adj->B3SOIEgPtr) += gcegb;
                *(adj->B3SOIGePtr) += gcgeb;

                /* v3.1 */
                if (here->B3SOIsoiMod != 2) /* v3.2 */
                {
                    (*(adj->B3SOIEbPtr) -= gcegb + gcedb + gcesb + gceeb + gcegmb); /* 3.2 bug fix */

                    /* v3.1 wanh changed GbPtr for RF */
                    if ((here->B3SOIrgateMod == 0) || (here->B3SOIrgateMod == 1))
                        (*(adj->B3SOIGbPtr) -= -gigb + gcggb + gcgdb + gcgsb
                                               + gcgeb - gIgtotb);
                    else /* v3.1 wanh added for rgateMod = 2 or 3 */
                        *(adj->B3SOIGbPtr) += gigb + gcgbb +gIgtotb - gcrgb;


                    (*(adj->B3SOIDPbPtr) -= (-gddpb - Gmbs + gcdgb + gcddb
                                             + gcdeb + gcdsb) + gcdgmb
                                            + gIdtotb ); /* v3.2 bug fix */

                    (*(adj->B3SOISPbPtr) -= (-gsspb + Gmbs + gcsgb + gcsdb
                                             + gcseb + gcssb) + gcsgmb
                                            + Gmin + gIstotb);
                    /* v3.2 bug fix */

                    (*(adj->B3SOIBePtr) += gbbe + gcbeb); /* v3.0 */
                    (*(adj->B3SOIBgPtr) += -gigg + gcbgb + gbbg);
                    (*(adj->B3SOIBdpPtr) += -gigd + gcbdb + gbbdp );
                    (*(adj->B3SOIBspPtr) += gcbsb + gbbsp - Gmin
                                            - gigs);
                    (*(adj->B3SOIBbPtr) += -gigb + gbbb - gcbgb - gcbdb
                                           - gcbsb - gcbeb + Gmin) ;
                }
                /* v3.1 */
                ...
                (*(adj->B3SOIEePtr) += gceeb);

                if (here->B3SOIrgateMod == 0)
                {
                    (*(adj->B3SOIGgPtr) += gigg + gcggb + Gmin
                                           + gIgtotg);
                    (*(adj->B3SOIGdpPtr) += gigd + gcgdb - Gmin
                                            + gIgtotd);
                    (*(adj->B3SOIGspPtr) += gcgsb + gigs + gIgtots);
                }
                else if (here->B3SOIrgateMod == 1) /* v3.1 for RF by wanh */
                {
                    *(adj->B3SOIGgPtr) += gigg + gcggb + Gmin
                ...
                else /* v3.1 for RF by wanh rgateMod == 2 or 3 */
                {
                    ...
                    *(adj->B3SOIGdpPtr) += gigd + gcgdb - Gmin
                                           + gIgtotd - gcrgd;
                    *(adj->B3SOIGspPtr) += gcgsb + gigs + gIgtots - gcrgs;
                }


                (*(adj->B3SOIDPgPtr) += (Gm + gcdgb) + gddpg - Gmin
                                        - gIdtotg); /* v3.0 */
                (*(adj->B3SOIDPdpPtr) += (here->B3SOIdrainConductance
                                          + here->B3SOIgds + gddpdp
                                          + RevSum + gcddb) + Gmin
                                         - gIdtotd); /* v3.0 */
                (*(adj->B3SOIDPspPtr) -= (-gddpsp + here->B3SOIgds + FwdSum
                                          - gcdsb) + gIdtots); /* v3.0 */

                (*(adj->B3SOIDPdPtr) -= here->B3SOIdrainConductance);

                (*(adj->B3SOISPgPtr) += gcsgb - Gm + gsspg - gIstotg); /* v3.0 */
                (*(adj->B3SOISPdpPtr) -= (here->B3SOIgds - gsspdp + RevSum
                                          - gcsdb + gIstotd)); /* v3.0 */

                (*(adj->B3SOISPspPtr) += (here->B3SOIsourceConductance
                                          + here->B3SOIgds + gsspsp
                                          + FwdSum + gcssb)
                                         + Gmin - gIstots); /* v3.0 */

                (*(adj->B3SOISPsPtr) -= here->B3SOIsourceConductance);


                (*(adj->B3SOIDdPtr) += here->B3SOIdrainConductance);
                (*(adj->B3SOIDdpPtr) -= here->B3SOIdrainConductance);


                (*(adj->B3SOISsPtr) += here->B3SOIsourceConductance);
                (*(adj->B3SOISspPtr) -= here->B3SOIsourceConductance);
                ...
                if (selfheat)
                {
                    (*(adj->B3SOIDPtempPtr) += GmT + gddpT + gcdT);
                    (*(adj->B3SOISPtempPtr) += -GmT + gsspT + gcsT);
                    (*(adj->B3SOIBtempPtr) += gbbT + gcbT - gigT);
                    (*(adj->B3SOIEtempPtr) += gceT);
                    (*(adj->B3SOIGtempPtr) += gcgT + gigT);
                    (*(adj->B3SOITemptempPtr) += gTtt  + 1/pParam->B3SOIrth + gcTt);
                ...
                *(ckt->CKTstate0 + here->B3SOIa_id) =
                    adj->matrix->compute_real(here->B3SOIdNode, ckt->CKTrhsOld);
                *(ckt->CKTstate0 + here->B3SOIa_is) =
                    adj->matrix->compute_real(here->B3SOIsNode, ckt->CKTrhsOld);
                *(ckt->CKTstate0 + here->B3SOIa_ig) =
                    adj->matrix->compute_real(here->B3SOIgNode, ckt->CKTrhsOld);
                *(ckt->CKTstate0 + here->B3SOIa_ib) =
                    adj->matrix->compute_real(here->B3SOIbNode, ckt->CKTrhsOld);
                *(ckt->CKTstate0 + here->B3SOIa_ie) =
                    adj->matrix->compute_real(here->B3SOIeNode, ckt->CKTrhsOld);
            }
// SRW - end
