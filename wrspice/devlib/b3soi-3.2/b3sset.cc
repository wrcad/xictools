 ...
 *                                                                        *
 *========================================================================*
 $Id: b3sset.cc,v 1.13 2016/09/26 01:47:22 stevew Exp $
 *========================================================================*/
...
#define TSTALLOC(ptr, first, second) \
    if ((here->ptr = ckt->alloc(here->first, here->second)) == 0) \
        return (E_NOMEM); \
    if (here->B3SOIadjoint) here->B3SOIadjoint->ptr = \
        here->B3SOIadjoint->matrix->get_elem(here->first, here->second);

namespace
{
    int get_node_ptr(sCKT *ckt, sB3SOImodel *model, sB3SOIinstance *here)
    {
        ...
        {
            TSTALLOC(B3SOIGEgePtr, B3SOIgNodeExt, B3SOIgNodeExt)
            TSTALLOC(B3SOIGEgPtr, B3SOIgNodeExt, B3SOIgNode)
            TSTALLOC(B3SOIGgePtr, B3SOIgNode, B3SOIgNodeExt)
            TSTALLOC(B3SOIGEdpPtr, B3SOIgNodeExt, B3SOIdNodePrime)
            TSTALLOC(B3SOIGEspPtr, B3SOIgNodeExt, B3SOIsNodePrime)
            if (here->B3SOIsoiMod !=2) /* v3.2 */
                TSTALLOC(B3SOIGEbPtr, B3SOIgNodeExt, B3SOIbNode)

                TSTALLOC(B3SOIGMdpPtr, B3SOIgNodeMid, B3SOIdNodePrime)
                TSTALLOC(B3SOIGMgPtr, B3SOIgNodeMid, B3SOIgNode)
                TSTALLOC(B3SOIGMgmPtr, B3SOIgNodeMid, B3SOIgNodeMid)
                TSTALLOC(B3SOIGMgePtr, B3SOIgNodeMid, B3SOIgNodeExt)
                TSTALLOC(B3SOIGMspPtr, B3SOIgNodeMid, B3SOIsNodePrime)
                if (here->B3SOIsoiMod !=2) /* v3.2 */
                    TSTALLOC(B3SOIGMbPtr, B3SOIgNodeMid, B3SOIbNode)

                    TSTALLOC(B3SOIGMePtr, B3SOIgNodeMid, B3SOIeNode)
                    TSTALLOC(B3SOIDPgmPtr, B3SOIdNodePrime, B3SOIgNodeMid)
                    TSTALLOC(B3SOIGgmPtr, B3SOIgNode, B3SOIgNodeMid)
                    TSTALLOC(B3SOIGEgmPtr, B3SOIgNodeExt, B3SOIgNodeMid)
                    TSTALLOC(B3SOISPgmPtr, B3SOIsNodePrime, B3SOIgNodeMid)
                    TSTALLOC(B3SOIEgmPtr, B3SOIeNode, B3SOIgNodeMid)
                }
...
int
B3SOIdev::setup(sGENmodel *genmod, sCKT *ckt, int *states)
{
...
//int    itmp1;
    ...
    for( ; model != NULL; model = model->B3SOInextModel )
    {
        ...
        if (!model->B3SOItoxmGiven)
            model->B3SOItoxm = model->B3SOItox; /* v3.2 */

        /* v3.2 */
        if (!model->B3SOIsoiModGiven)
            model->B3SOIsoiMod = 0;
        else if ((model->B3SOIsoiMod != 0) && (model->B3SOIsoiMod != 1)
                 && (model->B3SOIsoiMod != 2) && (model->B3SOIsoiMod != 3))
        {
            model->B3SOIsoiMod = 0;
        ...
        /* v3.1 wanh added for RF */
        if (!model->B3SOIrgateModGiven)
            model->B3SOIrgateMod = 0;
        else if ((model->B3SOIrgateMod != 0) && (model->B3SOIrgateMod != 1)
                 && (model->B3SOIrgateMod != 2) && (model->B3SOIrgateMod != 3))
        {
            model->B3SOIrgateMod = 0;
        ...
        /* v3.2 for noise */
        if (!model->B3SOIfnoiModGiven)
            model->B3SOIfnoiMod = 1;
        else if ((model->B3SOIfnoiMod != 0) && (model->B3SOIfnoiMod != 1))
        {
            model->B3SOIfnoiMod = 1;
            printf("Waring: fnoiMod has been set to default value:1.\n");
        }

        if (!model->B3SOItnoiModGiven)
            model->B3SOItnoiMod = 0;
        else if ((model->B3SOItnoiMod != 0) && (model->B3SOItnoiMod != 1))
        {
            model->B3SOItnoiMod = 0;
            printf("Waring: tnoiMod has been set to default value:0.\n");
        }

        if (!model->B3SOItnoiaGiven)
            model->B3SOItnoia = 1.5;
        if (!model->B3SOItnoibGiven)
            model->B3SOItnoib = 3.5;
        if (!model->B3SOIrnoiaGiven)
            model->B3SOIrnoia = 0.577;
        if (!model->B3SOIrnoibGiven)
            model->B3SOIrnoib = 0.37;
        if (!model->B3SOIntnoiGiven)
            model->B3SOIntnoi = 1.0;
        ...
        */
        ...
        /* v3.0 */
        if (!model->B3SOIvbs0pdGiven)
            model->B3SOIvbs0pd = 0.0; /* v3.2 */
        if (!model->B3SOIvbs0fdGiven)
            model->B3SOIvbs0fd = 0.5; /* v3.2 */
        ...
        /* v3.1 wanh added for RF */
        if (!model->B3SOIxrcrg1Given)
            model->B3SOIxrcrg1 =12.0;
        if (!model->B3SOIxrcrg2Given)
            model->B3SOIxrcrg2 =1.0;
        if (!model->B3SOIrshgGiven)
            model->B3SOIrshg =0.1;
        if (!model->B3SOIngconGiven)
            model->B3SOIngcon =1.0;
        if (!model->B3SOIxgwGiven)
            model->B3SOIxgw = 0.0;
        if (!model->B3SOIxglGiven)
            model->B3SOIxgl = 0.0;
        /* v3.1 wanh added for RF end */
        ...
        if (!model->B3SOInoffGiven)
            model->B3SOInoff = 1.0; /* v3.2 */
        ...
        /* Length dependence */
        /* v3.1 */
        if (!model->B3SOIlxjGiven)
            model->B3SOIlxj = 0.0;
        if (!model->B3SOIlalphaGB1Given)
            model->B3SOIlalphaGB1 = 0.0;
        if (!model->B3SOIlalphaGB2Given)
            model->B3SOIlalphaGB2 = 0.0;
        if (!model->B3SOIlbetaGB1Given)
            model->B3SOIlbetaGB1 = 0.0;
        if (!model->B3SOIlbetaGB2Given)
            model->B3SOIlbetaGB2 = 0.0;
        if (!model->B3SOIlndifGiven)
            model->B3SOIlndif = 0.0;
        if (!model->B3SOIlntrecfGiven)
            model->B3SOIlntrecf = 0.0;
        if (!model->B3SOIlntrecrGiven)
            model->B3SOIlntrecr = 0.0;
        if (!model->B3SOIlxbjtGiven)
            model->B3SOIlxbjt = 0.0;
        if (!model->B3SOIlxdifGiven)
            model->B3SOIlxdif = 0.0;
        if (!model->B3SOIlxrecGiven)
            model->B3SOIlxrec = 0.0;
        if (!model->B3SOIlxtunGiven)
            model->B3SOIlxtun = 0.0;
        if (!model->B3SOIlcgdlGiven)
            model->B3SOIlcgdl = 0.0;
        if (!model->B3SOIlcgslGiven)
            model->B3SOIlcgsl = 0.0;
        if (!model->B3SOIlckappaGiven)
            model->B3SOIlckappa = 0.0;
        if (!model->B3SOIluteGiven)
            model->B3SOIlute = 0.0;
        if (!model->B3SOIlkt1Given)
            model->B3SOIlkt1 = 0.0;
        if (!model->B3SOIlkt2Given)
            model->B3SOIlkt2 = 0.0;
        if (!model->B3SOIlkt1lGiven)
            model->B3SOIlkt1l = 0.0;
        if (!model->B3SOIlua1Given)
            model->B3SOIlua1 = 0.0;
        if (!model->B3SOIlub1Given)
            model->B3SOIlub1 = 0.0;
        if (!model->B3SOIluc1Given)
            model->B3SOIluc1 = 0.0;
        if (!model->B3SOIlatGiven)
            model->B3SOIlat = 0.0;
        if (!model->B3SOIlprtGiven)
            model->B3SOIlprt = 0.0;
        ...
        if (!model->B3SOIlnoffGiven)
            model->B3SOIlnoff = 0.0; /* v3.2 */

        /* v3.1 wanh added for RF */
        if (!model->B3SOIlxrcrg1Given)
            model->B3SOIlxrcrg1 =0.0;
        if (!model->B3SOIlxrcrg2Given)
            model->B3SOIlxrcrg2 =0.0;
        /* v3.1 wanh added for RF end */

        /* Width dependence */
        /* v3.1 */
        if (!model->B3SOIwxjGiven)
            model->B3SOIwxj = 0.0;
        if (!model->B3SOIwalphaGB1Given)
            model->B3SOIwalphaGB1 = 0.0;
        if (!model->B3SOIwalphaGB2Given)
            model->B3SOIwalphaGB2 = 0.0;
        if (!model->B3SOIwbetaGB1Given)
            model->B3SOIwbetaGB1 = 0.0;
        if (!model->B3SOIwbetaGB2Given)
            model->B3SOIwbetaGB2 = 0.0;
        if (!model->B3SOIwndifGiven)
            model->B3SOIwndif = 0.0;
        if (!model->B3SOIwntrecfGiven)
            model->B3SOIwntrecf = 0.0;
        if (!model->B3SOIwntrecrGiven)
            model->B3SOIwntrecr = 0.0;
        if (!model->B3SOIwxbjtGiven)
            model->B3SOIwxbjt = 0.0;
        if (!model->B3SOIwxdifGiven)
            model->B3SOIwxdif = 0.0;
        if (!model->B3SOIwxrecGiven)
            model->B3SOIwxrec = 0.0;
        if (!model->B3SOIwxtunGiven)
            model->B3SOIwxtun = 0.0;
        if (!model->B3SOIwcgdlGiven)
            model->B3SOIwcgdl = 0.0;
        if (!model->B3SOIwcgslGiven)
            model->B3SOIwcgsl = 0.0;
        if (!model->B3SOIwckappaGiven)
            model->B3SOIwckappa = 0.0;
        if (!model->B3SOIwuteGiven)
            model->B3SOIwute = 0.0;
        if (!model->B3SOIwkt1Given)
            model->B3SOIwkt1 = 0.0;
        if (!model->B3SOIwkt2Given)
            model->B3SOIwkt2 = 0.0;
        if (!model->B3SOIwkt1lGiven)
            model->B3SOIwkt1l = 0.0;
        if (!model->B3SOIwua1Given)
            model->B3SOIwua1 = 0.0;
        if (!model->B3SOIwub1Given)
            model->B3SOIwub1 = 0.0;
        if (!model->B3SOIwuc1Given)
            model->B3SOIwuc1 = 0.0;
        if (!model->B3SOIwatGiven)
            model->B3SOIwat = 0.0;
        if (!model->B3SOIwprtGiven)
            model->B3SOIwprt = 0.0;
        ...
        /* v3.1 wanh added for RF */
        if (!model->B3SOIwxrcrg1Given)
            model->B3SOIwxrcrg1 =0.0;
        if (!model->B3SOIwxrcrg2Given)
            model->B3SOIwxrcrg2 =0.0;
        /* v3.1 wanh added for RF end */
        ...
        if (!model->B3SOIwnoffGiven)
            model->B3SOIwnoff = 0.0; /* v3.2 */

        /* Cross-term dependence */
        /* v3.1 */
        if (!model->B3SOIpxjGiven)
            model->B3SOIpxj = 0.0;
        if (!model->B3SOIpalphaGB1Given)
            model->B3SOIpalphaGB1 = 0.0;
        if (!model->B3SOIpalphaGB2Given)
            model->B3SOIpalphaGB2 = 0.0;
        if (!model->B3SOIpbetaGB1Given)
            model->B3SOIpbetaGB1 = 0.0;
        if (!model->B3SOIpbetaGB2Given)
            model->B3SOIpbetaGB2 = 0.0;
        if (!model->B3SOIpndifGiven)
            model->B3SOIpndif = 0.0;
        if (!model->B3SOIpntrecfGiven)
            model->B3SOIpntrecf = 0.0;
        if (!model->B3SOIpntrecrGiven)
            model->B3SOIpntrecr = 0.0;
        if (!model->B3SOIpxbjtGiven)
            model->B3SOIpxbjt = 0.0;
        if (!model->B3SOIpxdifGiven)
            model->B3SOIpxdif = 0.0;
        if (!model->B3SOIpxrecGiven)
            model->B3SOIpxrec = 0.0;
        if (!model->B3SOIpxtunGiven)
            model->B3SOIpxtun = 0.0;
        if (!model->B3SOIpcgdlGiven)
            model->B3SOIpcgdl = 0.0;
        if (!model->B3SOIpcgslGiven)
            model->B3SOIpcgsl = 0.0;
        if (!model->B3SOIpckappaGiven)
            model->B3SOIpckappa = 0.0;
        if (!model->B3SOIputeGiven)
            model->B3SOIpute = 0.0;
        if (!model->B3SOIpkt1Given)
            model->B3SOIpkt1 = 0.0;
        if (!model->B3SOIpkt2Given)
            model->B3SOIpkt2 = 0.0;
        if (!model->B3SOIpkt1lGiven)
            model->B3SOIpkt1l = 0.0;
        if (!model->B3SOIpua1Given)
            model->B3SOIpua1 = 0.0;
        if (!model->B3SOIpub1Given)
            model->B3SOIpub1 = 0.0;
        if (!model->B3SOIpuc1Given)
            model->B3SOIpuc1 = 0.0;
        if (!model->B3SOIpatGiven)
            model->B3SOIpat = 0.0;
        if (!model->B3SOIpprtGiven)
            model->B3SOIpprt = 0.0;
        ...
        /* v3.1 wanh for RF */
        if (!model->B3SOIpxrcrg1Given)
            model->B3SOIpxrcrg1 =0.0;
        if (!model->B3SOIpxrcrg2Given)
            model->B3SOIpxrcrg2 =0.0;
        /* v3.1 wanh for RF end */
        ...
        if (!model->B3SOIpnoffGiven)
            model->B3SOIpnoff = 0.0; /* v3.2 */
        ...
        if (!model->B3SOIoxideTrapDensityAGiven)
        {
            if (model->B3SOItype == NMOS)
                model->B3SOIoxideTrapDensityA = 6.25e41;
            else
                model->B3SOIoxideTrapDensityA=6.188e40;
        }
        if (!model->B3SOIoxideTrapDensityBGiven)
        {
            if (model->B3SOItype == NMOS)
                model->B3SOIoxideTrapDensityB = 3.125e26;
            else
                model->B3SOIoxideTrapDensityB = 1.5e25;
        }
        if (!model->B3SOIoxideTrapDensityCGiven)
            model->B3SOIoxideTrapDensityC = 8.75e9;
                ...
                here=here->B3SOInextInstance)
        {
            ...
            *states += B3SOInumStates;
            ...
            if (!here->B3SOIdrainAreaGiven)
                here->B3SOIdrainArea = ckt->mos_default_ad();
            ...
            /* v3.2 */
            Qsi = Charge_q * model->B3SOInpeak
                  * (1.0 + model->B3SOInlx / here->B3SOIl) * 1e6 * model->B3SOItsi;
            Vbs0t = 0.8 - 0.5 * Qsi / model->B3SOIcsi + model->B3SOIvbsa;

            if (!here->B3SOIsoiModGiven)
                here->B3SOIsoiMod = model->B3SOIsoiMod;
            else if ((here->B3SOIsoiMod != 0) && (here->B3SOIsoiMod != 1)
                     && (here->B3SOIsoiMod != 2) && (here->B3SOIsoiMod != 3))
            {
                here->B3SOIsoiMod = model->B3SOIsoiMod;
                printf("Warning: soiMod has been set to its global value %d.\n", model->B3SOIsoiMod);
            }

            if (here->B3SOIsoiMod == 3) /* auto selection */
            {
                if (Vbs0t > model->B3SOIvbs0fd)
                    here->B3SOIsoiMod = 2; /* ideal FD mode */
                else if (Vbs0t < model->B3SOIvbs0pd)
                    here->B3SOIsoiMod = 0; /* BSIMPD */
                else
                    here->B3SOIsoiMod = 1;
            }
            ...
            /* v3.1 wanh added for RF */
            if (!here->B3SOIrgateModGiven)
                here->B3SOIrgateMod = model->B3SOIrgateMod;
            else if ((here->B3SOIrgateMod != 0) && (here->B3SOIrgateMod != 1)
                     && (here->B3SOIrgateMod != 2) && (here->B3SOIrgateMod != 3))
            {
                here->B3SOIrgateMod = model->B3SOIrgateMod;
                printf("Warning: rgateMod has been set to its global value %d.\n", model->B3SOIrgateMod);
            }
            if ((here->B3SOIrgateMod > 0) && (here->B3SOIgNode == 0))
            {
                error = CKTmkVolt(ckt,&tmp,here->B3SOIname,"gate");
                if(error) return(error);
                here->B3SOIgNode = tmp->number();
                if (here->B3SOIrgateMod == 1)
                {
                    if (model->B3SOIrshg <= 0.0)
                ...
                else if (here->B3SOIrgateMod == 2)
                {
                    ...
                    else if (model->B3SOIxrcrg1 <= 0.0)
                        printf("Warning: xrcrg1 <= 0.0 for rgateMod = 2!!!\n");
                }
            }
            else
            {
                here->B3SOIgNode = here->B3SOIgNodeExt;
            }

            if ((here->B3SOIrgateMod == 3) && (here->B3SOIgNodeMid == 0))
            {
                error = CKTmkVolt(ckt,&tmp,here->B3SOIname,"midgate");
                if(error) return(error);
                here->B3SOIgNodeMid = tmp->number();
                if (model->B3SOIrshg <= 0.0)
                ...
                here->B3SOIgNodeMid = here->B3SOIgNodeExt;
            /* v3.1 wanh added for RF end */
            ...
            */
            ...
            if (ckt->CKTcurTask->TSKuseAdjoint && !here->B3SOIadjoint)
                here->B3SOIadjoint = new B3SOIadj;
            error = get_node_ptr(ckt, model, here);
            if (error != OK)
                return (error);
        }
    }
...
// SRW
#define B3SOImodel sB3SOImodel
#define B3SOIinstance sB3SOIinstance

int
B3SOIdev::unsetup(sGENmodel *inModel, sCKT*)
{
#ifndef HAS_BATCHSIM
    B3SOImodel *model;
    B3SOIinstance *here;
...
//
int
B3SOIdev::resetup(sGENmodel *inModel, sCKT *ckt)
{
    for (sB3SOImodel *model = (sB3SOImodel*)inModel; model;
            model = model->next())
    {
        for (sB3SOIinstance *here = model->inst(); here;
                here = here->next())
        {
            int error = get_node_ptr(ckt, model, here);
            if (error != OK)
                return (error);
        }
    }
