 ...
 *                                                                        *
 *========================================================================*
 $Id: b3stemp.cc,v 1.5 2011/12/18 01:15:13 stevew Exp $
 *========================================================================*/
...
#define B3SOIcheckModel checkModel
...
int
B3SOIdev::temperature(sGENmodel *genmod, sCKT *ckt)
{
    ...
    for (; model != NULL; model = model->B3SOInextModel)
    {
                ...
                here = here->B3SOInextInstance)
        {
            ...
            if (Size_Not_Found)
            {
                ...
                /* v3.1 */
                pParam->B3SOIxj = model->B3SOIxj
                                  + model->B3SOIlxj * Inv_L
                                  + model->B3SOIwxj * Inv_W
                                  + model->B3SOIpxj * Inv_LW;
                pParam->B3SOIalphaGB1 = model->B3SOIalphaGB1
                                        + model->B3SOIlalphaGB1 * Inv_L
                                        + model->B3SOIwalphaGB1 * Inv_W
                                        + model->B3SOIpalphaGB1 * Inv_LW;
                pParam->B3SOIalphaGB2 = model->B3SOIalphaGB2
                                        + model->B3SOIlalphaGB2 * Inv_L
                                        + model->B3SOIwalphaGB2 * Inv_W
                                        + model->B3SOIpalphaGB2 * Inv_LW;
                pParam->B3SOIbetaGB1 = model->B3SOIbetaGB1
                                       + model->B3SOIlbetaGB1* Inv_L
                                       + model->B3SOIwbetaGB1 * Inv_W
                                       + model->B3SOIpbetaGB1 * Inv_LW;
                pParam->B3SOIbetaGB2 = model->B3SOIbetaGB2
                                       + model->B3SOIlbetaGB2 * Inv_L
                                       + model->B3SOIwbetaGB2 * Inv_W
                                       + model->B3SOIpbetaGB2 * Inv_LW;
                pParam->B3SOIndif = model->B3SOIndif
                                    + model->B3SOIlndif * Inv_L
                                    + model->B3SOIwndif * Inv_W
                                    + model->B3SOIpndif * Inv_LW;
                pParam->B3SOIntrecf = model->B3SOIntrecf
                                      + model->B3SOIlntrecf* Inv_L
                                      + model->B3SOIwntrecf * Inv_W
                                      + model->B3SOIpntrecf * Inv_LW;
                pParam->B3SOIntrecr = model->B3SOIntrecr
                                      + model->B3SOIlntrecr * Inv_L
                                      + model->B3SOIwntrecr * Inv_W
                                      + model->B3SOIpntrecr * Inv_LW;
                pParam->B3SOIxbjt = model->B3SOIxbjt
                                    + model->B3SOIlxbjt * Inv_L
                                    + model->B3SOIwxbjt * Inv_W
                                    + model->B3SOIpxbjt * Inv_LW;
                pParam->B3SOIxdif = model->B3SOIxdif
                                    + model->B3SOIlxdif* Inv_L
                                    + model->B3SOIwxdif * Inv_W
                                    + model->B3SOIpxdif * Inv_LW;
                pParam->B3SOIxrec = model->B3SOIxrec
                                    + model->B3SOIlxrec * Inv_L
                                    + model->B3SOIwxrec * Inv_W
                                    + model->B3SOIpxrec * Inv_LW;
                pParam->B3SOIxtun = model->B3SOIxtun
                                    + model->B3SOIlxtun * Inv_L
                                    + model->B3SOIwxtun * Inv_W
                                    + model->B3SOIpxtun * Inv_LW;
                pParam->B3SOIcgdl = model->B3SOIcgdl
                                    + model->B3SOIlcgdl * Inv_L
                                    + model->B3SOIwcgdl * Inv_W
                                    + model->B3SOIpcgdl * Inv_LW;
                pParam->B3SOIcgsl = model->B3SOIcgsl
                                    + model->B3SOIlcgsl * Inv_L
                                    + model->B3SOIwcgsl * Inv_W
                                    + model->B3SOIpcgsl * Inv_LW;
                pParam->B3SOIckappa = model->B3SOIckappa
                                      + model->B3SOIlckappa * Inv_L
                                      + model->B3SOIwckappa * Inv_W
                                      + model->B3SOIpckappa * Inv_LW;
                pParam->B3SOIute = model->B3SOIute
                                   + model->B3SOIlute * Inv_L
                                   + model->B3SOIwute * Inv_W
                                   + model->B3SOIpute * Inv_LW;
                pParam->B3SOIkt1 = model->B3SOIkt1
                                   + model->B3SOIlkt1 * Inv_L
                                   + model->B3SOIwkt1 * Inv_W
                                   + model->B3SOIpkt1 * Inv_LW;
                pParam->B3SOIkt2 = model->B3SOIkt2
                                   + model->B3SOIlkt2 * Inv_L
                                   + model->B3SOIwkt2 * Inv_W
                                   + model->B3SOIpkt2 * Inv_LW;
                pParam->B3SOIkt1l = model->B3SOIkt1l
                                    + model->B3SOIlkt1l * Inv_L
                                    + model->B3SOIwkt1l * Inv_W
                                    + model->B3SOIpkt1l * Inv_LW;
                pParam->B3SOIua1 = model->B3SOIua1
                                   + model->B3SOIlua1 * Inv_L
                                   + model->B3SOIwua1 * Inv_W
                                   + model->B3SOIpua1 * Inv_LW;
                pParam->B3SOIub1 = model->B3SOIub1
                                   + model->B3SOIlub1* Inv_L
                                   + model->B3SOIwub1 * Inv_W
                                   + model->B3SOIpub1 * Inv_LW;
                pParam->B3SOIuc1 = model->B3SOIuc1
                                   + model->B3SOIluc1 * Inv_L
                                   + model->B3SOIwuc1 * Inv_W
                                   + model->B3SOIpuc1 * Inv_LW;
                pParam->B3SOIat = model->B3SOIat
                                  + model->B3SOIlat * Inv_L
                                  + model->B3SOIwat * Inv_W
                                  + model->B3SOIpat * Inv_LW;
                pParam->B3SOIprt = model->B3SOIprt
                                   + model->B3SOIlprt * Inv_L
                                   + model->B3SOIwprt * Inv_W
                                   + model->B3SOIpprt * Inv_LW;
                ...
                pParam->B3SOIxrcrg1 = model->B3SOIxrcrg1
                                      + model->B3SOIlxrcrg1 * Inv_L
                                      + model->B3SOIwxrcrg1 * Inv_W
                                      + model->B3SOIpxrcrg1 * Inv_LW;
                pParam->B3SOIxrcrg2 = model->B3SOIxrcrg2
                                      + model->B3SOIlxrcrg2 * Inv_L
                                      + model->B3SOIwxrcrg2 * Inv_W
                                      + model->B3SOIpxrcrg2 * Inv_LW;
                ...
                pParam->B3SOIacde = pParam->B3SOIacde *
                                    pow((pParam->B3SOInpeak / 2.0e16), -0.25);
                /* v3.2 bug fix */
                ...
                pParam->B3SOInoff = model->B3SOInoff
                                    + model->B3SOIlnoff * Inv_L
                                    + model->B3SOIwnoff * Inv_W
                                    + model->B3SOIpnoff * Inv_LW; /* v3.2 */
                ...
                if (B3SOIcheckModel(model, here, ckt))
                {
                    ...
                    /* SRW
                                          (*(SPfrontEnd->IFerror)) (ERR_FATAL,
                                          "Fatal error(s) detected during B3SOIV3 parameter checking for %s in model %s",
                                          namarray);
                    */
                    DVO.textOut(OUT_FATAL,
                             "Fatal error(s) detected during B3SOIV3 parameter checking for %s in model %s",
                ...
                T7 = pParam->B3SOIxtun * (TempRatio - 1);
                DEXP(T7, T0);
                ...
                pParam->B3SOIlitl = sqrt(3.0 * pParam->B3SOIxj
                                         * model->B3SOItox);
                ...
                pParam->B3SOIAechvbEdge = pParam->B3SOIAechvb * pParam->B3SOIweff/here->B3SOInseg
                                          * pParam->B3SOIdlcig * pParam->B3SOIToxRatioEdge; /* v3.1 bug fix */
                pParam->B3SOIBechvbEdge = -pParam->B3SOIBechvb
                                          * model->B3SOItoxqm * pParam->B3SOIpoxedge;
                pParam->B3SOIAechvb *= pParam->B3SOIweff/here->B3SOInseg * pParam->B3SOIleff
                                       * pParam->B3SOIToxRatio; /* v3.1 bug fix */
                ...
                /* v3.2 */
                pParam->B3SOIk1eff *= model->B3SOItox / model->B3SOItoxm;
                pParam->B3SOIk2 *= model->B3SOItox / model->B3SOItoxm;
            ...
            pParam->B3SOIlratiodif = 1.0 + model->B3SOIldif0 * pow(T0,pParam->B3SOIndif);
            ...
            /* v3.2 */
            pParam->B3SOIqsi = Charge_q * model->B3SOInpeak
                               * (1.0 + pParam->B3SOInlx / pParam->B3SOIleff)
                               * 1e6 * model->B3SOItsi;


            /* v3.1 wanh added for RF */
            here->B3SOIgrgeltd = model->B3SOIrshg * (model->B3SOIxgw
                                 +pParam->B3SOIweff/here->B3SOInseg / 3.0 / model->B3SOIngcon) /
                                 (model->B3SOIngcon * (here->B3SOIl - model->B3SOIxgl));
            if (here->B3SOIgrgeltd > 0.0)
                here->B3SOIgrgeltd = 1.0 / here->B3SOIgrgeltd;
            else
            {
                here->B3SOIgrgeltd = 1.0e3; /* mho */
