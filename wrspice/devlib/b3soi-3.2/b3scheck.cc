 ...
 *                                                                        *
 *========================================================================*
 $Id: b3scheck.cc,v 1.7 2011/12/18 01:15:13 stevew Exp $
 *========================================================================*/
...
//    if ((fplog = fopen("b3soiv1check.log", "w")) != NULL)
    {
        pParam = here->pParam;
// SRW
        ChkFprintf(fplog, "BSIMSOI-3.2 Parameter Check\n");
        ChkFprintf(fplog, "Model = %s\n", (const char*)model->B3SOImodName);
...
// SRW
        ...
        {
            if (pParam->B3SOIacde < 0.1) /* v3.1.1 */
            {
        ...
        if (model->B3SOIparamChk ==1)
        {
            ...
            */
            ...
            if ((model->B3SOIacde < 0.1) || (model->B3SOIacde > 1.6))
            {
            ...
            if (pParam->B3SOIxj > model->B3SOItsi)
            {
                ChkFprintf(fplog, "Warning: Xj = %g is thicker than Tsi = %g.\n",
                           pParam->B3SOIxj, model->B3SOItsi);
...
// SRW
            {
                ChkFprintf(fplog, "Warning: capMod < 2 is not supported by BSIMSOI-3.2.\n");
                WarnPrintf("Warning: Warning: capMod < 2 is not supported by BSIMSOI-3.2.\n");
            }
