 ...
 *                                                                        *
 *========================================================================*
 $Id: b2eval.cc,v 1.0 1998/01/30 05:27:36 stevew Exp $
 *========================================================================*/
...
//
void
B2dev::evaluate(double Vds, double Vbs, double Vgs, sB2instance *inst,
    sB2model *model, double *gm, double *gds, double *gmb, double *qg,
    double *qb, double *qd, double *cgg, double *cgd, double *cgs,
    double *cbg, double *cbd, double *cbs, double *cdg, double *cdd,
    double *cds, double *Ids, double *von, double *vdsat, sCKT *ckt)
{
    double Vth, Vdsat=0;
    double Phisb, T1s, Eta, Gg, Aa, Inv_Aa, U1, U1s, Vc, Kk, SqrtKk;
    double dPhisb_dVb, dT1s_dVb, dVth_dVb, dVth_dVd, dAa_dVb, dVc_dVd;
    double dVc_dVg, dVc_dVb, dKk_dVc, dVdsat_dVd=0, dVdsat_dVg=0, dVdsat_dVb=0;
    ...
    double T0, T1, T2, T3, T4, T5, Arg1, Arg2, Exp0=0;
    double tmp, tmp1, tmp2, tmp3, Uvert, Beta1, Beta2, Beta0, dGg_dVb, Exp1=0;
    double T6, T7, T8, T9, n=0, ExpArg, ExpArg1;
...
/* Threshold Voltage. */
    if (Vbs <= 0.0)
    {   Phisb = inst->pParam->B2phi - Vbs;
        dPhisb_dVb = -1.0;
        T1s = sqrt(Phisb);
        dT1s_dVb = -0.5 / T1s; 
    }
    else
    {   tmp = inst->pParam->B2phi / (inst->pParam->B2phi + Vbs);
        Phisb = inst->pParam->B2phi * tmp;
        dPhisb_dVb = -tmp * tmp;
        T1s = inst->pParam->Phis3 / (inst->pParam->B2phi + 0.5 * Vbs);
        dT1s_dVb = -0.5 * T1s * T1s / inst->pParam->Phis3;
    }

    Eta = inst->pParam->B2eta0 + inst->pParam->B2etaB * Vbs;
    Ua = inst->pParam->B2ua0 + inst->pParam->B2uaB * Vbs;
    Ub = inst->pParam->B2ub0 + inst->pParam->B2ubB * Vbs;
    U1s = inst->pParam->B2u10 + inst->pParam->B2u1B * Vbs;

    Vth = inst->pParam->B2vfb + inst->pParam->B2phi + inst->pParam->B2k1
        * T1s - inst->pParam->B2k2 * Phisb - Eta * Vds;
    dVth_dVd = -Eta;
    dVth_dVb = inst->pParam->B2k1 * dT1s_dVb + inst->pParam->B2k2 
         - inst->pParam->B2etaB * Vds;
    ...
    T0 = Gg / T1s;
    tmp1 = 0.5 * T0 * inst->pParam->B2k1;
    Aa = 1.0 + tmp1;
    dAa_dVb = (Aa - 1.0) * (dGg_dVb / Gg - dT1s_dVb / T1s);
    Inv_Aa = 1.0 / Aa;

    Vghigh = inst->pParam->B2vghigh;
    Vglow = inst->pParam->B2vglow;

    if ((Vgst >= Vghigh) || (inst->pParam->B2n0 == 0.0))
    {   Vgeff = Vgst;
    dVgeff_dVg = 1.0;
    dVgeff_dVd = -dVth_dVd;
    dVgeff_dVb = -dVth_dVb;
    ...
    {   Vof = inst->pParam->B2vof0 + inst->pParam->B2vofB * Vbs
        + inst->pParam->B2vofD * Vds;
    n = inst->pParam->B2n0 + inst->pParam->B2nB / T1s 
      + inst->pParam->B2nD * Vds;
    tmp = 0.5 / (n * model->B2Vtm);

    ExpArg1 = -Vds / model->B2Vtm;
    ExpArg1 = SPMAX(ExpArg1, -30.0);
    Exp1 = exp(ExpArg1);
    tmp1 = 1.0 - Exp1;
    tmp1 = SPMAX(tmp1, 1.0e-18);
    tmp2 = 2.0 * Aa * tmp1;

    if (Vgst <= Vglow)
    {
            ExpArg = Vgst * tmp;
        ExpArg = SPMAX(ExpArg, -30.0);
        ...
        dVgeff_dVd = dVgeff_dVg * (n / tmp1 * Exp1 - dVth_dVd - Vgst
               * inst->pParam->B2nD / n + T0 * inst->pParam->B2vofD);
        dVgeff_dVb = dVgeff_dVg * (inst->pParam->B2vofB * T0
               - dVth_dVb + inst->pParam->B2nB * Vgst
    ...
    else
    {
        ...
        dVgeff_dVg = (Coeffb + Vgst * (2.0 * Coeffc + 3.0 * Vgst * Coeffd))
               * delta;
        T7 = Con3 * tmp;
        T8 = dT1s_dVb * inst->pParam->B2nB / (T1s * T1s * n);
        T9 = n * model->B2Vtm;
        dCon3_dVd = T7 * (n * Exp1 / tmp1 - Vglow * inst->pParam->B2nD 
              / n + T9 * inst->pParam->B2vofD);
        dCon3_dVb = T7 * (T9 * Inv_Aa * dAa_dVb + Vglow * T8
              + T9 * inst->pParam->B2vofB);
        dCon4_dVd = tmp * dCon3_dVd - T7 * inst->pParam->B2nD / n;
    ...
    if (Vgeff > 0.0)
    {
        ...
        Uvert = SPMAX(Uvert, 0.2);
        Inv_Uvert = 1.0 / Uvert;
        T8 = Ua + 2.0 * Ub * Vgeff;
        dUvert_dVg = T8 * dVgeff_dVg;
        dUvert_dVd = T8 * dVgeff_dVd;
        dUvert_dVb = T8 * dVgeff_dVb + Vgeff * (inst->pParam->B2uaB
               + Vgeff * inst->pParam->B2ubB);
        ...
        dVc_dVb = T8 * dVgeff_dVb + inst->pParam->B2u1B * Vgeff * Inv_Aa 
        ...
        Beta0 = inst->pParam->B2beta0 + inst->pParam->B2beta0B * Vbs;
        Betas = inst->pParam->B2betas0 + inst->pParam->B2betasB * Vbs;
        Beta2 = inst->pParam->B2beta20 + inst->pParam->B2beta2B * Vbs
          + inst->pParam->B2beta2G * Vgs;
        Beta3 = inst->pParam->B2beta30 + inst->pParam->B2beta3B * Vbs
          + inst->pParam->B2beta3G * Vgs;
        Beta4 = inst->pParam->B2beta40 + inst->pParam->B2beta4B * Vbs
          + inst->pParam->B2beta4G * Vgs;
        ...
        dBeta_dVg = T4 * (inst->pParam->B2beta2G * Vds - T0 * dVdsat_dVg) 
              + inst->pParam->B2beta3G * (Vds - T5) 
          - inst->pParam->B2beta4G * (Vds * Vds - model->B2vdd * T5);
        dBeta1_dVb = inst->pParam->Arg;
        dBeta_dVb = inst->pParam->B2beta0B + dBeta1_dVb * tanh + Vds
              * (inst->pParam->B2beta3B - Vds * inst->pParam->B2beta4B)
              + T4 * (inst->pParam->B2beta2B * Vds - T0 * dVdsat_dVb);
        ...
        if (Vds <= Vdsat) /* triode region */
        {
            T3 = Vds * Inv_Vdsat;
            T4 = T3 - 1.0;
            T2 =  1.0 - inst->pParam->B2u1D * T4 * T4;
            U1 =  U1s * T2;
            Utot = Uvert + U1 * Vds;
            Utot = SPMAX(Utot, 0.5);
            Inv_Utot = 1.0 / Utot;
            T5 = 2.0 * U1s * inst->pParam->B2u1D * Inv_Vdsat * T4;
            dU1_dVd = T5 * (T3 * dVdsat_dVd - 1.0);
            dU1_dVg = T5 * T3 * dVdsat_dVg;
            dU1_dVb = T5 * T3 * dVdsat_dVb + inst->pParam->B2u1B * T2;
          ...
          if (inst->pParam->B2ai0 != 0.0)
          {
              Ai = inst->pParam->B2ai0 + inst->pParam->B2aiB * Vbs;
              Bi = inst->pParam->B2bi0 + inst->pParam->B2biB * Vbs;
              T5 = Bi / (Vds - Vdsat);
              T5 = SPMIN(T5, 30.0);
              T6 = exp(-T5);
              FR = 1.0 + Ai * T6;
              ...
              dFR_dVb = T8 * dVdsat_dVb + T6 * (inst->pParam->B2aiB - Ai
                  * inst->pParam->B2biB / (Vds - Vdsat));
       ...
       {   T0 = Exp0 * Exp0;
           ...
           *Ids = Beta * model->B2Vtm * model->B2Vtm * T0 * (1.0 - T1);
           ...
           T4 = n * model->B2Vtm;
           T3 = *Ids / T4;
           if ((Vds > Vdsat) && inst->pParam->B2ai0 != 0.0)
           {   Ai = inst->pParam->B2ai0 + inst->pParam->B2aiB * Vbs;
               Bi = inst->pParam->B2bi0 + inst->pParam->B2biB * Vbs;
               T5 = Bi / (Vds - Vdsat);
               T5 = SPMIN(T5, 30.0);
           ...
           *gds = (T2 * dBeta_dVd + T3 * (inst->pParam->B2vofD * T4 - dVth_dVd
            - inst->pParam->B2nD * Vgst / n) + Beta * model->B2Vtm 
        * T0 * T1) * FR + *Ids * dFR_dVd;
           *gm = (T2 * dBeta_dVg + T3) * FR + *Ids * dFR_dVg;
           *gmb = (T2 * dBeta_dVb + T3 * (inst->pParam->B2vofB * T4 - dVth_dVb 
            + inst->pParam->B2nB * Vgst / (n * T1s * T1s) * dT1s_dVb)) * FR
            + *Ids * dFR_dVb;
           *Ids *= FR;
       }
   }
...
/* ChargeComputation: */
    ...
    *gds = SPMAX(*gds,1.0e-20);
    ...
    else
    {
       ...
       {   Vbseff = inst->pParam->B2phi - Phisb;
       dVbseff_dVb = -dPhisb_dVb;
       }
       Arg1 = Vgs - Vbseff - inst->pParam->B2vfb;
       Arg2 = Arg1 - Vgst;
       Qbulk = inst->pParam->One_Third_CoxWL * Arg2;
       dQbulk_dVb = inst->pParam->One_Third_CoxWL * (dVth_dVb - dVbseff_dVb);
       dQbulk_dVd = inst->pParam->One_Third_CoxWL * dVth_dVd;
       if (Arg1 <= 0.0)
       {
          *qg = inst->pParam->CoxWL * Arg1;
          *qb = -(*qg);
          *qd = 0.0;

      *cgg = inst->pParam->CoxWL;
      *cgd = 0.0;
      ...
      *cbg = -inst->pParam->CoxWL;
      *cbd = 0.0;
      *cbs = -*cgs;
       }
       else if (Vgst <= 0.0)
       {  T2 = Arg1 / Arg2;
          T3 = T2 * T2 * (inst->pParam->CoxWL - inst->pParam->Two_Third_CoxWL
         * T2);

      *qg = inst->pParam->CoxWL * Arg1 * (1.0 - T2 * (1.0 - T2 / 3.0));
          *qb = -(*qg);
          *qd = 0.0;

      *cgg = inst->pParam->CoxWL * (1.0 - T2 * (2.0 - T2));
      ...
      *cbs = -*cgs;
      ...
      {    Uvert = 1.0 + Vgst * (Ua + Vgst * Ub);
            ...
            * (inst->pParam->B2uaB + Vgst * inst->pParam->B2ubB);
               ...
               dVc_dVb = -T8 * dVth_dVb + inst->pParam->B2u1B * Vgst * Inv_Aa 
                   * Inv_Uvert - Vc * Inv_Aa * dAa_dVb - T9 * dUvert_dVb;
          ...
          if (Vds >= Vdsat)
          {       /* saturation region */
          *cgg = inst->pParam->Two_Third_CoxWL;
          ...
          *qb = -Qbulk;
          *qg = inst->pParam->Two_Third_CoxWL * Vgst + Qbulk;
          *qd = *cdg * Vgst;
           }
           else
           {       /* linear region  */
          ...
          *qg = inst->pParam->Two_Third_CoxWL * (Vgst + Vgdt 
             - Vgdt * T0) + Qbulk;
          *qb = -Qbulk;
          *qd = -inst->pParam->One_Third_CoxWL * (0.2 * Vgdt 
         + 0.8 * Vgst + Vgdt * T1 
         + 0.2 * T5 * (Vgdt - Vgst));

          *cgg = inst->pParam->Two_Third_CoxWL * (T2 + T3 * dVgdt_dVg);
          tmp = dQbulk_dVb + inst->pParam->Two_Third_CoxWL * (T3 * dVgdt_dVb 
              - T2 * dVth_dVb);
          *cgd = inst->pParam->Two_Third_CoxWL * (T3 * dVgdt_dVd 
           - T2 * dVth_dVd) + dQbulk_dVd;
          *cgs = -(*cgg + *cgd + tmp);

          T2 = 0.8 - 0.4 * T1 * (2.0 * T1 + T0 + T0 * (T1 - T0));
          T3 = 0.2 + T1 + T0 * (1.0 - 0.4 * T0 * (T1 + 3.0 * T0));
          *cdg = -inst->pParam->One_Third_CoxWL * (T2 + T3 * dVgdt_dVg);
          tmp = inst->pParam->One_Third_CoxWL * (T2 * dVth_dVb 
          - T3 * dVgdt_dVb);
          *cdd = inst->pParam->One_Third_CoxWL * (T2 * dVth_dVd 
           - T3 * dVgdt_dVd);
          *cds = -(*cdg + tmp + *cdd);
...
finished:       /* returning Values to Calling Routine */
    ...
    switch (valuetypeflag)
     {
      case 0: *Ids = SPMAX(*Ids,1e-50);
              break;
      case -1: *Ids = SPMAX(*Ids,1e-50);
              break;
      case -2: *Ids = *gm;
              break;
      case -3: *Ids = *gds;
