 ...
 *                                                                        *
 *========================================================================*
 $Id: b3load.cc,v 2.14 2015/06/11 01:12:28 stevew Exp $
 *========================================================================*/
...
int
// SRW BSIM3dev::load(sGENmodel *genmod, sCKT *ckt)
BSIM3dev::load(sGENinstance *in_inst, sCKT *ckt)
{
// SRW    sBSIM3model *model = static_cast<sBSIM3model*>(genmod);
// SRW    sBSIM3instance *here;
    sBSIM3instance *here = (sBSIM3instance*)in_inst;
    sBSIM3model *model = (sBSIM3model*)here->GENmodPtr;
...
*/
            ...
            else
            {
                ...
                {
                    vbs = DEV.pred(ckt, here->BSIM3vbs);
                    vgs = DEV.pred(ckt, here->BSIM3vgs);
                    vds = DEV.pred(ckt, here->BSIM3vds);
                    qdef = DEV.pred(ckt, here->BSIM3qdef);
                ...
                {
                    vgs = DEV.fetlim(vgs, *(ckt->CKTstate0+here->BSIM3vgs), von);
                    vds = vgs - vgd;
                    vds = DEV.limvds(vds, *(ckt->CKTstate0 + here->BSIM3vds));
                    vgd = vgs - vds;
                ...
                if (vds >= 0.0)
                {
                    vbs = DEV.pnjlim(vbs, *(ckt->CKTstate0 + here->BSIM3vbs),
                ...
                {
                    vbd = DEV.pnjlim(vbd, *(ckt->CKTstate0 + here->BSIM3vbd),
                                     CONSTvt0, model->BSIM3vcrit, &Check);
                    vbs = vbd + vds;
                }
            }
...
#define MSGN(x) (model->BSIM3type > 0 ? x : -(x))

            *(ckt->CKTstate0 + here->BSIM3a_cd) = MSGN(here->BSIM3cd);
            *(ckt->CKTstate0 + here->BSIM3a_cbs) = MSGN(here->BSIM3cbs);
            *(ckt->CKTstate0 + here->BSIM3a_cbd) = MSGN(here->BSIM3cbd);
            *(ckt->CKTstate0 + here->BSIM3a_cggb) = MSGN(here->BSIM3cggb);
            *(ckt->CKTstate0 + here->BSIM3a_cgdb) = MSGN(here->BSIM3cgdb);
            *(ckt->CKTstate0 + here->BSIM3a_cgsb) = MSGN(here->BSIM3cgsb);
            *(ckt->CKTstate0 + here->BSIM3a_cdgb) = MSGN(here->BSIM3cdgb);
            *(ckt->CKTstate0 + here->BSIM3a_cddb) = MSGN(here->BSIM3cddb);
            *(ckt->CKTstate0 + here->BSIM3a_cdsb) = MSGN(here->BSIM3cdsb);
            *(ckt->CKTstate0 + here->BSIM3a_cbgb) = MSGN(here->BSIM3cbgb);
            *(ckt->CKTstate0 + here->BSIM3a_cbdb) = MSGN(here->BSIM3cbdb);
            *(ckt->CKTstate0 + here->BSIM3a_cbsb) = MSGN(here->BSIM3cbsb);
            *(ckt->CKTstate0 + here->BSIM3a_von) = MSGN(here->BSIM3von);
            *(ckt->CKTstate0 + here->BSIM3a_vdsat) = MSGN(here->BSIM3vdsat);

            *(ckt->CKTstate0 + here->BSIM3a_gm) = here->BSIM3gm;
            *(ckt->CKTstate0 + here->BSIM3a_gds) = here->BSIM3gds;
            *(ckt->CKTstate0 + here->BSIM3a_gmbs) = here->BSIM3gmbs;
            *(ckt->CKTstate0 + here->BSIM3a_gbd) = here->BSIM3gbd;
            *(ckt->CKTstate0 + here->BSIM3a_gbs) = here->BSIM3gbs;
            *(ckt->CKTstate0 + here->BSIM3a_capbd) = here->BSIM3capbd;
            *(ckt->CKTstate0 + here->BSIM3a_capbs) = here->BSIM3capbs;
// SRW - end

            ckt->rhsadd(here->BSIM3gNode, -ceqqg);
            ckt->rhsadd(here->BSIM3bNode, -(ceqbs + ceqbd + ceqqb));
            ckt->rhsadd(here->BSIM3dNodePrime, (ceqbd - cdreq - ceqqd));
            ckt->rhsadd(here->BSIM3sNodePrime, (cdreq + ceqbs + ceqqg
                    + ceqqb + ceqqd));
            if (here->BSIM3nqsMod)
                ckt->rhsadd(here->BSIM3qNode, cqcheq - cqdef);

// SRW
            if (here->BSIM3adjoint)
            {
                dvaMatrix *mat = here->BSIM3adjoint->matrix;
                mat->clear();

                (*mat->get_elem(here->BSIM3gNode) -= ceqqg);
                (*mat->get_elem(here->BSIM3bNode) -=(ceqbs + ceqbd + ceqqb));
                (*mat->get_elem(here->BSIM3dNodePrime) += (ceqbd - cdreq - ceqqd));
                (*mat->get_elem(here->BSIM3sNodePrime) += (cdreq + ceqbs + ceqqg
                        + ceqqb + ceqqd));
                if (here->BSIM3nqsMod)
                    *mat->get_elem(here->BSIM3qNode) += (cqcheq - cqdef);
            }
// SRW - end
            ...
            T1 = qdef * here->BSIM3gtau;
            ckt->ldadd(here->BSIM3DdPtr, here->BSIM3drainConductance);
            ckt->ldadd(here->BSIM3GgPtr, gcggb - ggtg);
            ckt->ldadd(here->BSIM3SsPtr, here->BSIM3sourceConductance);
            ckt->ldadd(here->BSIM3BbPtr, here->BSIM3gbd + here->BSIM3gbs
                                    - gcbgb - gcbdb - gcbsb - here->BSIM3gbbs);
            ckt->ldadd(here->BSIM3DPdpPtr, here->BSIM3drainConductance
                                      + here->BSIM3gds + here->BSIM3gbd
                                      + RevSum + gcddb + dxpart * ggtd
                                      + T1 * ddxpart_dVd + gbdpdp);
            ckt->ldadd(here->BSIM3SPspPtr, here->BSIM3sourceConductance
                                      + here->BSIM3gds + here->BSIM3gbs
                                      + FwdSum + gcssb + sxpart * ggts
                                      + T1 * dsxpart_dVs + gbspsp);
            ckt->ldadd(here->BSIM3DdpPtr, -here->BSIM3drainConductance);
            ckt->ldadd(here->BSIM3GbPtr, -(gcggb + gcgdb + gcgsb + ggtb));
            ckt->ldadd(here->BSIM3GdpPtr, gcgdb - ggtd);
            ckt->ldadd(here->BSIM3GspPtr, gcgsb - ggts);
            ckt->ldadd(here->BSIM3SspPtr, -here->BSIM3sourceConductance);
            ckt->ldadd(here->BSIM3BgPtr, gcbgb - here->BSIM3gbgs);
            ckt->ldadd(here->BSIM3BdpPtr, gcbdb - here->BSIM3gbd + gbbdp);
            ckt->ldadd(here->BSIM3BspPtr, gcbsb - here->BSIM3gbs + gbbsp);
            ckt->ldadd(here->BSIM3DPdPtr, -here->BSIM3drainConductance);
            ckt->ldadd(here->BSIM3DPgPtr, Gm + gcdgb + dxpart * ggtg
                                     + T1 * ddxpart_dVg + gbdpg);
            ckt->ldadd(here->BSIM3DPbPtr, -(here->BSIM3gbd - Gmbs + gcdgb + gcddb
                                     + gcdsb - dxpart * ggtb
                                     - T1 * ddxpart_dVb - gbdpb));
            ckt->ldadd(here->BSIM3DPspPtr, -(here->BSIM3gds + FwdSum - gcdsb
                                      - dxpart * ggts - T1 * ddxpart_dVs - gbdpsp));
            ckt->ldadd(here->BSIM3SPgPtr, gcsgb - Gm + sxpart * ggtg
                                     + T1 * dsxpart_dVg + gbspg);
            ckt->ldadd(here->BSIM3SPsPtr, -here->BSIM3sourceConductance);
            ckt->ldadd(here->BSIM3SPbPtr, -(here->BSIM3gbs + Gmbs + gcsgb + gcsdb
                                     + gcssb - sxpart * ggtb
                                     - T1 * dsxpart_dVb - gbspb));
            ckt->ldadd(here->BSIM3SPdpPtr, -(here->BSIM3gds + RevSum - gcsdb
                                      - sxpart * ggtd - T1 * dsxpart_dVd - gbspdp));

            if (here->BSIM3nqsMod)
            {
                ckt->ldadd(here->BSIM3QqPtr, (gqdef + here->BSIM3gtau));

                ckt->ldadd(here->BSIM3DPqPtr, (dxpart * here->BSIM3gtau));
                ckt->ldadd(here->BSIM3SPqPtr, (sxpart * here->BSIM3gtau));
                ckt->ldadd(here->BSIM3GqPtr, -here->BSIM3gtau);

                ckt->ldadd(here->BSIM3QgPtr, (ggtg - gcqgb));
                ckt->ldadd(here->BSIM3QdpPtr, (ggtd - gcqdb));
                ckt->ldadd(here->BSIM3QspPtr, (ggts - gcqsb));
                ckt->ldadd(here->BSIM3QbPtr, (ggtb - gcqbb));
            }
...
// SRW
            if (here->BSIM3adjoint)
            {
                ...
                (*(adj->BSIM3DdPtr) += here->BSIM3drainConductance);
                (*(adj->BSIM3GgPtr) += gcggb - ggtg);
                (*(adj->BSIM3SsPtr) += here->BSIM3sourceConductance);
                (*(adj->BSIM3BbPtr) += here->BSIM3gbd + here->BSIM3gbs
                                       - gcbgb - gcbdb - gcbsb - here->BSIM3gbbs);
                (*(adj->BSIM3DPdpPtr) += here->BSIM3drainConductance
                                         + here->BSIM3gds + here->BSIM3gbd
                                         + RevSum + gcddb + dxpart * ggtd
                                         + T1 * ddxpart_dVd + gbdpdp);
                (*(adj->BSIM3SPspPtr) += here->BSIM3sourceConductance
                                         + here->BSIM3gds + here->BSIM3gbs
                                         + FwdSum + gcssb + sxpart * ggts
                                         + T1 * dsxpart_dVs + gbspsp);
                (*(adj->BSIM3DdpPtr) -= here->BSIM3drainConductance);
                (*(adj->BSIM3GbPtr) -= gcggb + gcgdb + gcgsb + ggtb);
                (*(adj->BSIM3GdpPtr) += gcgdb - ggtd);
                (*(adj->BSIM3GspPtr) += gcgsb - ggts);
                (*(adj->BSIM3SspPtr) -= here->BSIM3sourceConductance);
                (*(adj->BSIM3BgPtr) += gcbgb - here->BSIM3gbgs);
                (*(adj->BSIM3BdpPtr) += gcbdb - here->BSIM3gbd + gbbdp);
                (*(adj->BSIM3BspPtr) += gcbsb - here->BSIM3gbs + gbbsp);
                (*(adj->BSIM3DPdPtr) -= here->BSIM3drainConductance);
                (*(adj->BSIM3DPgPtr) += Gm + gcdgb + dxpart * ggtg
                                        + T1 * ddxpart_dVg + gbdpg);
                (*(adj->BSIM3DPbPtr) -= here->BSIM3gbd - Gmbs + gcdgb + gcddb
                                        + gcdsb - dxpart * ggtb
                                        - T1 * ddxpart_dVb - gbdpb);
                (*(adj->BSIM3DPspPtr) -= here->BSIM3gds + FwdSum - gcdsb
                                         - dxpart * ggts - T1 * ddxpart_dVs - gbdpsp);
                (*(adj->BSIM3SPgPtr) += gcsgb - Gm + sxpart * ggtg
                                        + T1 * dsxpart_dVg + gbspg);
                (*(adj->BSIM3SPsPtr) -= here->BSIM3sourceConductance);
                (*(adj->BSIM3SPbPtr) -= here->BSIM3gbs + Gmbs + gcsgb + gcsdb
                                        + gcssb - sxpart * ggtb
                                        - T1 * dsxpart_dVb - gbspb);
                (*(adj->BSIM3SPdpPtr) -= here->BSIM3gds + RevSum - gcsdb
                                         - sxpart * ggtd - T1 * dsxpart_dVd - gbspdp);

                if (here->BSIM3nqsMod)
                {
                    *(adj->BSIM3QqPtr) += (gqdef + here->BSIM3gtau);

                    *(adj->BSIM3DPqPtr) += (dxpart * here->BSIM3gtau);
                    *(adj->BSIM3SPqPtr) += (sxpart * here->BSIM3gtau);
                    *(adj->BSIM3GqPtr) -= here->BSIM3gtau;

                    *(adj->BSIM3QgPtr) += (ggtg - gcqgb);
                    *(adj->BSIM3QdpPtr) += (ggtd - gcqdb);
                    *(adj->BSIM3QspPtr) += (ggts - gcqsb);
                    *(adj->BSIM3QbPtr) += (ggtb - gcqbb);
                }

                *(ckt->CKTstate0 + here->BSIM3a_id) =
                    adj->matrix->compute_real(here->BSIM3dNode, ckt->CKTrhsOld);
                *(ckt->CKTstate0 + here->BSIM3a_is) =
                    adj->matrix->compute_real(here->BSIM3sNode, ckt->CKTrhsOld);
                *(ckt->CKTstate0 + here->BSIM3a_ig) =
                    adj->matrix->compute_real(here->BSIM3gNode, ckt->CKTrhsOld);
                *(ckt->CKTstate0 + here->BSIM3a_ib) =
                    adj->matrix->compute_real(here->BSIM3bNode, ckt->CKTrhsOld);
            }
// SRW - end
