 ...
 *                                                                        *
 *========================================================================*
 $Id: b3temp.cc,v 2.14 2011/12/18 01:15:21 stevew Exp $
 *========================================================================*/
...
#define BSIM3checkModel checkModel
...
int
BSIM3dev::temperature(sGENmodel *genmod, sCKT *ckt)
{
...
//                  Ldrn = here->BSIM3l;
//                  Wdrn = here->BSIM3w;
                Ldrn = here->BSIM3l - 0.5*model->BSIM3xl;
                Wdrn = here->BSIM3w - 0.5*model->BSIM3xw;
                pParam->Length = Ldrn;
                pParam->Width = Wdrn;
                ...
                if (BSIM3checkModel(model, here, ckt))
                {
                    ...
                    /* SRW
                                          (*(SPfrontEnd->IFerror)) (ERR_FATAL,
                                          "Fatal error(s) detected during BSIM3V3.2 parameter checking for %s in model %s",
                                          namarray);
                    */
                    DVO.textOut(OUT_FATAL,
                             "Fatal error(s) detected during BSIM3.2.4 parameter checking for %s in model %s",
                             namarray[0], namarray[1]);
                    return(E_BADPARM);
                }
