 ...
 *                                                                        *
 *========================================================================*
 $Id: b3set.cc,v 1.13 2016/09/26 01:47:35 stevew Exp $
 *========================================================================*/
...
#define TSTALLOC(ptr, first, second) \
    if ((here->ptr = ckt->alloc(here->first, here->second)) == 0) \
        return (E_NOMEM); \
    if (here->BSIM3adjoint) here->BSIM3adjoint->ptr = \
        here->BSIM3adjoint->matrix->get_elem(here->first, here->second);

namespace
{
    int get_node_ptr(sCKT *ckt, sBSIM3instance *here)
    {
...
int
BSIM3dev::setup(sGENmodel *genmod, sCKT *ckt, int *states)
{
...
// SRW - bug here: subsequent setup() calls with tnom given will add
// temperature offset multiple times.  The offset is now applied in
// the askModel/setModel functions
        /*
                else
                ...
                here=here->BSIM3nextInstance)
        {
            ...
            *states += BSIM3numStates;
            ...
            if (!here->BSIM3drainAreaGiven)
                here->BSIM3drainArea = ckt->mos_default_ad();
...
// SRW
            ...
            */
            ...
            if (ckt->CKTcurTask->TSKuseAdjoint && !here->BSIM3adjoint)
                here->BSIM3adjoint = new BSIM3adj;
...
// SRW
#define BSIM3model sBSIM3model
#define BSIM3instance sBSIM3instance

int
BSIM3dev::unsetup(sGENmodel *inModel, sCKT*)
{
#ifndef HAS_BATCHSIM
    BSIM3model *model;
    BSIM3instance *here;
...
//
int
BSIM3dev::resetup(sGENmodel *inModel, sCKT *ckt)
{
    for (sBSIM3model *model = (sBSIM3model*)inModel; model;
            model = model->next())
    {
        for (sBSIM3instance *here = model->inst(); here;
                here = here->next())
        {
