 ...
 *                                                                        *
 *========================================================================*
 $Id: traload.cc,v 2.18 2016/07/27 04:41:35 stevew Exp $
 *========================================================================*/
...
TXLine::update_cnv(double h)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        bo *= t;
        tm->cnv_i =
            (tm->cnv_i - bi*h)*e + (e - 1.0)*(ai*t + bi/tm->x);
        tm->cnv_o =
            (tm->cnv_o - bo*h)*e + (e - 1.0)*(ao*t + bo/tm->x);
    }
}
...
sTRAinstance::ltra_load(sCKT *ckt)
{
    ...
    // set up TRAinputs - to go into the RHS of the circuit
    // equations
