...
//    if ((fplog = fopen("bsim4.out", "w")) != NULL)
    {
        ...
        if (fabs(model->BSIM4version - 4.82) > 0.0001){
            ChkFprintf(fplog, "Warning: This model is BSIM4.8.2; you specified a wrong version number.\n");
            WarnPrintf("Warning: This model is BSIM4.8.2; you specified a wrong version number.\n");
        }
        ChkFprintf(fplog, "Model = %s\n", (const char*)model->BSIM4modName);

// SRW -- modified version checking
//XXX deal with new versioning
/*
...
*/
        ...
        if (model->BSIM4paramChk ==1)
        {
            ...
            {
                ChkFprintf(fplog,
                        "Warning: gidlclamp = %g is zero or positive.\n",
                        model->BSIM4gidlclamp);
                WarnPrintf("Warning: gidlclamp = %g is zero or positive.\n",
                        model->BSIM4gidlclamp);
            }
            ...
            {
                ChkFprintf(fplog, "Fatal: RBPS0 = %g is not positive.\n",
                    model->BSIM4rbps0 );
                FatalPrintf("Fatal: RBPS0 = %g is not positive.\n", model->BSIM4rbps0);
                Fatal_Flag = 1;
            }
            if (model->BSIM4rbpd0 <= 0.0)
            {
                ChkFprintf(fplog, "Fatal: RBPD0 = %g is not positive.\n",
                    model->BSIM4rbpd0 );
                FatalPrintf("Fatal: RBPD0 = %g is not positive.\n", model->BSIM4rbpd0);
                Fatal_Flag = 1;
            }
            if (model->BSIM4rbpbx0 <= 0.0)
            {
                ChkFprintf(fplog, "Fatal: RBPBX0 = %g is not positive.\n",
                    model->BSIM4rbpbx0);
                FatalPrintf("Fatal: RBPBX0 = %g is not positive.\n", model->BSIM4rbpbx0);
                Fatal_Flag = 1;
            }
            if (model->BSIM4rbpby0 <= 0.0)
            {
                ChkFprintf(fplog, "Fatal: RBPBY0 = %g is not positive.\n",
                    model->BSIM4rbpby0);
                FatalPrintf("Fatal: RBPBY0 = %g is not positive.\n", model->BSIM4rbpby0);
                Fatal_Flag = 1;
            }
            if (model->BSIM4rbdbx0 <= 0.0)
            {
                ChkFprintf(fplog, "Fatal: RBDBX0 = %g is not positive.\n",
                    model->BSIM4rbdbx0);
                FatalPrintf("Fatal: RBDBX0 = %g is not positive.\n", model->BSIM4rbdbx0);
                Fatal_Flag = 1;
            }
            if (model->BSIM4rbdby0 <= 0.0)
            {
                ChkFprintf(fplog, "Fatal: RBDBY0 = %g is not positive.\n",
                    model->BSIM4rbdby0);
                FatalPrintf("Fatal: RBDBY0 = %g is not positive.\n", model->BSIM4rbdby0);
                Fatal_Flag = 1;
            }
            if (model->BSIM4rbsbx0 <= 0.0)
            {
                ChkFprintf(fplog, "Fatal: RBSBX0 = %g is not positive.\n",
                    model->BSIM4rbsbx0);
                FatalPrintf("Fatal: RBSBX0 = %g is not positive.\n", model->BSIM4rbsbx0);
                Fatal_Flag = 1;
            }
            if (model->BSIM4rbsby0 <= 0.0)
            {
                ChkFprintf(fplog, "Fatal: RBSBY0 = %g is not positive.\n",
                    model->BSIM4rbsby0);
                FatalPrintf("Fatal: RBSBY0 = %g is not positive.\n", model->BSIM4rbsby0);
                Fatal_Flag = 1;
            }
            ...
            if (model->BSIM4tnoiMod == 1){
                WarnPrintf("Warning: TNOIMOD=1 is not supported and may be removed from future version.\n", model->BSIM4tnoiMod);
            }
	    if (model->BSIM4idovvdsc <= 0.0)
            {
                ChkFprintf(fplog, "Warning: idovvdsc = %g is zero or negative.\n", model->BSIM4idovvdsc);
                WarnPrintf("Warning: idovvdsc = %g is zero or negative.\n", model->BSIM4idovvdsc);
	    }
