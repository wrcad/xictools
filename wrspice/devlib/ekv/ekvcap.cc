 ...
 *                                                                        *
 *========================================================================*
 $Id: ekvcap.cc,v 2.10 2008/09/22 02:47:07 stevew Exp $
 *========================================================================*/
...
#else /* stdc */
extern void F_v_simple();
extern void F_v_n();

#endif /* stdc */

void EKVcap(sEKVmodel *model, sEKVinstance *here, double vds, double vbs,
    double *cgs, double *cgd, double *cgb)
/*
void EKVcap(model,here,vds,vbs,cgs,cgd,cgb)
...
//	double dQS_n_factor, dQB_VP_factor;
//	double dQD_n_factor, dir_factor;
	double dVP_dVD, dVP_dVS, dVP_dVG;
	double dif_dVD, dif_dVS, dif_dVG;
//	double d_QS_ir, d_QS_if;
//	double d_QD_if, d_QD_ir;
//	double cgb_ovl, cgs_ovl, cgd_ovl;
...
// SRW
// This appears to be wrong.  From the documentation page 14 (sect 6.2.1)
// Cxy = -dQx/dVy when x != y.  As is, circuits using EKV devices would
// not converge in transient analysis (mosamp2, comparator, oneshot,
// simple cmos inverter).  The cgs/cgd values were negative and in many
// cases fairly large (~1pf).  With the sign change below, there is no
// convergence problems with these circuits and results seem normal.
//
//	*cgs = dQG_dVS;
//	*cgd = dQG_dVD;
