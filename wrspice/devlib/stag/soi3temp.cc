 ...
 *                                                                        *
 *========================================================================*
 $Id: soi3temp.cc,v 2.13 2011/12/18 01:16:12 stevew Exp $
 *========================================================================*/
...
int
SOI3dev::temperature(sGENmodel *genmod, sCKT *ckt)
{
   ...
   are given. */
    ...
    for( ; model != NULL; model = model->SOI3nextModel)
    {
            ...
            model->SOI3bodyThickness == 0)
        {
				...
				/* JimB - this part gives an access violation error when linking STAG21 DLL */
				/* to the Simetrix code, so replace with printf statement */
        		printf(" SOI3 device film thickness must be supplied\n");
 ...
 * if we have a thick film device.  If not, complain ! */
                    ...
                    if(model->SOI3bodyThickness < xd_max)
                    {
						  		...
						  		/* JimB - this part gives an access violation error when linking STAG21 DLL */
                    		/* to the Simetrix code, so replace with printf statement */
                    		printf(" Body Film thickness may be too small for this model \n");
#else /* SIMETRIX_VERSION */
/* SRW
...
*/
            ...
            else /* NSUB not given, have to assume that VT0, PHI and GAMMA are given */
            {
               ...
               if(model->SOI3bodyThickness < xd_max)
               {
					   ...
					   /* JimB - this part gives an access violation error when linking STAG21 DLL */
                  /* to the Simetrix code, so replace with printf statement */
                  printf(" Body Film thickness may be too small for this model to be valid\n");
#else /* SIMETRIX_VERSION */
/* SRW
					   (*(SPfrontEnd->IFerror))(ERR_WARNING,
                  "%s :Body Film thickness may be too small \nfor this model to be valid",
...
/* End of thick film check - msll 21/2/94
   Changed to only give warning  - msll 31/10/95  */
                ...
                here = here->SOI3nextInstance)
        {
            ...
            if(!here->SOI3lGiven)
            {
               here->SOI3l = ckt->mos_default_l();
            }
            if(!here->SOI3wGiven)
            {
               here->SOI3w = ckt->mos_default_w();
            }
            ...
            if(here->SOI3l - 2 * model->SOI3latDiff <=0)
            {
					...
					/* JimB - this part gives an access violation error when linking STAG21 DLL */
					/* to the Simetrix code, so replace with printf statement */
               printf(" Effective channel length less than zero \nIncreasing \
