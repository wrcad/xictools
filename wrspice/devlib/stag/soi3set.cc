 ...
 *                                                                        *
 *========================================================================*
 $Id: soi3set.cc,v 2.15 2016/09/26 01:48:27 stevew Exp $
 *========================================================================*/
...
#define SOI3name GENname
#define SOI3modName GENmodName
#define CKTmkVolt(a, b, c, d) (a)->mkVolt(b, c, d)
#define CKTmkCur(a, b, c, d) (a)->mkCur(b, c, d)
...
namespace {
    int get_node_ptr(sCKT *ckt, sSOI3instance *inst)
    {
...
int
SOI3dev::setup(sGENmodel *genmod, sCKT *ckt, int *states)
{
...
/* now check to determine which CLM model to use */
        if((model->SOI3lx != 0) && (model->SOI3lambda != 0)) {
        		...
        		/* JimB - ERR_WARNING not defined in */
            printf(" Non-zero values for BOTH LAMBDA and LX. \nDefaulting to\
        		simple LAMBDA model");
...
*/
            ...
            {
#ifdef SIMETRIX_VERSION
            	 error = CKTmkVolt(ckt,&tmp,here->SOI3name,"int_drain");
#else /* SIMETRIX_VERSION */
...
#ifdef SIMETRIX_VERSION
                /* NTL modification 27.4.98 - add && error!=E_EXISTS. (Could be created by nodeset) */
            ...
            {
#ifdef SIMETRIX_VERSION
            	 error = CKTmkVolt(ckt,&tmp,here->SOI3name,"int_source");
#else /* SIMETRIX_VERSION */
...
// SRW
#undef inst
#define SOI3model sSOI3model
#define SOI3instance sSOI3instance

int
SOI3dev::unsetup(sGENmodel *inModel, sCKT*)
{
#ifndef HAS_BATCHSIM
    SOI3model *model;
    SOI3instance *here;
...
//
int
SOI3dev::resetup(sGENmodel *inModel, sCKT *ckt)
{
    for (sSOI3model *model = (sSOI3model*)inModel; model;
            model = model->next()) {
        for (sSOI3instance *here = model->inst(); here;
                here = here->next()) {
