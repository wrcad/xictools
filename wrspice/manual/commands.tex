...
% spCommands.hlp:commands 022519
...
\item{The {\it resultname} of another measure.  The event occurs when
the referenced measurement is performed.}
...
script file or codeblock.  The script can be used for additional
processing or testing of whatever sort.  If the script returns 1, the
current simulation will pause immediately (no waiting for other
measures) however a calling analysis, such as Monto Carlo, will
continue.  If 2 is returned, this indicates a fatal global error and
any calling analysis will be stopped too.  Any other return value
allows the run to continue normally.
\end{description}
