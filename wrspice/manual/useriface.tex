...
% spSetup.hlp:startup_files 101010
...
\index{.wrpasswd file}
\index{updating WRspice}
\index{password file}
\item{{\vt .wrpasswd} file}\\
On startup, {\WRspice} will read the {\vt .wrpasswd} file if found in
...
with the {\cb passwd} command.  This file can also be generated from
{\Xic}.  If present and the password is active, {\WRspice} will check
for the availability of updates on startup, and the {\cb wrupdate}
...
\index{initial operations}

% spSetup.hlp:startup_ops 092815

When {\WRspice} starts, the following operations are generally
performed.
...
wrupdate} command.  The {\cb passwd} command is used to enable this
...
\begin{description}
\item{\vt wrspice\_current\_mesg}\\
The is the current message text.
\item{\vt wrspice\_current\_mesg\_digest}\\
...
\item{If a new {\WRspice} release is run for the first time, the
...
% spToolbar.hlp:tracepanel 012709
%SU-------------------------------------
...
% spExpressions.hlp:plotvec 121215
...
% spExpressions.hlp:constants 040110
...
% spExpressions.hlp:mathfuncs 031416
