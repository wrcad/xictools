...
\begin{quote}
\begin{longtable}{|l|c|l|l|l|}\hline
...
dependence (see \ref{rsjtcf}) of the energy gap, default is 1.74.
...
\subsubsection{RSJ Model Temperature Dependence}
\label{rsjtcf}

When the junction is not operating at the nominal temperature as
specified with the {\vt tnom} parameter, a correction factor is
...
If the gap voltage is not specified with the {\vt vg} parameter or its
alias {\vt vgap}, the model will compute the gap voltage from BCS
theory using the superconducting transition temperature ({\vt tc}
parameter) and Debye temperature ({\vt tdebye} parameter) at the
...
ratio $\Delta/\Delta_{nom}$.  This will apply when the nominal gap
...
The temperature correction factor {\it tcf} multiplies the instance
critical current to account for the temperature difference between
operating and nominal temperatures.

\begin{quote}
$tcf = (Vg/Vg_{nom})(tanh(eVg/4KT)/tanh(eVg_{nom}/4KT_{nom}))$
\end{quote}

Here, $Vg$ is the computed gap voltage, $e$ is the electron charge,
$K$ is Boltzmann's constant, $T$ is temperature in Kelvin,


\subsubsection{Josephson Tunnel Junction Model}
\index{TJM}
\index{Werthamer model}
\label{tjmodel}

%spModels:tjmodel 051422
...
produced from the {\vt mmjco} utility provided with {\WRspice} (see
\ref{mmjco}).  Single temperature ``.fit'' files will overrule any
...
for along a path provided by setting the {\et tjm\_path} variable, or
a path can be provided directly.  The default is to search the current
directory and {\vt \$HOME/.mmjco} if that directory exists.
...
with {\vt mmjco} which would provide precomputed data for all
...
``tjm2''.  These are the MitMoJCo {\vt NbNb\_4k2\_008} and {\vt
NbNb\_4K2\_001} parameter sets, respectively.  Both assume niobium at
temperature 4.2K and differ in the level of smoothing applied to
mitigate the Riedel singularity.
</dl>

\item{\vt rtype}\\
For the tunnel junction model, {\vt rtype} is a flag, set by default,
that enables inclusion of the quasiparticle current.  If set to 0,
quasiparticle current will not be included in the model.

\item{\vt cct}\\
For the tunnel junction model, {\vt cct} is a flag, set by default,
...
\subsubsection{TJM Model Temperature Dependence}
\label{tjmtcf}
