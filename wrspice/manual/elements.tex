...
If any of the parameters {\it td1, td2, ...\/} appear, the period is
...
The {\it pw} defines the pulse width, as evident in the expression. 
If the {\it per} is given a nonzero value larger than twice the {\it
pw}, a train of pulses will be generated, the first being at {\it td}
and at time increments of {\it per} thereafter.  A {\it per}
...
If the amplitude is set to zero, i.e., {\it v2} = {\it v1\/}, the
...
will induce a single flux quantum of $\Phi_0$ = $h/(2{\cdot}\pi)$
= 2.06783fWb ({\it h} is Planck's constant, {\it e} is the electron
