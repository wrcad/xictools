...
{\vt phsf} & True if SFQ count change at current time point (read only).\\
 \hline
...
The following are additional parameters that are used when Level=3 only.
\begin{quote}
\begin{tabular}{|l|c|l|l|l|}\hline
\multicolumn{5}{|c|}{\bf JJ Model Parameters}\\ \hline
\bf name & \bf area & \bf parameter & \bf units & \bf default\\ \hline\hline
{\vt del1} & & \rr Energy gap side 1 & $V$ & 1.3e-3\\
\hline
{\vt del2} & & \rr Energy gap side 2 & $V$ & 1.3e-3\\
\hline
{\vt tc1} & & \rr Transition temperature side 1 & $K$ & 9.26\\
\hline
{\vt tc2} & & \rr Transition temperature side 2 & $K$ & 9.26\\
\hline
...
\hline
{\vt smf} & & \rr Smoothing factor & $-$ & 0.008\\
\hline
{\vt nterms} & & \rr Number of terms in compression & $-$ &8\\
\hline
{\vt nxpts} & & \rr Number of TCA table entries & $-$ &500\\
\hline
{\vt thr} & & \rr Fitting algorithn threshold & $-$ &0,2\\
\hline
\end{tabular}
\end{quote}
...
produced from the {\it mmjco} utility provided with {\WRspice}, and
can also be produced with the Python program supplied with MitMoJCo. 
The files are searched for along a path provided by setting the {\et
tjm\_path} variable, or a path can be provided directly.  The default
is to search the current directory and {\vt \$HOME/.mmjco} if that
directory exists.

These files are produced automatically as needed according to the
given model parameters and cached in the users {\vt .mmjco} directory. 
So it is not common to use this parameter to load a set by name.
...
resistance to infinity.  Conditions with {\vt rtype=1} and {\vt 2} are
...
1, only 0 and 1 are supported in levels 2 and 3.
\begin{quote}
\begin{tabular}{ll}
...
\item{\vt vm}\\
{\bf range:} 8mV -- 100mV, can be 0 if Level=3\\
...
subtracted if smaller than the given {\vt vm} implies.  If {\vt vm} is
set to 0, then no additional conductivity will be added and only the
...
{\bf range:} 8mV/{\vt icrit} -- 100mV/{\vt icrit}, can be 0 if Level=3\\
...
For the Level 3 model, the subgap conductance will be reduced by the
intrinsic condutance if this is smaller.  If {\vt vm} is given as 0
and this parameter is not given, the parameter value will be 0.  If
the value is 0, no additional conductance will be added.
...
{\vt temo\_k} instance parameter.  The default is the {\vt tnom}
value.
