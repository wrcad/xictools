...
\index{environment!XT\_NO\_CHECK\_UPDATE}
\index{XT\_NO\_CHECK\_UPDATE environment variable}
\item{\et XT\_NO\_CHECH\_UPDATE}\\
If set in the environment on program startup, the program will skip
attempting to communicate with the site hosting the distribution
repository to check for messages and updates.

\index{environment!XT\_UPD\_DEBUG}
\index{XT\_UPD\_DEBUG environment variable}
\item{\et XT\_UPD\_DEBUG}\\
When checking for updates with the site hosting the distribution
repository and an error occurs, if this variable is set then a message
will be emitted.  Otherwise, failure is silent.
\end{description}
