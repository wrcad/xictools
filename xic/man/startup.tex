...
All Generation 4 OS X distributions require OS X 10.9 (Mavericks) or
later.  Unfortunately, it is not possible to support earlier OS X
releases at this time.  However, the upgrade to 10.9 is free and was
accomplished smoothly over the internet at Whiteley Research.
...
OS X releases, it is not installed automatically in Mavericks.  The
...
The package distribution files are ``flat'' Apple package files.  This
is a different format from the release files for Generation 3, which
were tarred/gzipped Apple package directories.  As an alternative to
using {\vt wr\_install}, one can use Apple's installer (click on the
listing icon in the Finder).

The installation procedure is pretty much the same as under
Unix/Linux.  As under Unix/Linux (but not Windows) the {\Xic} and
{\WRspice} programs can make use of a separate license server, and all
of the workgroup configuration options apply.  Similarly, due to the
X-Windows support, graphical operation from remote OS X systems is
possible, unlike under Windows.
...
is needed.  The {\vt sudo} command can be used to start the license
server, if running the server is necessary.

\subsection{Un-Installation}

The OS X package system does not provide an uninstall function, and
overall is quite a bit less capable than the package management
systems in supported Unix/Linux systems.  In release 4.0.7 and later,
when a new release is installed, the preinstall script will explicitly
clear all of the files of an existing package if found, as is done by
the installer on other systems.  In earlier releases, a new release
would simply overwrite the existing release.

To uninstall, the files must be removed manually.  This is easy, as
all files are installed under {\vt /usr/local/xictools}.  One should
also clear the package ``receipt'' from the OS X database with the
terminal command
\begin{quote}
{\vt pkgutil --forget} {\it packagename}
\end{quote}
if the program is being removed permanently.  The {\it packagename} is
the name of the package as known to Apple, which is in a form like
``{\vt XicTools.Xic.4}''.  All of the Whiteley Research installed
packages can be listed by name with the terminal command
\begin{quote} \vt
...
``home'' directory.  The procedure is discussed below.  Then, the {\cb
!passwd} command in {\Xic} or the {\cb password} command in {\WRspice}
can be run, and the user/password to the distribution repository
entered.  This will produce a file ``{\vt .wrpasswd}'' in the home
directory, used to authenticate with the web server at {\vt
wrcad.com}.  When this is set up, the programs will check for updates
on startup.  If an update is available, the user is asked (separately)
whether to download the update distribution file, and whether to
install it.  The actual installation will start when the user exits
the program.
...
WARNING\\
The programs use an entry in the Windows Registry
to find their startup files, license, etc.  This entry is created by
the installer program.  Thus
\begin{enumerate}
...
IMPORTANT!\\
The installation prefix given to the installer for all programs, and
the gtk2-bundle, {\bf must} be the same.
...
Note that if you know your way around Windows {\vt bat} scripts, you
can avoid the installation location requirement in the warning above.
...
\item{The {\XicTools} programs can use a separate license server
daemon under Unix/Linux.  Under Windows, no license server is
used, as the authentication is built into the programs.}
\end{itemize}
...
license files, etc.  Enter the full path to this directory.  Check
spelling, Click {\cb OK}.}
\end{enumerate}
...
\begin{description}
\item{``Undocumented'' options:  {\vt --v}, {\vt --vv}, and {\vt --vb}}\\
...
are two hyphens in these options.  For {\vt --v} {\Xic} will print the
version followed by the distribution name, for example ``{\vt 3.2.5
LinuxRHEL5}''.  For {\vt --vv}, {\Xic} will print the CVS release tag,
for example ``{\vt xic-3-2-5}''.  For {\vt --vb}, {\Xic} will print
...
{\XicII}.  This enables those with a full {\Xic} license to run as
{\XicII}, for whatever reason.  One can create an ``{\vt xicii}''
...
\item{\et FORCE\_XIV}\\
If this variable is set when {\Xic} starts, the program will run as
{\Xiv}.  This enables those with a full {\Xic} license to run as
{\Xiv}, for whatever reason.  One can create an ``{\vt xiv}'' program
...
prepended to the {\vt LD\_LIBRARY\_PATH} in the {\Xic} process.  This
can be used to point to installed libraries needed for plugins,
\ifoa
for example the OpenAccess libraries,
\fi
without having to set {\vt LD\_LIBRARY\_PATH} in the environment. 
This is basically equivalent to setting {\vt LD\_LIBRARY\_PATH} in a
wrapper script, but avoids the need for the script.
...
is required to exist.  In addition, there are a few more files used to
maintain state for certain startup operations, described in the next
section.
...
\index{.wrpasswd file}
\item{{\vt .wrpasswd} file}
The {\vt .wrpasswd} file is created in the user's home directory with
the {\cb !passwd} command.  It is an encrypted file that contains the
...
to use the {\cb !update} command.  The user will also be alerted if
...
When {\Xic} starts, the following operations are generally
performed.

\begin{enumerate}
\item{If configured, the program will check for the availability of
software updates.  A pop-up message will notify the user if an update
is available.  These can be downloaded and installed with the {\cb
!update} command.  The {\cb !passwd} command is used to enable this
feature, The username and password to the Whiteley Research software
repository should be given, and the command will create a {\vt
.wrpasswd} file in the user's home directory.  This will enable update
checking in both {\Xic} and {\WRspice}.}

\item{The program will check the {\vt wrcad.com} web site for a
message file.  If a file is found, and the user has not seen it, the
message text will be shown in a pop-up window.  This is a means of
providing users with bug reports or other information which could be
important.

The record of the message is saved in the {\vt .wr\_cache}
directory in the user's home directory.  There are two files:
\begin{description}
\item{\vt xic\_current\_mesg}\\
The is the current message text.
\item{\vt xic\_current\_mesg\_digest}\\
This is a checksum of the current message.
\end{description}

The program will compare the checksum of the message to the checksum
on {\vt wrcad.com}.  If a message is found at {\vt wrcad.com}, and
there is no checksum or a different checksum in the user's home
directory, the message is downloaded and shown, and the user's files
are updated.  Thus, the user will only see the message once, but can
find a copy of the message in their {\vt .wr\_cache} directory.  Note
that there is only one message file, future messages overwrite the
files.}

\item{If a new {\Xic} release is run for the first time, the release
notes will appear in a pop-up window, as if the {\cb Notes} button in
the {\cb Help} menu was pressed.  There is a file in the user's {\vt
.wr\_cache} directory named {\vt xic\_current\_release} that contains
a release number.  If, when {\Xic} starts, this file is missing or the
...
If the {\et NoCheckUpdate} variable is set (from an initialization
file) then there will be no communications with {\vt wrcad.com}, so
the checking for updates and a message will be skipped.  This should
be set if the user doesn't have internet access, for example, to avoid
the delays caused by lack of access and subsequent connection timeout.
