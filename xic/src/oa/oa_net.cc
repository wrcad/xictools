...
bool
cOA::setupName(oaDesign *design, CDs *sdesc, bool symbolic)
{
...
//XXX rid nh_xxx
        ...
        // The device lacks an instNamePrefix property, otherwise the
        // name property would already by set from readProperties.  If
        // we're reading a schematic, then the instNamePrefix might be
        // found in the symbol, which we haven't seen yet.  Check this
        // now.
        if (!symbolic) {
            oaScalarName symViewName(oaNativeNS(), "symbol");
            oaDesign *symd = oaDesign::open(lib, cellName, symViewName);
            if (symd) {
                oaProp *prp = oaProp::find(symd, "instNamePrefix");
                if (prp) {
                    ...
                    if (value.isEqual("pin", true)) {
                        ...
                        lstr.add(" 0");
                        sdesc->prptyAdd(P_NAME, lstr.string();
                    }
                    else {
    ...
    int *px, int *py)
{
    ...
    else if (sh_type == oacEllipseType) {
        oaEllipse *ellipse = (oaEllipse*)shape;
        ellipse->getBBox(bb);
    }
