 ...
 *                                                                        *
 *========================================================================*
 $Id: miscgeom.cc,v 1.73 2017/03/19 01:58:37 stevew Exp $
 *========================================================================*/
...
namespace {
    ...
    int thread_proc(sTPthreadData*, void *arg)
    {
        ...
        if (gv->mode == CLsplitH) {
            ...
            // default
            ret = zret->to_poly_add(gv->sdesc, gv->ldesc, gv->ud, 0,
                gv->tmp_merge);
        }
    ...
    if (nth > 0) {
        ...
        while ((gBB = grd.advance()) != 0) {
            ...
            if (mode == CLsplitH) {
                ...
                // default
                ret = zret->to_poly_add(sdesc, ld, ud, 0, tmp_merge);
                if (ret != XIok)
                    return (ret);
            }
        }
    ...
    if (sld != ld || import || mode != CLdefault) {
        ...
        if (zimport) {
            if (mode == CLsplitV) {
                ...
                XIrt ret =
                    zimport->to_poly_add(sdesc, ld, ud, 0, use_merge);
                if (ret != XIok)
                    return (ret);
            }
            else {
            ...
            while ((odesc = gen.next(false, false)) != 0) {
                if (mode == CLsplitH) {
                    ...
                    XIrt ret = zl0->to_poly_add(sdesc, ld, ud, 0,
                        use_merge);
                    if (ret != XIok) {
                        delete odesc;
                        return (ret);
            ...
            if (mode == CLjoin && zcnt) {
                XIrt ret = zl0->to_poly_add(sdesc, ld, ud, 0, use_merge);
                if (ret != XIok)
                    return (ret);
            }
        }
