...
cSced::dumpSpiceFile(const char *fnamein)
{
    ...
    char tbuf[256];
    if (!filename) {
        snprintf(tbuf, sizeof(tbuf), "%s", Tstring(DSP()->CurCellName()));
...
SpOut::makeSpiceDeck(SymTab **sctab, bool add_top_sc)
{
    ...
    snprintf(buf, sizeof(buf), "* Generated by %s from cell %s",
        XM()->Product(), Tstring(sp_celldesc->cellname()));
        ...
        if (list_all || (check_terms(sdesc, odesc1, tnames, tsize) &&
            ...
            if (pdesc->value() == P_NEWMUT) {
                if (((CDp_nmut*)pdesc)->assigned_name()) {
                    ...
                    snprintf(tbuf, sizeof(tbuf), "%s%d %s %s %s", MUT_CODE,
                        refcnt, name1, name2, ((CDp_nmut*)pdesc)->coeff_str());
                }
            }
            else {
                snprintf(tbuf, sizeof(tbuf), "%s%d %s %s %g", MUT_CODE,
                    refcnt, name1, name2, ((CDp_mut*)pdesc)->coeff());
            }
            if (d0 == 0)
