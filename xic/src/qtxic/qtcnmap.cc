...
#define CHECK_BOX_STATE_CHANGED &QCheckBox::stateChanged
    ...
    connect(cn_prefix, &QLineEdit::textChanged,
        this, &QTcnameMap::prefix_changed_slot);
    connect(cn_suffix, &QLineEdit::textChanged,
        this, &QTcnameMap::suffix_changed_slot);
}
...
void
QTcnameMap::prefix_changed_slot(const QString &str)
{
...
void
QTcnameMap::suffix_changed_slot(const QString &str)
{
    if (str.isNull() || str.isEmpty()) {
        if (cn_output)
...
void
QTcnameMap::to_lower_slot(int state)
{
    if (cn_output) {
        if (state)
...
void
QTcnameMap::to_upper_slot(int state)
{
    if (cn_output) {
        if (state)
...
void
QTcnameMap::rd_alias_slot(int state)
{
    if (cn_output) {
        bool rd = state;
...
void
QTcnameMap::wr_alias_slot(int state)
{
    if (cn_output) {
