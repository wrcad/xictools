...
cConvert::PopUpCompare(GRobject caller, ShowMode mode)
{
    ...
    if (mode == MODE_OFF) {
        if (cCompare::self())
            cCompare::self()->deleteLater();
        return;
    }
    if (mode == MODE_UPD) {
        if (cCompare::self())
            cCompare::self()->update();
        return;
    }
    if (cCompare::self())
        return;
    ...
    QTdev::self()->SetPopupLocation(GRloc(LW_UR), cCompare::self(),
        QTmainwin::self()->Viewport());
    cCompare::self()->show();
}


cCompare::cCompare(GRobject c)
{
    instPtr = this;
    cmp_caller = c;
    cmp_popup = 0;
...
// Static function.
void
cCompare::cmp_cancel_proc(GtkWidget*, void*)
{
...
// Static function.
void
cCompare::cmp_action(GtkWidget *caller, void*)
{
    if (!Cmp)
        return;
...
// Static function.
void
cCompare::cmp_p1_action(GtkWidget *caller, void *arg)
{
    if (Cmp) {
        if (arg == (void*)1L) {
            if (GTKdev::GetStatus(caller))
...
// Static function.
void
cCompare::cmp_p1_fltr_proc(GtkWidget *caller, void*)
{
    if (Cmp) {
...
//
int
cCompare::cmp_popup_btn_proc(GtkWidget *widget, GdkEvent *event, void *arg)
{
...
// Static function.
void
cCompare::cmp_sto_menu_proc(GtkWidget *widget, void *arg)
{
    cCompare *cmp = (cCompare*)arg;
...
// Static function.
void
cCompare::cmp_rcl_menu_proc(GtkWidget *widget, void *arg)
{
    cCompare *cmp = (cCompare*)arg;
    const char *name = gtk_widget_get_name(widget);
    if (name && cmp) {
...
//
void
cCompare::cmp_drag_data_received(GtkWidget *entry,
