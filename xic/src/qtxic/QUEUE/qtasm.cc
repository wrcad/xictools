...
//
/*
GtkTargetEntry cAsm::target_table[] =
{
    { (char*)"TWOSTRING",   0, 0 },
    { (char*)"CELLNAME",    0, 1 },
    { (char*)"STRING",      0, 2 },
...
guint cAsm::n_targets =
    sizeof(cAsm::target_table)/sizeof(cAsm::target_table[0]);
*/

const char *cAsm::path_to_source_string =
    "Path to Source:  Layout File, CHD File, or CHD Name";
const char *cAsm::path_to_new_string = "Path to New Layout File";
int cAsm::asm_fmt_type = cConvert::cvGds;


cAsm::cAsm(GRobject c)
{
    Asm = this;
    asm_caller = c;
    asm_filesel_btn = 0;
...
#define SRC_SIZE 20
    asm_sources = new cAsmPage*[SRC_SIZE];
...
namespace {
    // Error checking for cAsm::dump_file.
    //
    bool
...
//
bool
cAsm::dump_file(FILE *fp, bool check)
{
...
void
cAsm::notebook_append()
{
    char buf[32];
    if (asm_pages+1 >= asm_srcsize) {
        cAsmPage **tmp = new cAsmPage*[asm_srcsize + asm_srcsize];
        unsigned int i;
    ...
    cAsmPage *src = new cAsmPage(this);
...
void
cAsm::notebook_remove(int index)
{
    char buf[32];
    if (index == 0 && asm_pages <= 1)
        // never delete first entry
        return;
    cAsmPage *src = asm_sources[index];
...
//
void
cAsm::store_tx_params()
{
    int ix = current_page_index();
    if (ix < 0)
        return;
    cAsmPage *src = asm_sources[ix];
...
//
void
cAsm::show_tx_params(unsigned int n)
{
    int ix = current_page_index();
    if (ix < 0)
        return;
...
// Static function.
void
cAsm::set_status_message(const char *msg)
{
    if (Asm) {
        if (Asm->asm_timer_id)
...
//
void
cAsm::pop_up_monitor(int mode, const char *msg, ASMcode code)
{
    if (mode == MODE_OFF) {
        delete AsmPrg;
        AsmPrg = 0;
...
//
void
cAsm::asm_drag_data_received(GtkWidget *entry, GdkDragContext *context,
...
// Static function.
void
cAsm::asm_cancel_proc(GtkWidget*, void*)
{
    delete Asm;
}
...
//
void
cAsm::asm_go_proc(GtkWidget*, void*)
{
...
// Static function.
void
cAsm::asm_page_change_proc(GtkWidget*, void*, int page, void*)
{
    if (!Asm)
        return;
    ...
    if (page > 0) {
        cAsmPage *src = Asm->asm_sources[page-1];
        int n = src->pg_curtlcell;
...
//
void
cAsm::asm_action_proc(GtkWidget *caller, void*)
{
    ...
    if (code == NoCode) {
        ...
        cAsmPage *src = Asm->asm_sources[ix];
...
//
void
cAsm::asm_save_cb(const char *fname, void*)
{
    if (!Asm)
        return;
    if (fname && *fname) {
...
//
void
cAsm::asm_recall_cb(const char *fname, void*)
{
    if (!Asm)
        return;
    if (fname && *fname) {
...
//
void
cAsm::asm_fsel_open(const char *path, void*)
{
    if (!Asm)
        return;
    if (path && *path) {
...
//
void
cAsm::asm_fsel_cancel(GRfilePopup*, void*)
{
    if (!Asm)
        return;
    Asm->asm_fsel = 0;
...
//
void
cAsm::asm_tladd_cb(const char *cname, void*)
{
    if (!Asm)
        return;
    if (cname) {
...
namespace {
    ...
    ifInfoMessage(INFOmsgType code, const char *string, va_list args)
    {
        ...
        if (code == IFMSG_INFO)
            cAsm::pop_up_monitor(MODE_UPD, buf, ASM_INFO);
        else if (code == IFMSG_RD_PGRS)
            cAsm::pop_up_monitor(MODE_UPD, buf, ASM_READ);
        else if (code == IFMSG_WR_PGRS)
            cAsm::pop_up_monitor(MODE_UPD, buf, ASM_WRITE);
        else if (code == IFMSG_CNAME)
            cAsm::pop_up_monitor(MODE_UPD, buf, ASM_CNAME);
...
// Static function.
void
cAsm::asm_setup_monitor(bool active)
{
...
//
bool
cAsm::asm_do_run(const char *fname)
{
