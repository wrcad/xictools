...
cMain::PopUpAttributes(GRobject caller, ShowMode mode)
{
    ...
    new cAttr(caller);
    QTdev::self()->SetPopupLocation(GRloc(), cAttr::self(),
        QTmainwin::self()->Viewport());
    cAttr::self()->show();
}


cAttr::cAttr(GRobject c)
{
    instPtr = this;
    at_caller = c;
    at_minst = 0;
...
// Static function.
void
cAttr::at_cancel_proc(GtkWidget*, void*)
{
...
// Static function.
void
cAttr::at_action(GtkWidget *caller, void*)
{
    if (!Attr)
        return;
...
// Static function.
void
cAttr::at_curs_menu_proc(GtkWidget *caller, void*)
{
...
// Static function.
void
cAttr::at_menuproc(GtkWidget *caller, void*)
{
    if (!Attr)
        return;
    int i = gtk_combo_box_get_active(GTK_COMBO_BOX(caller));
...
// Static function.
void
cAttr::at_val_changed(GtkWidget *caller, void*)
{
    if (!Attr)
        return;
    const char *name = gtk_widget_get_name(caller);
...
// Static function.
void
cAttr::at_ebt_proc(GtkWidget *caller, void*)
{
    int i = gtk_combo_box_get_active(GTK_COMBO_BOX(caller));
    if (i < 0)
        return;
