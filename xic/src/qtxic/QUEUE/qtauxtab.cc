...
cConvert::PopUpAuxTab(GRobject caller, ShowMode mode)
{
    ...
    QTdev::self()->SetPopupLocation(GRloc(), cAuxTab::self(),
        QTmainwin::self()->Viewport());
    cAuxTab::self()->show();
}


cAuxTab *cAuxTab::instPtr;

cAuxTab::cAuxTab(GRobject c)
{
    instPtr = this;
    at_caller = c;
    at_addbtn = 0;
...
void
cAuxTab::action_hdlr(GtkWidget *caller, void*)
{
...
int
cAuxTab::button_hdlr(bool up, GtkWidget *caller, GdkEvent *event)
{
    if (up) {
        at_dragging = false;
        return (false);
...
int
cAuxTab::motion_hdlr(GtkWidget *widget, GdkEvent *event)
{
    if (at_dragging) {
...
//
void
cAuxTab::select_range(int start, int end)
{
    GtkTextBuffer *textbuf =
        gtk_text_view_get_buffer(GTK_TEXT_VIEW(wb_textarea));
...
//
void
cAuxTab::at_cancel(GtkWidget*, void*)
{
...
//
void
cAuxTab::at_action_proc(GtkWidget *caller, void *client_data)
{
    if (AT)
...
//
ESret
cAuxTab::at_add_cb(const char *name, void*)
{
    if (AT && name) {
        if (CDcdb()->auxCellTab()) {
...
//
void
cAuxTab::at_remove_cb(bool state, void *arg)
{
    char *name = (char*)arg;
...
//
void
cAuxTab::at_clear_cb(bool state, void*)
{
...
//
void
cAuxTab::at_drag_data_get(GtkWidget *widget, GdkDragContext*,
...
//
void
cAuxTab::at_drag_data_received(GtkWidget*, GdkDragContext *context,
...
//
int
cAuxTab::at_btn_hdlr(GtkWidget *caller, GdkEvent *event, void*)
{
    if (AT)
...
// Static function.
int
cAuxTab::at_btn_release_hdlr(GtkWidget*, GdkEvent*, void*)
{
    if (AT)
...
//
int
cAuxTab::at_motion_hdlr(GtkWidget *caller, GdkEvent *event, void*)
{
    if (AT)
...
//
void
cAuxTab::at_resize_hdlr(GtkWidget*, GtkAllocation*)
{
    if (AT)
        AT->update();
}
...
//
void
cAuxTab::at_realize_hdlr(GtkWidget *w, void *arg)
{
