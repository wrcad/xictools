...
//
void
cCGDopen::apply_proc(GtkWidget *widget)
{
    int ret = true;
...
//
char *
cCGDopen::encode_remote_spec(GtkWidget **bad)
{
    if (bad)
        *bad = 0;
    sLstr lstr;
...
// Static function.
void
cCGDopen::cgo_cancel_proc(GtkWidget*, void*)
{
    delete Cgo;
}
...
// Static function.
void
cCGDopen::cgo_action(GtkWidget *caller, void*)
{
    if (!Cgo)
        return;
